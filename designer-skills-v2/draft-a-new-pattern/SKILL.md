...
1. Build the new thing **from the system's primitives**: tokens by intent (never
   raw hex/px), the type composites in `knowledge/canon/type.css` (never raw font
   values), square corners and sentence case (Apollo Mono), real icons from
   `knowledge/assets/icons/` only, the accessibility floor (AA contrast, focus
   ring, target size, reduced-motion, ARIA), and the motion tokens.
2. **Stay monochrome** — in Apollo Mono, colour is for RAG status + data-vis
   only; the only red is `#B92F1E` (status, never action/nav).
3. Give it **all the states** (default / hover / pressed / focus / disabled /
   loading / error / empty as relevant).
4. Name it consistently with the existing components.
5. **Document it** in the same shape as an existing component so it can slot in —
