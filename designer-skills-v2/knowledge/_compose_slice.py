...
# OPENED records every path the reader opens (s279-D1, #279 lane PK): the pack's allow-set is
# checked against THIS set by the selftest, so the read closure is derived from the code, never
# typed from memory — an exclusion that returns is a bite failing, not a silent fallback.
OPENED = set()
...
def selftest():
    ...
    # — s279-D1 (#279 lane PK, Dave: "i agree 'a' it is"): EVERY FILE THE READER OPENS IS INSIDE THE
    # PACK'S ALLOW-SET, so the `_rulings.json` exclusion can never silently return. The closure is
    # RECORDED (OPENED) from a seed + the 12 asks, never typed; it is checked against (1) the Spider
    # generator's allow-set with the s279-D1 block armed, (2) a fresh designer-skills-v2 bake, and,
    # where neither exists (inside a shipped pack), (3) the folder the reader sits in.
    OPENED.clear()
    s9 = build_slice(TASK_A)
    live9 = load_live()
    for q in ASK_12:
        _try(ask, q, seed=s9, live=live9)
    opened = {os.path.relpath(os.path.normpath(p), HERE) for p in OPENED}
    opened |= {os.path.basename(__file__), "_helpgate.py"}
    outside = sorted(p for p in opened if p.startswith(".."))
    bite("PACK allow-set: the reader opens %d files, all under its own folder (HERE), none by a path back to the repo" % len(opened),
         not outside, outside[:5])
    checked = []
    import importlib
    gen_py = os.path.join(HERE, "_release", "_gen_pack_manifest.py")
    if os.path.exists(gen_py):
        sys.path.insert(0, os.path.dirname(gen_py))
        _argv, sys.argv = sys.argv, ["_gen_pack_manifest"]
        try:
            gm = importlib.import_module("_gen_pack_manifest")
        finally:
            sys.argv = _argv
        _gv = gm.VERSION
        try:
            gm.VERSION = gm.READER_SHIPS_FROM
            tbl = gm.groups()
            shipped = set(gm.GATE_DATA_CANDIDATES) | {"knowledge/_helpgate.py"}
            unclaimed = sorted(p for p in opened
                               if not any(g["match"]("knowledge/" + p) for g in tbl) and "knowledge/" + p not in shipped)
            excluded = sorted(p for p in opened if any(("knowledge/" + p).startswith(x) for x, _ in gm.excluded_rows()))
        finally:
            gm.VERSION = _gv
        bite("PACK allow-set (Spider, _gen_pack_manifest armed at %s): every opened file is claimed by a group or rides as gate data — 0 unclaimed" % gm.READER_SHIPS_FROM,
             not unclaimed, unclaimed[:6])
        bite("PACK allow-set (Spider): no opened file is under an EXCLUDED row once the block is armed (the _rulings.json exclusion is gone)",
             not excluded, excluded[:6])
        checked.append("spider")
    bake_sh = os.path.join(ROOT, "designer-skills-v2", "build-designer-kb.sh")
    if os.path.exists(bake_sh):
        import subprocess
        tmp9 = tempfile.mkdtemp(prefix="compose-slice-bake-")
        try:
            r = subprocess.run(["bash", bake_sh], cwd=ROOT, capture_output=True, text=True,
                               env=dict(os.environ, SRC=HERE, DST=os.path.join(tmp9, "knowledge"), PROVENANCE="selftest"))
            missing = sorted(p for p in opened if not os.path.exists(os.path.join(tmp9, "knowledge", p)))
            bite("PACK allow-set (designer-skills-v2 bake, v2.1): the bake exits 0 and ships every opened file — 0 missing",
                 r.returncode == 0 and not missing, (r.returncode, (r.stderr or "")[-200:], missing[:6]))
        finally:
            shutil.rmtree(tmp9, ignore_errors=True)
        checked.append("v2-bake")
    if not checked:
        gone = sorted(p for p in opened if not os.path.exists(os.path.join(HERE, p)))
        bite("PACK allow-set (inside a shipped pack): every opened file exists beside the reader — 0 missing", not gone, gone[:6])
