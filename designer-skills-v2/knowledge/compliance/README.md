...
| `1.4.11` Non-text Contrast | dark-mode contrast audit joined via `tokens/_blast-radius.json`, PLUS a secondary snippet-contrastPair join for the 2 the blast-radius join couldn't reach | **8/8** applies_to components covered — all 8 **pass** — see finding below |
...
Widening the classifier to include border tokens (and, per WCAG 1.4.11's own "except for inactive components" text, explicitly excluding `*/disabled` tokens) changed the join to **6/8 covered**: Badge and Links remained `not_covered` (neither binds a border/brand/status token our audit models); Button, Status indicator, Account card, Cards, Divider, and Dropdown **all passed** — the last four only after the token-value fix below (e.g. Divider's own `divider/border/subsection` was sitting at 1.05:1 against `#1D1D1D`, nowhere near the 3:1 floor, before it was lifted). That's the point of the join: it surfaced an honest `fail` first rather than hiding behind a silent `not_covered` or an inflated `pass`, and that honest fail is what got it fixed. See `_VERIFICATION-EDGES.md` → Component-level breakdown, and `rules/wcag-1.4.11-non-text-contrast.json` → `verified_by.per_component`.

**Badge and Links resolved (2026-07-14, same day):** the remaining `not_covered` pair genuinely doesn't bind a border token — Badge's own boundary is its red fill against the page, Links' is its arrow and download-icon glyph, neither modelled by the border-token audit. Digging into each component's reference snippet found the evidence already existed: Links' `Links.reference.html` already declared two `"ui"`-context `contrastPairs` (the arrow, the icon) that the *blocking* snippet gate (`_validate_snippets.py`, check 3) has been checking every build all along — the compliance graph's `verified_by` join simply wasn't consulting that evidence source, so a real, currently-passing check was being reported as `not_covered`. Badge declared only a `"text"` pair for its number label, with no `"ui"` pair for the fill itself — so a matching pair (`primary/background/default` vs `background/default`) was added to `Badge.reference.html`, mirroring Links' pattern, bringing it under the same already-enforced gate. `_build_verification_edges.py` was extended with `snippet_ui_contrast_join()` — a secondary evidence path that, for any 1.4.11 component the blast-radius join can't cover, reads that component's own reference-snippet `contrastPairs` and resolves/computes any `"ui"`/`"icon"`-context entry (same `resolve()` walk and same `_contrast_utils` formula the snippet gate itself uses — not a new check, just surfacing evidence that already exists). Both resolved to real passes: Badge's fill at 5.22:1 (light) / 4.02:1 (dark) against the 3:1 floor, Links' arrow at the same ratios and its icon at 12.63:1 / 21.0:1 against the 4.5:1 floor (icon-015 promotion). `1.4.11` is now **8/8 pass**, 0 `not_covered`. See `_VERIFICATION-EDGES.md` → "Resolved via secondary evidence" and `rules/wcag-1.4.11-non-text-contrast.json` → `verified_by.secondary_evidence`.
...
- **Next for the verification/external-refs layer** (not this pass): (1) decide whether any of the 13 axe-core "easy win" SCs are worth actually wiring into the build (that import only catalogs availability, it doesn't adopt anything); (2) the ACT ingest, if it earns its own session. The former `not_covered` pair (Badge, Links) was resolved 2026-07-14 — see the finding above.
