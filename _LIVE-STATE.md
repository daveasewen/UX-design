...
*Last refreshed: 2026-08-15 (Sat from `date` — **#176 wrap**, and the DATE SEAM IS DECLARED: the session ran 2026-08-14 into the small hours, both rulings are dated 08-14, and this stamp is taken from `date` rather than from the session's own belief. FABLE conductor + FOUR OPUS build/measurement subs + this OPUS wrap sub, Dave live throughout, **TWO RULINGS, BOTH DAVE'S — THE STEP TRACKERS INHERIT THE SUCCESS ROUNDELS WHOLESALE, AND "INK" IS NOW A RULED PRINCIPLE**. ✅ **`s176-D1`** — quoted from the entry: *"'BLACKEST INK' RESOLVES PER THEME, AND IT IS THE BLACKEST INK THAT ISN'T PURE BLACK, TO AVOID HALATION."* Dave verbatim: *"the simple rule for ink is the blackest ink that isn't pure black, to avoid halation. #13110E for SC / #1a1a1a for Mono and console I think, that might not be right / #333333 for Legacy"* — he HEDGED Mono/Console and the hedge was resolved by **verifying against the token store BEFORE the read-back**: Mono `mono/4` #1A1A1A (R-D16) · Console LOCKED = Mono (R-D25) · SC `warm/2` #13110E (ADR-0014) · Legacy Grey 8 #333333 (col25-011). ⛔ **NO VALUE CHANGES** — the store already embodied the principle; the ruling inscribes the PRINCIPLE and its RATIONALE. **DISCHARGES `s175-D1`(f).** Confirmed firm: *"yes this is fine"*. ✅ **`s176-D2`** — ruled BY EYE off three live review pages: `step/complete` **aliases `rag/success`** (Mono #66CC8D · Console/SC #5DAC7B, mode-invariant); **LEGACY EXCEPTION #DB0011 light / #FFFFFF dark**, the correction sequence recorded verbatim (*"the red"* → *"warning red"* → *"Legacy error, sorry my mistake"* → off the v1 page, *"Use the primary red for Legacy"*, FINAL), and the dark=white leg **SUPERSEDES `s175-D1`'s Legacy red-both-modes by his word** (*"both are true"*); **thick tick** (`yes-thick.svg`) everywhere, Mono/Console/SC tick = dark ink #1A1A1A per `s122-D2`/`D3`, **`step/on-complete` MINTED**; **LEGACY BAR** `progress/complete` #333333 light / #FFFFFF dark, **declared per the fall-through class**; the **Legacy dark tick #1A1A1A is a DECLARED EXCEPTION** taken by the build (an exact roundel match would be white-on-white). ★ **THE CONTRAST DOCTRINE IS NOW HIS RULED RATIONALE**, verbatim: *"the intent/meaning is carried by the glyph a label… the label and the symbol must carry the contrast not the decoration"* ⇒ gated legs (tick-on-fill + label) **all pass 5.22–8.77 / 12.63–17.45**, fill-on-page and fill-on-track are **DECLARED ABSENCES with that rationale** — which is what resolves the six sub-3:1 light cells AND the two-session-old fill-on-track finding, **by doctrine, not by moving a value**. ⛔ **MINTED WITH EYES OPEN:** *"There will be reviews by others later so some changes might be made in the future, thats just the process. lets get these in for now!"* — ratified AS-BUILT, future revision EXPECTED, not a hedge on firmness. ⛔ **THE FINDING: THE MONO AND CONSOLE STEP INDICATORS PAINTED *NOTHING* AT HEAD** — `canon.css`'s TOKENS block had never been regenerated since #175 minted `step/complete`, so the token was in the store, bound in the snippets, and had **no `:root` definition to resolve against**; found by **measuring computed styles in a real browser** (the controller sub), never by reading the store, and **every gate stayed green** because a token that resolves to nothing is not a parse error. ★ **A ruling can be fully enacted in the STORE and absent from the ARTEFACT.** ⛔ **A GREEN THAT COULD NOT FAIL:** `gen_theme_cascade.py`'s selftest carried an **indentation defect** — clauses inside a wrong-scope `if`-branch that could only execute **after another assertion had already failed**, i.e. unreachable on the passing path. Fixed and **mutation-proven** (receipt 5 of the refinement sub). ⚠ **THE SERIALIZER CLASS BIT AGAIN AND WAS CAUGHT BY THE CHECK IT DEMANDS:** the first `s176-D1` inscription attempt went through `json.dumps` and **reformatted the whole rulings file, 549 lines churned** — caught by `git diff` before commit, reverted, redone as a textual tail insertion. Reported as a HIT, not a near-miss. ⚠ **THE INSCRIPTION TOOK `_governs --selftest` FROM 7 FAILS TO 14**, measured **both ends first-hand at this wrap** by swapping in `git show HEAD:knowledge/_rulings.json`, running, restoring **byte-identically** and re-running; all seven new fails name `s176-D1`/`s176-D2` (evidence strings with no `chat #<n>`/`commit ` pointer; four contrast figures read as non-resolving pointers). ⛔ **NOT REPAIRED — the entries are ratified record and Dave's.** ⚠ **`_validate_token_forks` IS RED — 4 `--complete` forks measured at this wrap** (the Legacy bar move is the new one); the HEAD baseline of 3 is the build sub's figure, RELAYED. ⛔ **The fork ledger is an UNRULED BASELINE and it is DAVE'S** — nothing declared, unified or added. ✅ **ALSO LANDED, both #175 residual items consumed:** the ~15 lines of **stale component prose** are FIXED and the **Legacy selftest mirror** is written. ⛔ `_build_all.py` **NOT RUN, by instruction.** ⚙ **GAUGE (relayed):** boot **57,400 real** vs the `s171-D1` band **56,158 ±849** ⇒ **~400 ABOVE the 57,007 ceiling — a datapoint OUTSIDE the band, DECLARED, never corrected into it and never used to re-base** · **FILL at wrap-open ~131,000 vs stop 150,929** ⇒ ~20K of room against a 42–49K conductor wrap, **which is why this wrap was delegated** · job window ≈ **73.6K real**, rung **`M`** · **`subs 603941 tokens (n=4)`** (77,576 + 127,464 + 186,874 + 212,027), this wrap sub's own spend EXCLUDED and unknowable · ⚠ **the brief's prose says THREE build subs while FOUR figures were given — declared, not reconciled** · `tiktoken` needed installing cold. ⚙ **PANEL, WITH NUMBERS: All 10% · Fable 10%, resets Thursday** — Dave: *"Fable and 'others' are tracking each other, thats what we're aiming for"* ⇒ **parity IS the routing target and it is currently hit**; and *"extra tokens… maybe we crank up the work in a couple of days"*. ✅ **MEMORY step 3 RAN.** ⛔ **NOT RULED:** the fork ledger · `step/incomplete` · the `step/on-complete` name beyond as-ratified · `s151-D1` · `s149-D1` · any colour value · the three review pages' content · the Legacy literal-vs-alias choice · the seven new `_governs` fails · `knowledge/_build_all.py` (**NOT RUN, NOT TOUCHED — `#166` labels are JOIN KEYS**). Dossier: `_DECISION-HISTORY/2026-08-15-176-the-step-trackers-inherit-the-roundels.md`.)*  *Last refreshed: 2026-08-14 (Fri from `date` — **#175 wrap**: ONE OPUS build sub + this OPUS wrap sub, Dave live, **ONE RULING, DAVE'S — THE RULE TURNS ON WHAT IS MEASURED, NOT ON WHAT IT LOOKS LIKE**. ✅ **`s175-D1`**, taken on plain-prose read-backs across four exchanges, unobjected: a **CONTINUOUS QUANTITY** (`Progress-bar`) is **INK ONLY in all four themes, RULED**; **DISCRETE STEPS** (`Progress-tracker`, `Stepper`) **MAY use colour** — Legacy **DEFINITE**, Mono/Console/Supercharge **UNDECIDED, FLOATED, NOT RULED**; and the **responsive collapse to a bar shape** at `@container (max-width:520px)` is **the SAME component**, which is the proof the rule is drawn on MEANING not on shape. His words: *"Just use the default ink colours (blackest) for all the progress bars, all themes, no red"* · *"the other ruling was a miscommunication. the only type of progress indicator that may use colour is the step tracker component"* · *"these will have colours in legacy for sure. Its undecided for teh other themes as yet"* · *"thats the right black for supercharge"*. ★ **IT SUPERSEDES A PRIOR DAVE RULING BY HIS OWN WORD** (the `progress/complete` `$note`, Dave 2026-07-21) — ⛔ the old text is **PRESERVED AND APPENDED TO**, never deleted. ✅ **THE #174 INK FLOAT IS DISCHARGED at reading 3, the NARROWEST**; ⛔ **`s151-D1` the two-red law and `s149-D1` the mono error ink camp are UNTOUCHED and NOT REOPENED**; the float is retired to `_retired/_FLOAT-174-dave-ink-clarification-DISCHARGED-175.md`. ✅ **BUILT** (one OPUS sub, `_BRIEF-s175-progress-ink-2026-08-14-v1.md`): `step/complete` **MINTED**, the Legacy (`#DB0011`) and Supercharge (`#B92F1E`/`#CC4333`) overrides **MOVED** onto it, the `progress/complete` overrides **deleted**, `Progress-tracker` + `Stepper` rebound, `Progress-bar` untouched as a file. **Net rendered change: Progress-bar only, Legacy and Supercharge only** — Legacy → `#1A1A1A`/`#FFFFFF`, Supercharge → `#13110E`/`#F7F6F4` through the ADR-0014 warm DNA tier to its OWN ink; **step components BYTE-IDENTICAL in all 8 theme×mode cells against a HEAD control**. ✅ **THE #174 BLAST-RADIUS CLASS DID NOT RECUR AND WAS FENCED DELIBERATELY** — the brief required naming which `canon.css` regions each generator owns and attributing every hunk; **verified FIRST-HAND by the conductor: 11 insertions / 11 deletions, two regions, ZERO unattributed**, no `--status-*` touched, no comment block deleted. ⚠ **CONTRAST CLEARED HONESTLY** (measured, 3:1 non-text): Progress-bar Legacy dark **1.75 → 9.15**, Supercharge dark **2.38 → 10.46**; ⛔ **the same pair STILL FAILS on the STEP components** (1.75 / 2.38) — the finding **narrowed**, it did not disappear, and it is **still DAVE'S**. ✅ **GATES rc=0 before and after** (rc captured directly, never through a pipe): snippet · a11y · radius `--strict` · coverage · icon · five `--check` generators · theme-cascade `--selftest`; **the type-composite ratchet was RED at HEAD at 1101 and is still EXACTLY 1101 — baseline, not regression**; the filtered state-contrast run **overwrote the tracked `_STATE-CONTRAST-AUDIT.md` and it was RESTORED BYTE-IDENTICAL**. **`machinery: 10 instrument / 52 feature`** — the 10 are a **RETARGET of an existing selftest assertion whose SUBJECT MOVED**, mutation-tested to prove it bites; ⛔ **NO NEW INSTRUMENT WAS BUILT** and the four-theme contrast checker stays queued (`s172-D3`). ⛔ **THE CONDUCTOR DECLARES ITS OWN OVERSPEND: it opened `GOOD-MORNING.md` at boot, ~26K — the exact reflex `_CHAIN.md` bans and the exact overspend #173 declared as its own finding ⑦, SECOND SESSION RUNNING**, and it is why this wrap was delegated; ★ **the read-chain cut is enforced by a COMMENT, not a gate**. ⚠ **A STALE PREMISE IN THE #175 TITLE was corrected at the opener** — it called the `--status-*` strip *uncommitted* when it was committed at `76b024c` and pushed. ⚠ **UNPROVEN, PRICED:** the full 76-snippet `_validate_state_contrast.py` exceeds the ~178s call cap and is **OWED TO CI**; `_build_all.py` **NOT RUN by instruction**. ⚠ **QUEUED ~15 lines:** `Progress-bar.reference.html` `knownFindings` + `components/progress-bar.meta.json:22,75` still describe the old red fill and the old contrast failure, **both now FALSE**, mirrored into `canon.css:8029`. ⚙ **GAUGE:** boot **56,547 real** (inside the `s171-D1` band) · **FILL at wrap-open 128,839 vs stop 150,929** ⇒ ~22.1K of room against a 42–49K conductor wrap, **which is why this wrap was delegated** · job window ≈ **72.3K real** · **`subs 116803 tokens (n=1)`**, this wrap sub's own spend EXCLUDED and unknowable · panel: **no numbers given this session; #174's qualitative *"spend freely"* stands — the WINDOW bound, not quota**. ✅ **MEMORY step 3 RAN.** ⛔ **NOT RULED:** the three UNDECIDED step themes · the per-theme-ink generalisation (**the INSTANCE was ratified, the PRINCIPLE was not**) · `step/incomplete` · the Legacy selftest mirror · the step-component contrast finding · `s151-D1` · `s149-D1` · `[45]`/`[50]`/`[107]`/`[113]` · the `progress-family` · the two stale inventory documents · `tooltip.tip` · base red 30 · the G-series · the palette reshape · `-graphic` · the CI sweep tier · `knowledge/_build_all.py` (**NOT RUN, NOT TOUCHED — `#166` labels are JOIN KEYS**). Dossier: `_DECISION-HISTORY/2026-08-14-175-the-rule-that-turned-on-what-is-measured.md`.)*  *Last refreshed: 2026-08-14 (Fri from `date` — **#174 wrap**: ONE OPUS build sub + this OPUS wrap sub, Dave live, **ONE RULING, DAVE'S — THE FIRST COMPONENT WENT THROUGH THE SCAFFOLD ROUTE AND ALL SIX GATES OF THE BRIEF ARE NOW CLOSED**. ✅ **`s174-D1`** closes gates 2–6 on a plain-prose read-back: **build the COMPONENT not a SCAFFOLDER** + a friction log (the route exists ⇒ what is missing is CONVENIENCE not CAPABILITY; a scaffolder now would be a speculative instrument under `s172-D3`) · **DETERMINATE ONLY** (the indeterminate state overlaps `Loading-indicator`, ownership unruled) · **no `component-type` family** (proposal only; `component-types.json` untouched) · **the two stale inventory documents are a SEPARATE LANE, after the build** · **one OPUS sub, one window, ~110–130K**, no Sonnet subs. ⇒ **the brief is CONSUMED**; `_rulings.json` **153 → 154**, tail `s174-D1`, by textual tail insertion with the s171/s172/s173 discipline (prefix byte-identical assert · 153 prior entries compared equal one by one · `_README` compared equal · SPLICE POINT asserted · tail read back). ✅ **THE PROGRESS BAR IS BUILT** — `knowledge/snippets/Progress-bar.reference.html`, `knowledge/components/progress-bar.meta.json`, `showroom/progress-bar.html`, `reviews/REVIEW-174-progress-bar-four-themes-v1.html`, `notes/_receipts/2026-08-14-s174-first-component-progress-bar.md`, `knowledge/_validate_radius.py` (`MIGRATED_SNIPPETS` +1, strict from birth). Gates RUN and PASSED: snippets 76/0 · a11y 0 · radius 0 strict · coverage 76/76 · icons 0 UNKNOWN · type-composites **debt 1101 HELD, the new file contributes 0 — the only clean file of 91** · five `--check` generators; ⛔ `_build_all.py` NOT RUN by instruction. Render: **all 8 panes correct**, `fillW/trackW = 260.4/420 = 62.0%` matches `aria-valuenow="62"`, **font asserted by canvas measurement against two controls** (347 / DejaVu 375 / nonexistent 301). ⚠ **COULD NOT RUN, DECLARED:** the full 76-snippet `_validate_state_contrast.py` exceeds the ~178s call cap; the filtered run is clean rc=0 in a real browser, it overwrote the tracked `knowledge/_STATE-CONTRAST-AUDIT.md` **which was restored byte-identical**, and **a full population run is OWED TO CI**. ★ **STEP 0 CAUGHT TWO STALE PREMISES** — HEAD `be1e0a7` not `e5ab8ee`, and the brief's *"will not appear in the index"* is FALSE (`gen_showroom.py:483` = `CAT_OF.get(slug, "More")`). ⚠ **A CONTRAST FAILURE FOUND BEFORE BUILDING AND NOT FAKED INTO A PASS:** fill-on-track fails 3:1 in **Legacy dark (1.75:1)** and **Supercharge dark (2.38:1)**; ⛔ deliberately NOT declared as a gated `contrastPair` (that would be a green that cannot fail in two themes) — meaning is carried by a numeric value in text plus `aria-valuetext`; pre-existing, shared with `Progress-tracker`, queued to `knowledge/_DS-IMPROVEMENTS.md` with three options, **DAVE RULES IT**. ⛔ **THE FINDING: A DEFERRED, DAVE-OWNED CHANGE RODE IN AS A BUILD SIDE-EFFECT** — regenerating `canon.css` also ran the theme-cascade generator, which **stripped 25 explicit `--status-*` declarations from Legacy, Console and Supercharge** (conductor-verified: 25 removed, 0 added) and **deleted the hand-written `#168`/`#168-A` comment block from inside the generated region**; those are residual `[50]` and `[45]`, both on the do-not-rule list. ★ **A do-not-rule list names DECISIONS; it cannot fence a GENERATOR'S BLAST RADIUS.** ⬛ **DAVE'S RESPONSE IS FLOATED, NOT RULED, recorded verbatim:** *"lets keep it nice and simple, all of them use the appropriate dark ink on white and the lightest ink for dark"* — read back as a background-keyed ink rule, ambiguity declared openly, regenerated state left in place; ⛔ **it is NOT `s174-D2`**. ★ **THE FRICTION LOG IS THE DELIVERABLE — nine items**, the largest being that **the snippet gate is single-theme by construction** so any four-theme contrast claim must be computed by hand outside it. ✅ **TWO REDS PROVED PRE-EXISTING WITH CONTROLS** (`gen_theme_cascade --check` already rc=1 at HEAD; the `gen_canon_components.py` deletion isolated by moving the component out of tree). **`machinery: 0 instrument / 274 feature`** — a four-theme contrast checker was **QUEUED, NOT BUILT**. ⚙ **GAUGE:** boot **56,342 real** (inside the `s171-D1` band) · **FILL at wrap-open 94,056 vs stop 150,929** ⇒ ~56.9K of room against a 42–49K conductor wrap, **which is why this wrap was delegated** · job window ≈ **37.7K** · **`subs 192927 tokens (n=1)`**, this wrap sub's own spend EXCLUDED and unknowable · panel asked and answered, **all three roomy — *"spend freely"***. ✅ **MEMORY step 3 RAN.** Dossier: `_DECISION-HISTORY/2026-08-14-174-the-first-component-and-the-generator-that-rode-in.md`.)*  *Last refreshed (#173 and earlier, rolled #176 — 2d boundary): full text moved to `_LIVE-STATE-ARCHIVE.md` § Rolled 2026-08-15 #176, § Rolled 2026-08-14 #175, § Rolled 2026-08-14 #174, § Rolled 2026-08-14 #173, § Rolled 2026-08-14 #172, § Rolled 2026-08-14 #171, § Rolled 2026-08-14 #170, § Rolled 2026-08-13 #169, § Rolled 2026-08-13 #168, § Rolled 2026-08-13 #167, § Rolled 2026-08-12 #164, § Rolled 2026-08-12 #163, § Rolled 2026-08-12 #162, § Rolled 2026-08-12 #161, § Rolled 2026-08-12 #160, § Rolled 2026-08-12 #159, § Rolled 2026-08-12 #158, § Rolled 2026-08-12 #157 and § Rolled 2026-08-11 #156.*
...
## ⏱ PRIOR DELTA — 2026-08-14 (Fri **#175**, ONE OPUS build sub + this OPUS wrap sub, Dave live, **ONE RULING, DAVE'S — THE RULE TURNS ON WHAT IS MEASURED, NOT ON WHAT IT LOOKS LIKE** — ✅ **THE #174 INK FLOAT IS DISCHARGED AT ITS NARROWEST READING** · ✅ **THE #174 BLAST-RADIUS CLASS DID NOT RECUR** · ⛔ **THE CONDUCTOR OPENED `GOOD-MORNING.md` AT BOOT AGAIN**)
...
## ⏱ PRIOR DELTA — 2026-08-14 (Fri **#174**, ONE OPUS build sub + this OPUS wrap sub, Dave live, **ONE RULING, DAVE'S — THE FIRST COMPONENT WENT THROUGH THE ROUTE, AND ALL SIX GATES ARE CLOSED** — ⛔ **THE FINDING: A DEFERRED, DAVE-OWNED CHANGE RODE IN AS A BUILD SIDE-EFFECT** · ⬛ **HIS ANSWER TO IT IS FLOATED, NOT RULED** · ★ **THE FRICTION LOG IS THE DELIVERABLE**)
