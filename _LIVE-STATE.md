...
- ⏳ DV-J2b — sparkline toggle markup + CSS (JS already injected, dormant) — queued · receipt ⛔ SUPERSEDED — READ THE RECEIPT, NOT THE STATE WORD. The state stays `queued` ONLY because `_gen_lanes.py`'s STEP_STATES is a CLOSED enum ('landed','active','queued') and widening it is a schema ruling, not a wrap sub's (declared #185, priced TODO). knowledge/_proforma/_DATAVIZ-DECISIONS.md § Open/pending (split from DV-J2 by Dave's ruling #27 — scatter half only, to keep the diff attributable) · SUPERSEDED s182-D2 (#182): sparkline is an atom alone; table CTA moved to the future trend-card component (floated, not ruled). Successors: trend-card composition (needs Dave's word) or sparkline colour/height-snap at the tuner.
...
## ⏱ PRIOR DELTA — 2026-08-16 (Sun from `date`) (**#184**, FABLE conductor + **TWO OPUS build subs** + this OPUS wrap sub, Dave LIVE throughout, **THREE RULINGS, DAVE'S — `s184-D1` · `s184-D2` · `s184-D3`**)
