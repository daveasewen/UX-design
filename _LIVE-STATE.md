...
*Last refreshed: 2026-08-02 (Sun from `date` — #81 wrap: `ds-021` shape (C) RULED by Dave and BUILT, and then the session's real finding arrived as HIS question — the knowledge-transfer defect. #54's ruling was stored TEN times and reached nobody, because retrieval is triggered by suspicion. TRIGGER INDEX built as a pair (`_rulings.json` + `_governs.py`), fires off `git diff`, wired into the gate in both modes. 2c/2d/2f run via `_gm_move.py`, dry-run first, receipts in stdout. Previous: 2026-08-02 #80.)*
