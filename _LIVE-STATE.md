...
*Last refreshed: 2026-08-02 (Sun from `date` — #80 wrap: #79-D1 built (the gauge REFUSES, paired + mutation-tested ×3), P5 promoted to ABORT on Dave's word, and a five-whys that found the project's "tape" unit is an OpenAI tokenizer under-reporting by ~36% against a REAL-token budget — UNRULED, his. 2c/2d/2f run via `_gm_move.py`, dry-run first, receipts in stdout; residual line GENERATED. Previous: 2026-08-02 #79.)*
