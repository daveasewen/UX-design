...
*Last refreshed: 2026-07-30 (Thu #48 — open 16 (a) ENACTED: the cap now measures the whole `_CHAIN.md` FILE, not the slice, via a SEPARATE `chain_file_tk()` because patching the slicer in place would have been unbounded recursion; `CHAIN_BUDGET_TK` restated (4500,6000)→(4917,6417) on the measured 417-tape wrapper so the verdict is arithmetically unchanged; open 15 UNBLOCKED; a control proved the new bite bites) · Previous: #47 (Dave RULED open 16 (a)+(c); (a) proved UNBUILDABLE as written because the footer publishes the size of the file containing it, so (c) landed as a FIXED POINT and the stamp is exact by construction, `62b6e1e`; the build no longer fits one sandbox call, which #46 had already found and filed where nothing reads it)*
