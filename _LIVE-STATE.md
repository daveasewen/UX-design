...
- ⬛ **POST-WRAP ADDENDUM (5b) — HOMED HERE AND NOT ON THE ★ LATEST BANNER, DECLARED RATHER THAN SLIPPED** (the `s241-D2` cap is BLOCKING at 10 substantive lines / 1,200 tape and this banner closed at **1,179 / 10**, so an eleventh line would be refused, correctly). ⛔ **This is the ELEVENTH wrap to meet the 5b-versus-cap conflict and the TENTH to answer it this way; it is ruling-shaped and it is his.** The wrap commit sha, the push verdict verbatim and the CI read job-by-job — polled to completion, never stopped at a colour that was still moving — are in the filed report `notes/_subreports/2026-09-21-291-W-wrap.md` § THE ADDENDUM and in the returned stub. ⛔ **The sha is a declared gap in the `COMMIT STATE #291` block: a commit cannot name itself.**

- ⛔★★ **5b, THE ADDENDUM ITSELF — sha, push verdict and CI, written to the filed report FIRST and quoted here and into the returned stub, never the other way round (`s241-D2` record-first-then-quote).** **WRAP COMMIT `753d48a100b928ea14091ada8872da9924673eff`**, the session's THIRD commit. ⚠ **The script REFUSED ONCE FIRST and the refusal was correct:** MENTION-MAP — `knowledge/_graph-mention-map.json` stale, regenerated by the script, which then refused to stage a path this seat had not named (P5). **#288's, #289's and #290's exact trap, met a FOURTH time and paid the same way: the path appended, a FRESH msgfile with a FRESH name, a clean run, exit 0.** ✅ **No `.git/index.lock` was stranded at this seat** — the per-session delete grant was still active and `rm` worked. **PUSH VERDICT, verbatim: `✅ pushed and VERIFIED: remote master == local 753d48a100b928ea14091ada8872da9924673eff`**, re-verified independently by `git ls-remote origin HEAD`, range `b721144a..753d48a1`; ⛔ **the conductor's own two commits were ALREADY pushed when this ritual opened, so this push carries the wrap alone.** **CI: run `35575122198` on `head_sha=753d48a1` — RUN completed / failure, read JOB BY JOB over the public API and POLLED TO COMPLETION: ⛔ gates FAILURE (steps 5 and 6) · ✅ render SUCCESS · ⛔ release FAILURE at step 12 `Build-script selftest — the release refusals, driven (BLOCKING)`.** ⛔ **Every red is INHERITED and none was born here** — the CI on the conductor's own `b721144a` was also read at this seat and returned the IDENTICAL three colours (run `35573076214`), as did #289's `77a77eea` and #290's `7e0227fd`; **the release red is `_drive_chart_engine.py`'s un-run re-drive, now FOUR sessions old and #292's.** ⚠ **`render` stayed `in_progress` for roughly ten minutes after the other two had closed; the verdict is the one taken after it finished, not the one available first.** ⚠★ **AND THE `s214-D6` CHAIN FIGURE HAS TWO READINGS, BOTH PUBLISHED — the #240 lesson met again and DECLARED rather than discovered: the stratum carries 10,792 tape, taken after the declare-last `size:` stamp and before 5b exactly as the #241 rule prescribes, and THIS LINE LANDS INSIDE THE ⏱ LATEST DELTA, WHICH IS THE CHAIN SLICE, so the regenerated chain reads larger — the post-5b reading is `_CHAIN.md`'s own generated footer, exact by construction and `--check`-blocked, and it is the figure the NEXT SESSION actually pays. ⇒ Neither reading is rewritten** [[measure-dont-convert-units]].
