...
## ⏱ PRIOR DELTA — 2026-08-28 (Fri from `date`) (**#222**, one day, FABLE conductor + 5 MEASURED Opus subs (mono enact · encoder vendoring · fallback encoder · route (a) · release plan) + this OPUS wrap sub, DELEGATED wrap, ★★★ **THREE RULINGS INSCRIBED `s222-D1` · `s222-D2` · `s222-D3`** — the day the release became the only lane, and a designer's broken machine ruled it)

*(WHY/HOW: ✅ **A `_DECISION-HISTORY/` DOSSIER WAS WRITTEN — `_DECISION-HISTORY/2026-08-28-222-the-release-becomes-the-only-lane.md`** (row `W-252`): why a designer's failed first hour outranked a design lane, how an out-of-the-box ruling grew a second ruling the same afternoon, why the fallback lane STOPPED at the frozen shim instead of crossing it, and why the four findings nobody went looking for are the reason the deliverable is a plan and not a bake.)*

- ★★★ **`s222-D1` — MONO'S GALLERY DEFAULT IS DECIDED, AND `s220-D2`(3)'s EXPRESSLY OPEN CLAUSE IS CLOSED.** Off `reviews/MONO-GALLERY-DEFAULT-2026-08-27-v1.html` at the opener, Dave: *“yes the defaults for mono are fine”* and, on the reflected reading, *“yes this is correct”* ⇒ **square image + transparent caption, console's colours, BOTH MODES**; the grey caption ground **stays an edit-pass option** and nothing leaves the grammar. Enacted at `d4e69d0`; report `notes/_subreports/2026-08-28-222-mono-default-switch.md` (row `W-242`). ⚠ **The mono DARK CHORD is still expressly open.**
- ⛔ **THEN A DESIGNER'S MACHINE CHANGED THE DAY.** The **first live Copilot-bridge session** — Dave's own machine, row `W-244`, home `apollo-spider/FIRST-SESSION.md` — refused the chain inscription because tiktoken's encoder data could not be fetched: **the L4-predicted first-hour stumble, now real evidence rather than a prediction.** His unblock on the day was the `TIKTOKEN_CACHE_DIR` export. His words: *“I need this to work out of the box for the designers.”*
- ★★★ **`s222-D2` + `s222-D3` — THE PACK MEASURES OUT OF THE BOX, AND THE RELEASE OUTRANKS EVERY OTHER LANE UNTIL BAKED.** `s222-D2` vendors the **cl100k_base encoder data inside the pack** (no download, no env var, no reachable blob host; the honest refusal stays for genuinely broken installs) — enacted `36754e2`, report `notes/_subreports/2026-08-28-222-encoder-vendoring.md` (row `W-245`). `s222-D3` picks **option B off a priced A/B surface**: a **pure-Python EXACT fallback encoder** over that same vendored data, live only when `import tiktoken` fails, **engine NAMED in every output, never silent** — and rules the sequencing.
- ★★ **THE EQUALITY GATE IS THE HARDEST NUMBER OF THE DAY, MEASURED NOT ASSERTED: 1,910 files · 69,483,685 chars · 21,813,749 tokens · ZERO divergent tokens**, and the two engines generate a **byte-identical `_CHAIN.md`** (sha256 `fe70784b…` both). Report `notes/_subreports/2026-08-28-222-fallback-encoder.md` (row `W-247`). ⛔ **That lane then STOPPED at the frozen shim rather than cross Dave's `#64` boundary**, and priced route (a) in its own ⑤ Q1 instead.
- ✅ **ROUTE (a) ENACTED, AND THE RED WAS RED BY CONSTRUCTION.** A fifth sub re-ported `measure_tokens` in the source and **both** frozen shim copies (**33 lines each, byte-identical**); report `notes/_subreports/2026-08-28-222-route-a-report.md` (row `W-250`). The day landed as **four pushed commits** — `d4e69d0` · `36754e2` · `ba2c9f5` (wire, **delta gate red by construction**: the remedy's second half needs a sha that does not exist yet) · `789f433` (pointer bump, **delta gate green**).
- ⚠ **FOUR FINDINGS NOBODY WENT LOOKING FOR, EACH VERIFIED IN ITS FILED REPORT.** The **ship roster has silently drifted 55 → 56** against `s219-D9`'s ruled 55 — two browser gates die on a raw traceback in a third state nobody covered (playwright *imports* but its browsers are missing), so **which gates ship depends on what is installed on the machine that runs the probe** · **`RATIFY_ID` is pinned to `s219-D10`, making `--release` ratification a NO-OP for v-next** · **the version bump lives in FOUR homes, not two** · **`_validate_package_delta.py` arm 2 has never audited the shim and a real re-port passed straight through it.**
- ⬛ **THE DELIVERABLE IS A PLAN, NOT A BAKE — AND NOTHING IN IT IS DECIDED.** `notes/_briefs/2026-08-28-222-release-plan-v1.md` (row `W-249`): every loose end to the v-next bake in priority and dependency order, naming **five decisions that are his** (route (a) retro-confirm · the version number · his fresh ratifying word and whether ratify stops keying to a `#219` id · the blind delta-gate arm — repair, re-scope or park · his eye on the release page and the word “bake”), **plus item 5's roster count if it still moves and item 12, his fifteen minutes in Copilot, after.** **PARKED BY HIS OWN WORD:** `W-217`'s close, the mono dark chord, the mutation-arm call, the whole bento sitting.
- ⚙ **GAUGE — FOUR MOMENTS, NONE ROUNDED INTO ANOTHER** [[measure-dont-convert-units]]. **boot 75,422 real — ⛔ OUT of the `s208-D1` band by 17,519, the TWELFTH consecutive**, declared at the opener and logged once. **brief-cut FILL 268,020 real** (from the transcript's own usage record at the brief-writing turn) · **sub-cut 272,491 real over 117 turns, first-hand** (delta **+4,471**). ⛔ **TWO CONDUCTOR MISSES, HIS OWN HONESTY CLAUSE: the ~190,000/200,000 crossings were declared LATE, at the seam, and the `s214-D6` recall probe was NEVER PLANTED** — the band ran degraded-declared, mechanical-only after the seam. FILL then crossed the **256,000 UNQUALIFIED WALL (266,423 at the declaring check-in)** and the session wrapped at the wall.
- ⛔ **NOT RUN / NOT CLAIMED BY THIS WRAP:** no memory step 3 (NON-REPO, **the conductor's, DECLARED SKIPPED**) · no CI read-back (`gh` absent; the verdict stays the conductor's, `s203-D1`) · no bake, `apollo-spider/dist/` untouched · no `_build_all.py` · **no write to `knowledge/_rulings.json` — `git status` shows it CLEAN, which is the mechanical proof this seat read the store and never wrote it.**
- ⬛ **OPEN AND DAVE'S, CARRIED FORWARD:** the **five release-plan decisions** · **item 5's roster count under `s219-D9`** · his **fifteen minutes in Copilot** · the **mono DARK CHORD** · **`W-217`, expressly still open** · the **97 open sitting calls + 25 candidates, unchanged** · the **`s208-D1` re-base** (twelfth drift; his boot-REDUCTION rider binds) · his **end-of-day quota reading, owed in chat and NOT invented**.
