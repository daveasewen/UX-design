...
- **OPEN (Dave's):** the **one-point slack** (`≤ 45` vs GREEN `< 45` — the LITERAL ruling shipped) · **HOLE lines for #9/#10/#11/#19 NOT backfilled** — a HOLE asserts *"wrote no stratum"*, and what exists is evidence of **absence** only · whether the strict `RESERVE SPEND` marker stays strict · the M10 numbers + the disarmed 28,000 trigger (untouched) · **job 1 — make the usage data bite — DISPLACED TO #35**, dropped at the door rather than squeezed in.
