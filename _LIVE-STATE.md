...
- ✅ **`s168-D4` — THE `rag/*-graphic` LIGHT LEGS REUSE THE RULED TEXT VALUES.** Dave: *"we already have red and green values for text in light mode, can we use those… choose a new amber and blue. I'm happy with the current colours in dark mode."* Light error **`#DA1A00`** (`s151-D1`) · light success **`#137F3C`** (`s155-D1`/`s144-D1`); **all four DARK legs stay** (`#CC4333`/`#C58900`/`#4A9568`/`#2674DC`). **Subsumes and ratifies 82-B as-built**, pending his eye on specimen v2. ⚠ **Rung name `-graphic` = declared default, UNRATIFIED.**
- ⚠ **AND THE HONEST HALF: the light amber `#C58900` and light blue `#306EC6` in `knowledge/canon/canon.css` are ENACTED-PENDING-EYE PLACEHOLDERS** — ⛔ not picks, not ruled values; they are what `reviews/REVIEW-168-graphic-amber-blue-picker-v1.html` exists to replace. ★ A placeholder inside a token file is how an unruled colour becomes canon by attrition.
