...
*Last refreshed: 2026-08-02 (Sun from `date` — #84 wrap: **a premise died on the primary record of the ruling it cited.** The `.md` prose gate Dave ruled at #84-D1 was BUILT, mutation-tested ×5 and UNWIRED the same session on a reason read off a banner; #81-D1 keeps the tape/bill MACHINERY and the same record calls the PROSE retired. Gate re-measured 1-for-2, not 0-for-1; its own positive control is GREEN on the rot. Pre-flight missed for the FIFTH session — `check_preflight` grades a stamp but nothing makes its ABSENCE fail.)  2026-08-02 #82.)*
