...
### ⬛ DAVE'S — the SIX parked consequences of the #122 mark-map pass (born #122, standing)
...
### ⛔ `gen_canon_tokens.py` DESTROYS the hand-authored TOKENS atoms if run — PRICED, UNFIXED
The canon `TOKENS alpha` / `marks` / `mark-carriers` atoms sit inside AUTO markers but have **no
store origin**, so a `gen_canon_tokens.py` run rewrites the AUTO span and takes them out. Pre-existing
`s121-D1` defect, surfaced again #122, **priced TODO, not fixed** — nothing in the build currently
runs it, which is why it has stayed survivable rather than becoming an outage.
[born #121 · restated #122 · guards: this line · until: the atoms get a store origin or the generator learns to preserve them]
