...
### ⬛ `s125-D1` — RULED, **NOT ENACTED**: the chain banner's build-step count (born #125, rolls to #126)
**Dave ruled** that the build-step count in the chain banner stops being a typed number and becomes a
**GENERATED figure** — `knowledge/_gen_chain.py` reads `len(STEPS)` out of `knowledge/_build_all.py`'s
AST at generation time. He chose this **over a third re-stamp**, explicitly. ⛔ **The window ran out and
it was NOT BUILT.** ⚠ **The banner still says 75; disk measured 98 at this wrap (AST, `len(STEPS)`).**
The stale number is **marked as known-stale in GOOD-MORNING.md rather than re-stamped**, because
re-stamping is the act the ruling forbids. ★ This is the session's through-line instance 1: a perishable
prose number that the `size:` stamp gate cannot see, because **no gate parses prose**
[[no-gate-parses-the-artefact]]. Ledger: `knowledge/_rulings.json` · `notes/_MEMENTO-DECISIONS.md` § ★ #125.
[born #125 · guards: this block + the ruling · until: `_gen_chain.py` generates the figure]
...
cannot drift), v1 KEPT and TOMBSTONED.** ⛔ **Neither has happened; both roll to #126.** ⚠ The v1 file is
still live and still asserts the stale figures — that is instance 1 of the through-line in a second medium.
[born #125 · guards: this block · until: v2 lands and v1 carries its tombstone]
...
⇒ **`_RUNBOOK-render-verify.md` needs a RE-VERIFY at #126** — a fresh, deliberate download attempt whose
...
[born #125 · guards: this block · until: #126 re-verifies the runbook end to end]
