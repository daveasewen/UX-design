...
*Last refreshed: 2026-07-30 (Thu #49 — open 15 CLOSED after four sessions: `CHAIN_STAMP_RE` gives the chain figure a live assertion against `chain_file_tk` (the FILE), scoped to the `size:` stamp and held there by a control, banning PRESENCE rather than drift because a drift check passes the very wrap that re-adds the figure; ★★ the positive control found more than it was aimed at — `_gm_fixture` had been minting the retired form into every green fixture since #34; ⛔ the FAIL-vs-WARN tier is agent-picked and awaiting Dave) · Previous: #48 (the cap binds the FILE via a SEPARATE `chain_file_tk()`, patching the slicer in place being unbounded recursion; `CHAIN_BUDGET_TK` restated (4500,6000)→(4917,6417) on the measured 417-tape wrapper, verdict unchanged; open 15 unblocked)*
