...
|---|---|---|---|
| **Components** | Component graph — one `*.meta.json` node per component (props, variants, token bindings, relationships, anti-patterns, a11y, token-validation, provenance) | `components/` (101 metas — count registered as ASSERT-009, re-tested not repeated) | Look up a component's contract before generating code; traverse `relationships` edges |
