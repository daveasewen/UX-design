...
|---|---|---|---|
| **Components** | Component graph — one `*.meta.json` node per component (props, variants, token bindings, relationships, anti-patterns, a11y, token-validation, provenance) | `components/` (138 metas — count registered as ASSERT-009, re-tested not repeated) | Look up a component's contract before generating code; traverse `relationships` edges |
...
9. **snippet gate** — authored reference snippets in `snippets/` validated against the store: declared token values match (light+dark), contrast pairs pass, real focus indicator. **Gates** on drift. (`requiredAria` is NOT checked here — s263-D11 retired the source-text arm because it failed open; the one gate for that rule is the DOM-driven arm in `_validate_dataviz.py`.)
