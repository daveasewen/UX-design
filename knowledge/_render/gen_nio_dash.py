...
OUT = os.path.join(KNOW, "_fitness-test", "nio-dash-console-v1.canon.html")
...
def build():
    ...
    donut_chart = f'''<figure class="dv" data-dv-type="donut" data-total="{SPEND_TOTAL}" data-surface="page" data-labelling="spider" role="group" aria-labelledby="sp-h">
      ...
      <div class="dv-stage"><div class="dv-donut-row">
        ...
        <ul class="dv-leg vert t-cm-chart-label" id="sp-legend" role="group" aria-label="Categories — uncheck a swatch to dim it, click a name to isolate — then check swatches to add — Reset to show all">
        {legs}
...
<title>Nio business-banking overview — Apollo Console, composed from canon</title>
...
<!--
  ...
  HOW IT IS BUILT
    ...
    · ZERO font-size and ZERO font-weight declared on this page. Every text element carries a
      .t-cm-* / .t-ed-* composite. The <style> below is LAYOUT HARNESS ONLY (.nio-*): no #hex,
      no .c-*/.cn-* redefinition, no component CSS.
  ...
  MAPPING TABLE: nio-dash-console-v1.mapping.md, beside this file.
-->
<style>
  /* ================= LAYOUT HARNESS ONLY — no colour values, no type, no .c-*/.cn-* ========= */
  body{{ margin:0; background:var(--surface-hover); color:var(--text); }}
  .nio-shell{{ min-height:100vh; display:flex; flex-direction:column; }}
  .nio-shell > main{{ flex:1; }}
    ...
    <section class="tpl-section" aria-label="Accounts and spending">
      <div class="nio-two">
        ...
        <div class="stat-card tpl-panel nio-flush">
          ...
          <div class="cn-list-items nio-acct-list">
