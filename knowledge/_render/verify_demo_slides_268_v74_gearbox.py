...
v7.4 is THE PARTS BIN: a line-drawn 3D gearbox, hidden-line rendered,
orbiting on the mouse. Dave, verbatim: "The parts bin — do you think we
could have a line drawn 'engineering' style 3d diagram of a gearbox, it
might even respond to mouse movements too??"

What this file proves, over and above the v7 chassis:
  ...
  G5  THE DRAWING SITS INSIDE THE CARD with >= 5% margin on every side.
...
Usage:
  source knowledge/_render/seat_env.sh && \
    python3 knowledge/_render/verify_demo_slides_268_v74_gearbox.py
"""
import os as _hg_os, sys as _hg_sys  # noqa: E402 - help gate (#158 write-by-default class; guarded #304 R1)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        if st["solid_segments"] < 400:
            fails.append(f"only {st['solid_segments']} solid segments — the drawing is thin")
        if st["hidden_segments"] < 100:
            fails.append(f"only {st['hidden_segments']} hidden segments — "
                         "hidden-line rendering is not happening")
        ...
        for k, v in margins.items():
            ...
            if v < gate:
                fails.append(f"drawing bbox margin {k} {v:.1f}px < 5% ({gate:.1f}px)")
        ...
        # the structural half of the same claim, on the output gear itself:
        # at rest its FRONT face must be entirely solid and its BACK face
        # entirely dashed — 180 profile edges each way.
        aud = probes.get("output_gear_faces") or {}
        report["output_gear_faces"] = aud
        near = ("back" if aud.get("back_hidden", 1) == 0 else "front")
        far = "front" if near == "back" else "back"
        report["output_gear_near_face"] = near
        if aud.get(near + "_hidden", 1) != 0 or aud.get(near + "_solid", 0) < 150:
            fails.append(f"the output gear's camera-side face is not wholly solid: {aud}")
        # the away-side face is dashed EXCEPT where its edge is shared with a
        # tooth flank that does face us — that is the hidden-line rule, not a
        # leak, so the gate is "a substantial share dashed", not "all".
        if aud.get(far + "_hidden", 0) < aud.get(far + "_solid", 0) * 0.5:
            fails.append(f"the output gear's away face is barely dashed: {aud}")
    ...
    keys = ("stats", "mesh", "fps_mouse_moving", "view_rest", "view_far_left",
            ...
            "edge_readings", "output_gear_faces", "output_gear_near_face", "opacity", "card_corner_rgb", "gb_running_off_section",
