/* dv-render-combo — the Chart-combo's column + line on one canvas TYPE PARTIAL for the dv-render engine.
   ...
   claimed the name, "the combo type partial is NOT BUILT yet" means someone has and has not
   finished. Fail loud, and say which.

   WHAT THE LANE THAT FILLS THIS OWES: the bar partial's rects for the column series plus the line partial's polyline for the overlay series, sharing ONE band axis and (today) ONE value scale.
...
(function () {
  ...
  function combo() {
    throw new Error('dv-render-combo: the combo type partial is NOT BUILT yet — ' +
      'fill knowledge/canon/dv-render-combo.js (registered stub, #259 engine-core lane)');
  }
  combo.axis = 'y';
  window.dvRender.types['combo'] = combo;
}());
