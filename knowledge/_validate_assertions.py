...
class RootUnreachable(RootUnresolved):
    """#193: a KNOWN root that THIS environment cannot reach (e.g. 'mount' outside the
    sandbox). Subclass of RootUnresolved so untaught callers keep failing safe; taught
    callers read it as the COULD-NOT-ASK class (exit 77), a fact about the environment."""
    pass
    ...
    while True:
        ...
        if parent == p:
            raise RootUnreachable(
                f"root 'mount' unreachable HERE: no '<session>/mnt' ancestor of ROOT={ROOT} "
                f"(running outside the sandbox mount) — an environment fact, not a registry defect")
...
def check(a):
    ...
    try:
        return verb(pred)
    except RootUnreachable as e:
        # #193 COULD-NOT-ASK: a KNOWN root this environment cannot reach. NOT a pass,
        # NOT a fail, never silently re-pointed at ROOT — untestable HERE, and says so.
        # Failing it here dressed an environment fact as document rot (first seen:
        # ASSERT-007 in the first CI chained build).
        return "REFUSED", f"ROOT UNRESOLVED — {e}"
    except RootUnresolved as e:
        # LOUD and NAMED. An UNKNOWN root is a REGISTRY defect and fails in every
        # environment — never defaulted, never excused as a refusal (M2 bites this).
...
def run():
    ...
    for a in assertions:
        ...
        mark = "✓" if ok is True else ("⊘" if ok == "REFUSED" else "✗")
        print(f"  [{mark}] {a['id']} {a['kind']:<12} {detail}")
        if ok == "REFUSED":
            refusals.append((a, detail))
        elif not ok:
    ...
    if fails:
        ...
        return 1
    if refusals:
        # #193 COULD-NOT-ASK: no claim is false; some are untestable HERE. Third verdict,
        # counted and printed, never silenced — and never dressed as rot.
        named = "; ".join(f"{a['id']}: {d}" for a, d in refusals)
        print(f"\nCOULD-NOT-ASK: assertion gate — {len(refusals)} of {len(assertions)} "
              f"claim(s) untestable in this environment, 0 false — {named}")
        return 77
