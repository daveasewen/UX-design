...
## ⭐ T-D15 — RULED: the mini control ramp 12/14/16 minted (2026-07-24)

**Dave, on the mini-ramp proposal (`reviews/MINI-TEXT-RAMP-2026-07-24-v1.html`): *"mint the 3-step ramp at
500 … but with 16 at 400."*** Enacted:

- **Three composites in `canon/type.css`:** `.t-cm-ctl-12` (12/500 / `--slot:16px`), `.t-cm-ctl-14`
  (14/500 / `--slot:20px`), `.t-cm-ctl-16` (16/**400** / `--slot:20px`). A label-/control-class item steps
  up or down the ramp without a bespoke size each time.
- **Weight ladder is per-role, not monotonic (cf. T-D14):** 500 at 12+14 (control weight — matches
  `.t-cm-chart-label` 12/500, so the chart label is the de-facto step 1), **400 at 16** — Dave's call: larger
  text reads fine at regular. Consistent with heading(32)=400 / section-label(20)=500.
- **Ceiling 16, VERIFIED:** the Component scale has no 18 rung (12/14/16/20/24/28/32/40/52; 18 appears only
  as a line-height). 20 was dropped (Dave changed his mind). Lands on HSBC's own "16 and below = standard text".
- **Bound via the T-D9 selector-list mechanism** (not markup-class): `.seg.sm button` → ctl-12, `.seg.md button`
...
### STILL OPEN
- **Which other label/control classes adopt the ramp.** Chart labels are already 12/500 (= step 1 de-facto).
  View options' unsized `.seg button` could migrate to `.seg.md`. Quiet toolbar controls are candidates. Accrete
  from observed use, not speculation.
