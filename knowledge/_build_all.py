...
The generators must run in this order because later ones read earlier outputs:
  ...
  6. _build_integrity.py                  -> _INTEGRITY-REPORT.md   (the gate; needs 3)

Run:  python3 knowledge/_build_all.py
Exits non-zero if the integrity gate (step 5) finds any ERROR — so this is the
...
for i, (label, rel) in enumerate(STEPS, 1):
    ...
    if r.returncode != 0:
        # integrity is the only step allowed to gate the build
