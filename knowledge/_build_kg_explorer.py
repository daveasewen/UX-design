...
ASSET_DRAWN = ('inGroup', 'activeVariantOf', 'usesIcon', 'usesLogo', 'defaultFor', 'ruledBy',
                'governedBy')  # the six + governedBy, DRAWN since 1.26 (s282-D5, Dave 2026-09-18:
                               # the logo rules are ours and they are in logos.md, so the 6 declared
                               # nulls s277-D7 opened resolve to 12 logo→rule lines). defaultActive
                               # stays DECLARED-NULL ONLY.
...
def extract_extra(base_nodes, base_edges, K=K):
    ...
    # #281 s281-D6 — `restsOn` points at a `ux:` node, and the UX family is pass D, which has not
    # run yet. Read here (this is the file it is stored in) and LINKED in pass D2, below: a
    # rule→principle line whose target does not exist yet would be counted as skipped, which is how
    # an authored line disappears without anyone noticing.
    deferred_rests = []
    for e in RE:
        if e['s'] not in known_r: rep['rule_edges_skipped'] += 1; continue
        if e.get('type') == 'restsOn': deferred_rests.append(e); continue
    ...
    # ---- D2. restsOn (#281, s281-D3 authorised / s281-D6 landed) — AUTHORED BY HAND from Dave's
    # export of the 59-card page, stored in _rule_nodes.json, read verbatim. Runs after D because
    # the target is a `ux:` node and pass D is where those are minted.
    #
    # The STORAGE family is the rule family, which is where the line is filed and what INSPECT
    # names. The CHIP that draws it is the Theory view's own (`restson`), exactly the s281-D4 shape
    # one type along: a rule's reason is read where the principle lives, and turning the HSBC
    # `rule:` chip on is not the price of reading it. The edge carries the page's own `$why`
    # sentence for the option Dave picked, the principle's grade AT THE TIME OF WRITING (the ux:
    # node's own `grade` field stays authoritative on a re-grade), the radio letter he saw, whether
    # he overruled the recommendation, and — where he wrote one — HIS NOTE VERBATIM in `daveNote`.
    # A rule that rests on convention alone is a declared null with the note, s275-D2's shape, and a
    # null and a principle MAY stand on the same rule (type26-002).
    known_x = base | set(nodes)
    for e in deferred_rests:
        kw = {k: v for k, v in e.items()
              if k in ('why', 'grade', 'choice', 'recommended', 'overruled', 'daveNote', 'both',
                       'besideNull', 'besideOption')}
        if e.get('t') is None:
            link(e['s'], None, 'restsOn', RULE_FAM, note=e.get('note', ''), **kw)
            rep['rests_on_null'] += 1
        elif e['t'] in known_x:
            link(e['s'], e['t'], 'restsOn', RULE_FAM, note=e.get('note', ''), **kw)
            rep['rests_on'] += 1
            if e.get('daveNote'): rep['rests_on_notes'] += 1
            if e.get('both'): rep['rests_on_both'] += 1
        else:
            rep['rests_on_skipped'] += 1
    rep['rests_on_rules'] = len({e['s'] for e in deferred_rests if e.get('t')})
...
# #281 s281-D6 — A RULE'S OWN REASON. `restsOn` is the authored line from an HSBC rule to the UX
# principle it rests on: written by hand, one option per line, from Dave's export of the 59-card
# page. It is MANY-TO-MANY — on 13 rows his note said the rule rests on BOTH options where the
# radio could only take one, and on those rows both land, each with its own `$why`. A rule that
# rests on convention alone is a declared null with his note. Drawn in the THEORY view under its own
# chip, off by default, because a rule's reason is read where the principle lives — the s281-D4
# shape one type along. The storage family is the rule family and is untouched.
RESTS_WHY = ('rests on: an AUTHORED line from an HSBC rule to the UX principle it rests on '
             '(s281-D3, landed by s281-D6 on Dave’s own export of the 59 BLOCKING rules). '
             'MANY-TO-MANY — a rule may rest on more than one principle, and 13 of the 59 do, '
             'because his note said so where the radio could take one answer. Each line carries the '
             'page’s `$why` sentence for that option, the principle’s grade at the time of '
             'writing, and his own note verbatim where he wrote one. Convention alone is a declared '
             'null with the note, never a dropped row. Drawn in the THEORY view, under its own chip; '
             'its storage family is the rule family and is untouched.')
...
def main():
    ...
    data = {'generated': datetime.date.today().isoformat(), 'version': VERSION, 'commit': sha, 'nodes': nodes, 'edges': edges,
            ...
            'extra': {'nodes': len(xnodes), 'edges': len(xedges),
                      ...
                      'citedWhy': CITED_WHY,
                      # #281 s281-D6 — the rule→principle branch, named in the data so the page and
                      # the legend can say it in his own terms.
                      'rests': rep.get('rests_on', 0), 'restsNulls': rep.get('rests_on_null', 0),
                      'restsRules': rep.get('rests_on_rules', 0),
                      'restsBoth': rep.get('rests_on_both', 0),
                      'restsNotes': rep.get('rests_on_notes', 0),
                      'restsType': 'restsOn', 'restsFrom': 'rule', 'restsTo': 'ux',
                      'restsWhy': RESTS_WHY}}
    ...
    print(f"  restsOn (s281-D3 authorised, s281-D6 landed; chip OFF by default): {rep.get('rests_on', 0)} rule→ux edges"
          f" + {rep.get('rests_on_null', 0)} declared nulls · {rep.get('rests_on_rules', 0)} rules rest on ≥1 principle"
          f" · {rep.get('rests_on_both', 0) // 2} rules rest on two · {rep.get('rests_on_notes', 0)} lines carry Dave's note"
          + (f" · SKIPPED {rep['rests_on_skipped']}" if rep.get('rests_on_skipped') else ''))
