...
# are the other branch and they are NOT this lane's to draw: they are marked HELD here — carried in
# the data, readable in INSPECT, drawn by nothing — until Dave rules where an obligation on a UX
# principle is drawn. A held edge is a declared gap with its own flag, not a silent drop.
HELD_WHY = ('held pending a ruling: an obligation on a UX principle has no drawn home yet — '
            's277-D8 fixed Design governance at three provenances and a ux: target is none of them')


def hold_obeys_ux(edges):
    """Mark every obeys edge whose target is a ux: principle. Returns the count."""
    n = 0
    for e in edges:
...
def main():
    ...
    held = hold_obeys_ux(edges)   # #281 lane CM — the obeys→ux branch, carried and never drawn
    ...
    print("  chip map (#281, s281 lane CM): rules += " + " · ".join(
        ...
        + f" · guidelinerules += obeys {ec['obeys'] - held} · HELD obeys→ux {held} (drawn by nothing)")
