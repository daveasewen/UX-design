...
- **HIGH-RES SNIPPET SET as prototyping fallback (flagged to strategy 2026-06-20):** refine all 32 to the Tabs bar via the multi-solution loop; output = prototyping fallback vs Figma Make while Sutherland is out. Build chat holding pending strategy decision — see `notes/_STRATEGY-KICKOFF.md` (Open decisions). Sequence: flag → rubric+runbook → loop.
