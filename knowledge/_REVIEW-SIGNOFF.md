...
| `_RESEARCH-graph-engineering-2026-08-05-v1.html` + `-v2.html` + `-v3.html` (graph-engineering audit, Swiss) | 2026-08-05 #105 · **-v3 DELIVERED #107** | ✅ **-v3 DELIVERED #107, pays #105-D1** — `_RESEARCH-graph-engineering-2026-08-05-v3.html`, Dave verbatim *"I want it to be honest."* Six honesty defects fixed; worst: the closing band self-labelled "the honest read" still carried the pre-correction four-type framing. Also reconciled a 7-vs-9 edge-type contradiction against `_build_decision_graph.py:38–53` + ADR-0012's dated amendment (nine is current, the 7→9 change now dated explicitly). Responsive gap declared in v2 CLOSED by numeric assertion. ⬛ **AWAITING DAVE'S READ** — v3 untracked at repo root as of #107, not yet staged. v1+v2 were staged at #105's wrap. |
| `reviews/TYPE-CSS-180-SPECIMEN-2026-08-06-v1.html` (`type.css:180` dark-mode specificity, #106-D3 deferred pending a specimen) | 2026-08-06 #107 | ⬛ **AWAITING DAVE'S EYE — ready to rule.** Measured: `#111111` vs `#1A1A1A`, delta +9 on R,G,B identically — achromatic, so the red/amber instability caveat does NOT apply. Contrast vs white body text: 18.88:1 (current `#111`) vs 17.40:1 (declared `#1A1A1A`), both AAA — not an accessibility question. ⚠ **Record correction:** `type.css:180` really is `[data-theme="dark"]{background:#111;color:#fff;}` at 0-1-0, but there is no literal `body{background:var(--page)}` line INSIDE `type.css` — that 0-0-1 pattern lives in each consuming snippet's local stylesheet (e.g. `Account-card.reference.html:32`). Mechanism and outcome verified; the record had named a generic pattern as a single line. **Bonus finding:** the same line 180 also overrides `color:#fff` over each snippet's `color:var(--text)` — invisible today because both resolve white, same literal-over-token pattern. |
| `outputs/_PARTITION-button-primary-2026-08-06-v1.md` (button/primary scope partition, enacts #107-D1 "partition first") | 2026-08-06 #107 | ⬛ **AWAITING DAVE'S SCOPE RULING — deferred to #108 on a clean set (#107-D1, Dave: partition first, then rule).** Four data-driven groups: **A** full button ladder (4 files: Button, Form-layout, Icon-button, Stepper) · **B** simple CTA, default+hover only (5: Action-bar, Confirmation, Drawer, Empty-state, Modals) · **C** brand scope, NOT a button case (1: Hero — `#DB0011`, resolves via `--primary-background-default` not `--button-primary-*`) · **D** secondary only (1: File-upload). ★★ VALUES identical across the 9 true button sites; MARKUP is not, and the divergence is DOCUMENTED AND DELIBERATE — quoted `Drawer.reference.html:19-20`. Option A priced: 78 occurrences / 34 lines / 11 files, destination `canon.css:338-339` exists but zero files link to it, Hero needs a different upstream name. Option B priced: no include mechanism exists for standalone snippet files; the `--check` gate is CSS-only and detection-after-the-fact. ⚠ Lives in the gitignored `outputs/` folder — not a git-tracked path; Dave reads it locally. |
