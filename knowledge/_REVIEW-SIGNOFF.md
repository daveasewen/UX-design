...
| `showroom/index.html` (side tree nav — Dave's #96 job, built #97) | 2026-08-05 #97 | ⬛ **AWAITING DAVE — shape PROPOSED not ruled** (inline-iframe preview was Claude's pick): 260px category tree → preview pane, deep-link `#c=<slug>&theme=<t>`. Render-proven 1180+700 (receipt `notes/_receipts/2026-08-05-97-side-tree-nav.md`). Flags: ① doubled chrome (embedded page header under viewbar — kept for the width slider) · ② 700px tree→intro gap, cosmetic, declared. Candlestick/bullet eyeballs reachable from the tree. |
