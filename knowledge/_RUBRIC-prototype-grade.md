...
> **DECISION B — ENACTED 2026-06-22 (native-keyboard AT).** The scorer now credits AT=1.0 for an interactive component keyboard-operable via a native element (`<a>`/`<button>`/`<input>`/`<select>`/`<textarea>`) + a visible `:focus-visible` indicator, as well as the custom role+aria+keydown path — guard: a component exposing nothing still scores 0. Board average 6.8 → 7.3. Rationale that drove it: The `AT(interactive)` signal demands a JS `keydown`/`keyup` handler, which UNDER-CREDITS components whose interactivity is NATIVE (`<a>` / `<button>` / native inputs) — they're fully keyboard-operable WITHOUT a JS handler (a `keydown` on a native `<a>` is redundant / an anti-pattern). This bit Selection-controls (native checkbox/radio/switch) and Cards (whole-card native `<a>` link). **BAND-AID used so far:** add a genuinely keyboard-driven sub-pattern so a real `keydown` exists (SC → chips radiogroup; Cards → selectable-card radiogroup). **ROOT-CAUSE FIX (make deliberately, like decision A):** credit AT for components keyboard-operable via native semantics + visible focus, not only a JS handler — guarded so a control exposing nothing still scores 0. Decide **before the native-link components (Links, Breadcrumbs, Pagination, Navigations)**, where bolting on a variant stops being honest.

> **DECISION C — ENACTED 2026-06-24 (simple state model).** The `states` signal rewarded ≥4 of {`:hover`,`:active`,`:focus-visible`,`:disabled`,`aria-selected/checked/expanded/current`}. That penalised genuinely SIMPLE interactives whose design has only default + hover/pressed + focus — e.g. **Tags** (no selected/checked/disabled/expanded/current; hover & pressed are one combined state). They could never exceed 3 selectors, capping at 0.5 however complete they were. **Resolution (same shape as decision A):** a component's meta may set `"stateModel": "simple"`; the scorer then credits `states`=1 when it implements its whole applicable set (`:hover` + `:active` + `:focus-visible`), guarded so a "simple" component missing any of those still falls back to the count. Measures the states the component actually needs, not states it can't have. Effect: Tags 8.5→9.0. **Do NOT** use it to dodge a real selected/disabled state a component genuinely has — that's the gaming the [[gate-blindspot-state-contrast]] lesson warns against; human review stays the backstop.
