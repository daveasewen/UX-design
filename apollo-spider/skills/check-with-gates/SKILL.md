...
```
python3 ci-template/run-gates.py --list   # what will run
python3 ci-template/run-gates.py          # run them
python3 ci-template/run-gates.py --browser  # + the playwright three
```
...
| **Did I really retrieve this, or rebuild it from memory?** — step 0 of the screen gate: re-hashes every spliced region against the page's own provenance receipt, so invention is a comparison and not a judgement. It also reds `DEMO-CHROME-COPIED` (`s258-D3`) when the page carries an `APOLLO-DEMO` fence — showroom harness copied in with the component | `python3 knowledge/_validate_receipt.py path/to/your-screen.html` |
