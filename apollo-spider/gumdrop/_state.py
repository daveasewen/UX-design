...
def check(doc=None, path=STORE):
    ...
    # ⚠ `items` GUARDS BOTH NOTES (#219 N3). On an EMPTY store — the shape Memento ships to a
    # new project — every legacy id is trivially "retired" and the coverage note reads
    # "0 of 0", so a cold start opened with two paragraphs of this repo's own history and a
    # name that means nothing to the reader. A note that fires when there is nothing to
    # measure is decoration, not a measurement, and the same argument the project split
    # already makes one line above [[measure-dont-convert-units]]. Inside Apollo, where the
    # store has items, both notes fire exactly as before.
    if stale_legacy and items:
...
def selftest():
    ...
    # ⛔ THE FIXTURE HOME IS DERIVED, NOT TYPED (#219 N3). It used to be the literal
    # `knowledge/_GOVERNING-RECORDS.md`, which resolves only inside THIS repo: run the same
    # selftest from a Memento cut shipped in a designer's pack and every `bite()` that expects
    # a PASS failed with "home UNRESOLVABLE", i.e. the selftest measured the environment and
    # reported it as a store defect [[gate-cannot-pass-in-one-environment]]. This module's own
    # file always exists under the same root the resolver uses, so the fixture now addresses
    # THAT — and it cannot rot when a document is renamed either (#167 home-pointer class).
    _fixture_home = os.path.relpath(os.path.abspath(__file__), os.path.dirname(HERE))
    ...
    def healthy():
        return {"meta": {"schema": SCHEMA}, "items": [dict(
            ...
            links=[], home=_fixture_home, project=PROJECT_VALUES[0])]}
