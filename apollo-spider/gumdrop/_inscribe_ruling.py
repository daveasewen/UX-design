...
def compose(text, entry):
    ...
    open_i = text.index("[", text.index('"rulings"'))
    ...
    if inside.strip() == "":
        # ⛔ THE EMPTY ARRAY IS A REAL SHAPE, NOT AN EDGE CASE (#219 N3, s219-D5(Q1)).
        # A cold-start Memento ships `"rulings": []` and the FIRST ruling a project ever
        # inscribes lands here. Measured before the fix: `head.rindex("}")` raised
        # `ValueError: substring not found` on an empty store, so the sanctioned writer was
        # the one thing that could not write the first entry — and the empty store the
        # release ships was therefore illegal on arrival, which is worse than shipping none.
        # ⚠ AND THE OLD LINE WAS LATENTLY WRONG EVEN WHEN IT "WORKED": `rindex` searched the
        # WHOLE head, so on any file whose pre-`rulings` keys contain an object (a dict
        # `_README`, a `meta` block) it would splice the new entry OUTSIDE the array and the
        # R3 parse-check would be the only thing between that and a corrupted store. The
        # insertion point is now derived from the array's own `[`, structurally, both arms.
        span = "\n" + body + "\n"
        at = open_i + 1
    else:
        # the last entry's closing `}` and the newline+indent before `]` are preserved
        # verbatim; we splice after the final `}` of the array's last element.
...
# ---------------------------------------------------------------------------
# ⛔ THE FIXTURE'S OWN POINTER IS DERIVED, NOT TYPED (#219 N3), and for the same reason as
# `_state.py`'s fixture home: `knowledge/_inscribe_ruling.py` is a path that resolves only
# inside THIS repo. Run the selftest from a Memento cut shipped in a designer's pack — where
# this module sits at `machinery/` — and R5 refused its own control entry, so the selftest
# reported an ENVIRONMENT as a defect in the tool [[gate-cannot-pass-in-one-environment]].
# `_governs.REPO` is `dirname(dirname(__file__))` of _governs, which is the same root this
# module's helper walk lands on, so addressing THIS FILE relative to it resolves in both.
_SELF_REL = os.path.relpath(os.path.abspath(__file__),
...
GOOD = {
    ...
    "governs": [_SELF_REL],
    "evidence": ["chat #000 2026-01-01 (live) - synthetic", _SELF_REL],
    "status": "SELFTEST",
}


# The seed the EMPTY-STORE arm plants. A DIFFERENT id from `GOOD` on purpose: arm 1 inscribes
# `GOOD` into a fresh copy, and a seed that shared its id would turn the control into an R4
# duplicate refusal — a green suite proving the opposite of what it says.
SEED = dict(GOOD, id="zz-seed-D0",
            says="Planted by --selftest when the store it copies is EMPTY, so the amend and "
                 "duplicate arms have a target. Never written to a live store.")


def _tmp_copy(tmpdir):
    """A throwaway copy of the store under test — and, when that store is EMPTY, a copy with
    one seed ruling planted in it.

    ⛔ WHY THE PLANT (#219 N3, s219-D5(Q1)). A cold-start Memento ships `"rulings": []`. Every
    arm below that needs an EXISTING ruling — the duplicate-id arm, all eleven amend arms —
    used to read `["rulings"][-1]` unguarded, so run from a shipped Gumdrop cut the selftest
    did not FAIL, it CRASHED with an IndexError [[a-crash-is-not-a-fail]], and the release's
    own answer to "is this empty store legal?" was a traceback. The plant is centralised here
    rather than repeated per arm so there is exactly one place the empty shape is handled.
    ⚠ It also makes every call an implicit control on the empty-array insertion path itself:
    if `compose()` could not write the first entry, this function would fail loudly.
    """
    dst = os.path.join(tmpdir, "_rulings-%d.json" % _tmp_copy.n)
    _tmp_copy.n += 1
    with open(RULINGS, encoding="utf-8") as a, open(dst, "w", encoding="utf-8") as b:
        b.write(a.read())
    if not json.loads(open(dst, encoding="utf-8").read())["rulings"]:
        inscribe(SEED, dst, write=True)
    return dst
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # ⚠ EMPTY-STORE PROOF (#219 N3): on a cold-start store `_tmp_copy` plants the seed,
        # so this arm has a real target instead of an IndexError.
        p3 = _tmp_copy(td)
        ...
        p6 = _tmp_copy(td)
        # ⚠ THE R6 ARMS NEED THE ROLLING FILES TO EXIST (#219 N3). R5 runs BEFORE R6 and
        # refuses a path that does not resolve, so in an environment where Apollo's rolling
        # files are absent — a Memento cut shipped inside a designer's pack — these arms are
        # refused for the R5 reason and would be reported as an R6 failure. That is
        # [[refusal-names-the-first-obstacle]]: the arm would be measuring the file inventory,
        # not the clause. Where the fixtures are absent the arms are DECLARED UNMEASURED, in
        # this module's own honest vocabulary — never silently skipped, and never a pass.
        _rolls_present = [r for r in _governs.ROLLING_FILES
                          if os.path.exists(os.path.join(REPO, r))]
        if len(_rolls_present) != len(_governs.ROLLING_FILES):
            absent = [r for r in _governs.ROLLING_FILES if r not in _rolls_present]
            fails.append(f"   arm6/arm6b/armA3b: UNMEASURED in this environment — the rolling "
                         f"fixtures {absent} do not exist under {REPO!r}, so R5 bites before R6 "
                         f"can. NOT a pass: the R6 clause was not exercised here. Drive this "
                         f"suite from the Apollo repo to measure it.")
        for i_, roll in enumerate(_rolls_present):
        ...
        _skip6 = len(_rolls_present) != len(_governs.ROLLING_FILES)
        r6b["evidence"] = ["_LIVE-STATE.md"]
        try:
            inscribe(r6b, p6, write=True)
            if not _skip6:
                fails.append("arm6b: a bare rolling path was ACCEPTED")
        except InscriptionRefused as ex:
            if "rolling" not in str(ex) and not _skip6:
        ...
        try:
            ...
            if len(json.loads(orig_a1)["rulings"]) < 2:
                fails.append("   armA1: UNMEASURED — the store under test carries fewer than "
                             "two rulings, so a whole-file reserialization is byte-identical "
                             "to the original and there is no tidying for A2 to catch. NOT a "
                             "pass. Drive this suite against a store with real history.")
            else:
        ...
        for roll in _rolls_present:          # UNMEASURED where the fixture is absent — see arm 6
            try:
