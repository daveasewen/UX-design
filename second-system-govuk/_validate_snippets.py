...
TOK = os.path.join(ROOT, "tokens-resolved")
_sys = json.load(open(os.path.join(ROOT, "system.json")))
THEMES = tuple(_sys.get("themes", ["light", "dark"]))
...
def validate(path):
    ...
    blocks = {t: theme_block(html, t) for t in THEMES}
    for t, b in blocks.items():
        if not b:
            errors.append(f"{name}: missing [data-theme=\"{t}\"] block")
