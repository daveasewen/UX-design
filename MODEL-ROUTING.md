...
7. **Adversary at the BRIEF, verifier only at CANON — added 2026-09-06 #250, by ADDITION (Dave: *"go for it"*, after *"i might be wrong dont follow me blindly"* — a working shape, NOT a ruling; no `s250-*` id).** Fable-conductor sessions while the Fable bucket is the binding quota (Fable 51% vs all-models 27% at the #250 opener): **every lane is an Opus 5 sub**; Fable does brief, judge, read-back, wrap only. Three placements, priced against what actually failed (#244 wrong premise → lane built 0/4 correctly · #238 gate built clean then falsified · #202 vocabulary collision, asserts green — an output verifier passes all three): **(i) PREMISE PROBE before any canon-touching build** — a small Opus sub that tries to break the brief's premise (drive the thing, probe `derived`, mutation-test the clause), template `notes/_briefs/_TEMPLATE-premise-probe.md`; first run = `notes/_subreports/2026-09-06-250-PROBE-byte-gate.md` (≈100K Opus, corrected the fork's own "(c) shave clears it" premise). **(ii) Rule-5 adversarial verifier ONLY where the output touches canon** — gates, `canon.css`, rulings inscriptions, packs; analysis/research lanes get one Opus sub and the conductor's read. **(iii) Coherence is a BRIEF property** — subs are stateless: every brief carries the ruling ids it must obey, the report carries a `CITES:` line beside `COUNTS:` (advisory, not parsed), the conductor checks citations. Judgment-shaped claims from a single seat: the builder's brief carries *"state the strongest case for the option you rejected before recommending"* — a line, not a second sub. Fable-5.1 loop notes from the live docs (#250): briefs say *"batch independent reads/writes in one turn"* and *"Edit, never Write, on existing files"* — both cost tokens otherwise.
