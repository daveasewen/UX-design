...
**⬛ OWED BY #55, DEFERRED TO #56 BY DAVE — TWO ITEMS, NEITHER STARTED, NEITHER SILENT.**
**(a) `MEMORY.md` COMPACTION — ~9 pts.** The index is **19.8KB against a 24.4KB read limit**; the hook warned at
17.1KB. ⚠ **It lives OUTSIDE the repo, so no gate here can see it and bash cannot reach it** — `Read`/`Write` only,
which is why it is expensive. ⛔ **Compaction = trim hooks + move entries to `MEMORY-ARCHIVE.md`, NEVER delete a
file**, and **probe for SOLE HOMES before any trim** ([[home-by-addition-then-cut]]: orphans written FIRST, cut
second, never one motion). **(b) THE #55 1b DOSSIER — ~4 pts**, declared owed at the wrap, not slipped.
⚠ **Both were surfaced in chat and would have died there** — a dated home is not a home.
