...
> **0c. NEXT BUILD CANDIDATES — ✅ THREE OF FOUR CONSUMED #66:** scatter geometry RULED×3 + ENACTED GREEN (`768b508`+`383fe89`) · instrument fixes LANDED — **the trustworthy corpus figure is 30 (was 78 UNTRUSTED); triage is DAVE'S** (ledger § ★ #66) · remaining: **DV-J2b** — ⚠ superseded-in-part by #66-D6 D2 (sparkline's consumes declaration IS the D2 enact; fold DV-J2b into the #67 wave) · **ds-020**, FENCED by his #27 ruling. ⚠ Price the render sandbox IN (~4 calls, fresh; `LD_LIBRARY_PATH` chromelibs — see `_RUNBOOK-render-verify.md`). [born #27 · 3/4 consumed #66 · guards: `_lanes.json` · until: DV-J2b folds + ds-020 ships]
> **0d. ⬛ THE #67 ENACT WAVE — RULED #66-D6, UNENACTED, ONE SEQUENCE:** D1 A2 permanent-STRICT (every member declares; `gen_component_partials.py` semantics change FIRST) → D2 sparkline declares + sheds 16,661 B → D3 scatter key connects (`.dv-legrow` + subscribe; legend gate goes green again) → D4 dv-lockup partial SCOPED (not built blind). Ledger § ★ #66-D6 has the numbers; `_RUNBOOK-review-doc.md` governs any re-presentation. [born #66 · guards: ledger + `_check_legend_migration.py` RED on scatter · until: the wave lands]
