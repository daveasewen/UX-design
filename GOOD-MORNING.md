...
> **pre-flight #58 (RETROSPECTIVE — no band was written before this window's work, and that is a LAPSE, declared. The opener asked pace and Dave answered *"no constraint, lets just fix the problems"*, which retires the CONSTRAINT and not the OBLIGATION TO PRICE):** boot **30,633** (disk **10,633 real, MEASURED** by `_gauge_tokens.py` · harness **20,000 est ±8,000**, `ds-025` item 1, unobservable from any mount) + job **70,000 est** + wrap **25,000 est** = **125,633 of 200,000 — GREEN** (amber 160,000). ★ **NAME WHICH STANDARD APPLIES: a pre-flight is a PLANNING ESTIMATE, not a measurement** — refusing to state one because the harness half is unobservable is not honesty, it is the [[m8-cap-at-its-own-floor]] move applied to a stamp. **Say which half is measured — the disk half is, the rest is not.**
...
> - **⚠ ONE UNREPRODUCED RED AT BOOT, AND IT IS LOGGED AS UNPROVEN RATHER THAN DIAGNOSED.** `_gen_chain.py --check` exited **1** on the first call of the session with a **clean tree** — the #57 defect's exact signature. But regeneration produced a **byte-identical file** (`diff` = 0 lines, `git status` clean) and `--check` then exited **0** three times running on that same file. ⇒ **either `check()` is nondeterministic or the first reading was a cold-mount artefact; I could not tell which, and the gate is BLOCKING at the commit seam, so a flaky RED blocks every commit and a flaky GREEN defeats it.** ★ **An honest UNPROVEN is a priced TODO — it is in §C·4, not dressed as a finding.**
