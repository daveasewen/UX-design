...
> **size:** GM **16.4K tape / ~25.8K bill** · §A 4.21K (EXEMPT) · compactable **~12.0K tape / ~18.8K bill — AT THE BLOCK, trimmed four times this wrap** · chain **3.7K tape / ~5.8K bill** · corpus 35.3K tape · measured #35 (cl100k; `bill` DERIVED ×1.57, PROVISIONAL n=2). ★ **`tape` = tiktoken counts · `bill` = what the window charges — THE TAPE IS NOT THE BILL.** ⚠ **#35: the block was hit FOUR TIMES in one wrap and every trim came out of #35's own prose. This is no longer pressure, it is a ceiling — and Dave's offload ruling (§C·4) is the first thing that actually relieves it.**
> **STATE: ★★ THE READ CHAIN IS CUT (#33) and it HELD — this session read it and nothing else; `A:U` two sessions running.** You are reading the contract; it is four paragraphs below the banners. §A and §C are STILL IN THIS FILE, reached by `_memento_search.py`, not by reading top-to-bottom. Build **72/72 exit 0**. ★★ **#35 built the READER the usage instrument never had (`usage_history_probe`, ADVISORY) and it immediately falsified its own brief: the "~3,275 tape dead every window" was a PRE-CUT number — none of those sections is in the chain. What remains is RECORD cost, 3,431 tape, and Dave has RULED what happens to it.**
> **★ #36's JOB, in order:** **(1) ENACT DAVE'S #35 RULING** — offload seven (`GM:C2b` `C3` `C4b` `C5` · `LS:DEAD` `SPINOFFS` `TARGETS`) to the archives, **DE-MATERIALISE `LS:LIFECYCLE`** (it is generated from `_decision-graph.json`), and **ship the DEFERRED REGISTER IN THE SAME PASS — it is the condition, not a nicety** (§C·4 carries the whole ruling). **(2) ★ MEASURE THE BOOT** — ~17 of #35's ~20-pt floor is harness + memory index + system prompt, **never measured by any session here**, and every pre-flight number in this repo rests on an estimate of it. Dave's call, proposed #35. **(3) the M10 numbers** — agent-derived, third session untouched. **(4) charts.**
...
> **pre-flight #36: the ceiling is GATED.** `fill + job + wrap` **< 45** — **45 ITSELF FAILS** (it is AMBER; ruled Dave #34) — `_capture_gate.py::check_preflight`; the ONLY way over is the literal marker **`RESERVE SPEND — forked to Dave`**, declared IN ADVANCE. In flight the stop line = **60 − your priced wrap** and MOVES with that price. ⚠ **#35 declared 42, closed at ~50 = its own stop line, and dropped the enactment at the door rather than squeezing it in — that is the ceiling working, twice running.** ⚠ `tiktoken` is NOT preinstalled: `pip install tiktoken --break-system-packages`. ★ **READ THE CHAIN ONLY — header + ★ LATEST + the LS LATEST delta, 3.75K tape.** ★ **A claim about repo state is verified against `git log` / a real run, never against a banner** (#29→#35: seven consecutive) — and #35 adds the sibling: **verify a PREMISE the same way, because a number can stop being about anything while every word around it stays true.**
...
> **TITLE THE NEXT CHAT →** `Apollo — #36 ENACTS THE OFFLOAD: Dave RULED it #35, nothing has moved. Seven sections (GM:C2b/C3/C4b/C5 · LS:DEAD/SPINOFFS/TARGETS) → archives, which ARE in the retrieval corpus (verified, so offload is lossless); LS:LIFECYCLE DE-MATERIALISED because _build_live_state.py already generates it from _decision-graph.json; and the DEFERRED REGISTER ships in the SAME pass — without it, offloading the QUEUE (C3/C4b/C5) silently discards decided work, because *never cited* is ambiguous between *not needed* and *invisible*. Standing list: `python3 knowledge/_gm_usage.py --history`. THEN: measure the BOOT — ~17 of a ~20-pt floor has never been measured by anyone here, and every pre-flight figure rests on that estimate. ~12-15 pts. PRICE IT BEFORE YOU START; the ceiling is gated and has now bitten two sessions running.`
