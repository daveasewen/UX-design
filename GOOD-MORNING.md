...
> **size:** GM 15.7K tk · §A 4.21K tk · chain 33.6K tk · measured 2026-07-28 (tiktoken cl100k_base) · budget = the COMPACTABLE region (all but §A),
> 8K warn / 12K block — `_RUNBOOK-capture-ritual.md` step 2. **Compactable 11,463 tk = WARN, and this is the tightest it has been: ~540 tk of headroom before the 12K BLOCK.** *(This wrap hit the BLOCK at 12,503 and had to pay before it could close — the 2f stratum roll + the item-0 retirement receipt bought it back down. That is the mechanism working, but it means the next wrap starts with almost nothing.)* ⚠ **#28 MUST PAY BEFORE IT SPENDS** — a normal banner (~1.0–1.2K tk) will cross the block on its own. Cheapest payers, in order: the §C·2 batch (1,076 + 484 tk in C2/C2b — items 8–22 are pointer-shaped already and several are stale), DOFIRST (2,297 tk, grew +160 this wrap), and the STRATA stack (1,167 tk, 2f-rollable). Do NOT chase below 8K and do NOT touch §A (standing, exempt). Chain cap 24K is ADVISORY (M10, #18); ⚠ the 28,000 promotion threshold is still CROSSED (chain 33,162 tk at this wrap); the payer remains LS's standing body (§C·4 M10 line), not the deltas.
> **STATE: ★ LANE 2 IN FLIGHT — DV-J2 SCATTER HALF LANDED #27, DV-J2b (sparkline) QUEUED.** The
> consumes-manifest has its **first live narrow declaration**: Chart-scatter declares
> `consumes:["dv-behaviour"]` — 13,251 B in, dv-legend's 16,271 B refused — and the narrow path now
> bites FOUR ways on a real member (unknown name · empty list · declared-away markers · **and the
> declaration REMOVED**, which the author had predicted would pass). ADR-0015-A2 stays **TENTATIVE
> (Dave: "flexible… might be neater global")**. DV-J1 queued · **ds-020 OPEN** = scatter is the only
> cartesian member still pre-DV-D07, fenced by ruling, needs a paired control before anyone touches it.
> Memento steady: retrieval `_memento_search.py` → `--fetch` · gates BLOCKING (usage · M9 ·
> lane-routing) · consult-receipts ADVISORY · M12's UNATTENDED Sun 08-02 07:10 fire = the proof owed.**
> **pre-flight #28 guide:** cold floor **MEASURE IT — never inherit this number.** #27 measured
> **32,179 tk = 16.1%** at its opener (harness ~7 pts inferred on top; Read-render overhead reads the
> floor LOW — sizes are FILE cost only).
> **#28's first job is DAVE'S PICK, not the queue's** — the obvious candidates are **§C·2 item 20's
> one-word close** (see item 0), **DV-J2b** (sparkline markup+CSS, JS already injected and dormant)
> and **ds-020** (needs the before/after control, so price it with the render pipeline in). ⚠ **The
> render sandbox is FRESH every session** — playwright + chromium-headless-shell + libs + fonts is
> ~4 calls before a single pixel; price it, don't discover it. · **retrieval FIRST:** `python3
> knowledge/_memento_search.py "<q>"` → `--fetch <id>` — quote the record, never reconstruct ·
> **wrap moves RUN THROUGH `_gm_move.py`** (`---` is NOT a usable anchor — 12 matches; use the file's
> title line) · stratum carries **usage + sizes + consult-receipts** lines · a routing edit = records
> FIRST, regen view, then §C·1's line — **and a `landed` step may not sit after an unlanded one**
> (the generator refuses; reorder the sequence to match reality) · M1: RED = wrap-only, announce
> crossings · 15% reserve · panel Tue 14:25 — ask fresh.
> **RENAME THE WRAPPED CHAT →** `Apollo — DV-J2 SCATTER HALF (#27, Opus solo): the manifest's FIRST NARROW DECLARATION goes live — 13,251 B in, dv-legend's 16,271 B refused, [72/72] + render-proven; the queue said "rename 28 titles to data-tip" and the canon was a four-part contract with keyboard focus — 27 marks, not 28; bite 3 refused where I predicted it would pass, so the declaration can't be silently deleted; DV-D07 fenced by ruling as ds-020`
> **TITLE THE NEXT CHAT →** `Apollo — #28 FIRST JOB IS DAVE'S PICK: §C·2 item 20's one-word close (scatter is no longer "Layer-1 safe") · DV-J2b sparkline markup+CSS (JS already dormant) · ds-020 DV-D07 catch-up (needs the paired control — price the FRESH render sandbox in, ~4 calls before a pixel); MEASURE the floor (do not inherit 32.2K); retrieval first — _memento_search.py "<q>" then --fetch <id>; M12 fires UNATTENDED Sun 08-02 07:10 — the proof owed; panel last Tue 14:25 (All 0.95× behind · Fable 1.19× ahead)`
...
> *(⚠ neither door indexes `docs/decisions/` — ADR-0015 § Amendment 2, the actual contract this session enacted, was read DIRECT because retrieval cannot reach it. Stated, not hidden: the memento index covers GM/LS/archives/ledger/gauge/briefs/dreams/runbooks/lanes, and `_consult.py` covers rules/rulings/assertions/gates. **ADRs sit in neither.** That is a retrieval hole a session with a fresher budget should price — it is the ds-016 shape: a live, in-force document the index cannot see.)*
>
