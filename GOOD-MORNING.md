...
**⬛ FLOATED BY DAVE #56, UNRULED, HIS — THE DELEGATION INVERSION.** *"use subagents more often, and parallels
when I'm needed — it's basically the other way round."* ★ **The Mode-2 ruling (2026-07-23) is stale for a
nameable reason:** it predates `_CHAIN.md`, the retrieval index and the gates — subagents were the exception
*because briefing a cold agent was expensive*, and twenty sessions since built the machine that makes a cold
start cheap ([[premise-ages-faster-than-rule]]). **Three counters are on the table with it, all his to rule:**
**(1)** fan-out has a ceiling and it is the ORCHESTRATOR's window — ~15 returns at ~3K a brief / ~8K a result
against 200,000 · **(2)** ★ **the binding constraint is VERIFIABILITY, not budget** — a subagent cannot ask
Dave and its errors come back as confident prose ⇒ **delegate what a GATE can check, keep what only Dave can
judge** · **(3)** he is currently doing the routing job himself. Full form: `notes/_MEMENTO-DECISIONS.md` § #56.
[born #56 · guards: this line · until: Dave rules it]
