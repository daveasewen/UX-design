...
> **size:** GM ~14.7K tk cl100k (~23K charged) · §A 4.21K tk · compactable ~10.5K cl100k (~16.4K charged) · chain ~33.6K cl100k (~52K charged) · measured 2026-07-28 #31 (tiktoken cl100k_base; charged = ×1.56 provisional) · caps (ds-021 (b), RULED #31): compactable warn 8K cl100k ≈ 12.6K charged / block 12K ≈ 18.8K charged — **bind on CHARGED once #32's gate edit lands; until then the gate measures cl100k. NAME THE UNIT on every token number.** §C 158 vs warn 150 — WARN.
> **STATE: ★ THE THROTTLE IS RULED, NOT YET ENFORCED — ds-021 (b) · ds-022 (c)+(a) · ds-023 (stop = 60 − priced wrap · ceiling 45) all RULED #31 as DELEGATED PICKS ("whatever you recommend" — vetoable, provenance in the ledger `knowledge/_DS-IMPROVEMENTS.md`). ENACTMENT = #32's FIRST JOB. Chart work UNTOUCHED two sessions running — #29's whole open list stands.**
...
> **pre-flight #32: measure the floor in BOTH units** (⚠ `tiktoken` NOT preinstalled — `pip install tiktoken --break-system-packages` first; log the cl100k↔charged pair per ds-021 (b)), **price the job out loud, and REFUSE it unless fill+job+wrap ≤ 45 projected.** In flight the stop line = **60 − priced wrap**; RED = wrap-only (M1). A claim about repo state is verified against `git log`/`git ls-files`, never against a banner (#29→#30→#31: three consecutive instances).
> **#32's JOB (ruled #31, delegated): ENACT THE THROTTLE —** (1) `_capture_gate.py`: pre-flight-ceiling check ≤45 w/ `RESERVE SPEND — forked to Dave` escape + the 60−wrap stop + the N−1-or-HOLE gauge-log check (ds-022 (a)) · (2) `_gm_move.py`: fold the 2f split in (ds-022 (c)) · (3) ds-021's THREE HOMES TOGETHER: `_RUNBOOK-context-gauge.md` § THE FLOOR IS MEASURED · gate cap constants · stamp format — dual-unit, bind on charged. Selftests + bites per the standing rule. ~15–20 pts, Opus-shaped (mechanical enactment; Fable 80% = ration). THEN charts, unchanged (§C·1 + DO-FIRST 0c).
> ⚠ **Render sandbox is FRESH every session** (~4 calls — price it) · **retrieval FIRST:** `_memento_search.py "<q>"` → `--fetch <id>` · **wrap moves RUN THROUGH `_gm_move.py`** (⚠ **banner headings are BLOCKQUOTED — anchor `> ## ★ PRIOR …`**) · **PANEL Tue ~19:15 #31: All 66% = 0.95× (behind; catch-up 1.10× to Thu 23:00) · Fable 80% = 1.16× (RATION, 0.65× from here)** — perishable; ask fresh at the opener.
> *(**RENAME is delivered IN CHAT at wrap, not here — RULED #28, ENACTED in runbook step 4b by #30.**)*
> **TITLE THE NEXT CHAT →** `Apollo — #32 ENACTS THE THROTTLE IT RULED: capture-gate ceiling(45) + stop(60−wrap) + N−1-or-HOLE checks · mover 2f fold · ds-021 three homes dual-unit-bind-on-charged — selftests + bites, ~15–20 pts, Opus-shaped. All three rulings were DELEGATED PICKS (#31, "whatever you recommend") — veto window OPEN, the numbers (45/60/15%) his to re-dial. THEN charts where #29 left them: scatter geometry (x=2→46/y=9→11, HIS), the two instrument fixes (78 UNTRUSTED), DV-J2b, ds-020. ASK FOR A PANEL.`
