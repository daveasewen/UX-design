...
> **size:** GM 16.08K tape / ~25.2K bill · §A 4.21K tape / ~6.6K bill · compactable 11.87K tape / ~18.6K bill — **99.0% of block, re-trimmed at this wrap to clear it** · **★ chain 3.56K tape / ~5.6K bill** · corpus 34.64K tape / ~54.4K bill · measured 2026-07-28 #34 (tiktoken cl100k_base; `bill` DERIVED ×1.57 PROVISIONAL, n=2 — the gate forks to Dave at n≥4) · caps (ds-021, ENACTED #34 — **now BIND ON `bill`**): compactable warn ~12.6K bill / block ~18.8K bill. §C 161 vs **warn 150 / block 225 — WARN. warn ≠ block.** ★ **`tape` = what tiktoken counts · `bill` = what the window charges. THE TAPE IS NOT THE BILL — name the unit on every token number.**
> **STATE: ★★ THE READ CHAIN IS CUT (#33) — 3,487 tk cl100k, from 34,094. You are reading the new contract; it is four paragraphs below the banners.** §A and §C are STILL IN THIS FILE and are now reached by `_memento_search.py`, not by reading top-to-bottom. Build **72/72 exit 0** (`b62d4c6` fixed the two-session red; #33 kept it green). ⚠ **#31's throttle rulings (ds-021 (b) · ds-022 (c)+(a) · ds-023) are STILL UNENACTED — a THIRD session — and now have a standing home in §C·4.** Veto window on all three still OPEN.
> **★ #34's JOB, in order:** **(1) MAKE THE USAGE DATA BITE** — `PRIOR` referenced-never-consumed 8/8, `C2b`/`C3`/`C4b`/`C5` unused 5 sessions running, ~3,275 tk cl100k carried dead every window. ⚠ **#33 made this sharper, not softer:** the retrieval door is now per-§A-subsection while usage testimony is still one `A:<code>`, so **the door is finer than the instrument that measures it** — decide whether testimony follows (ids already minted in `GM_A_SUBVOCAB`). **(2) #31's THROTTLE ENACTMENT**, still unbuilt, still the reason every band is directional. **(3) charts.** ⚠ **RULE THE M10 NUMBERS FIRST if you touch the gate** — they are agent-derived (§C·4).
...
> **pre-flight #35: the ceiling is now GATED, not advisory.** `fill + job + wrap ≤ 45` **FAILS the wrap** (`_capture_gate.py::check_preflight`); over it, the ONLY way through is the literal marker **`RESERVE SPEND — forked to Dave`**, declared IN ADVANCE. In flight the stop line = **60 − your priced wrap** and it MOVES with that price. ⚠ **#30–#33 all exceeded their own projections; #34 declared 48 up front and Dave ruled it a spend** (*"I can live with 48 if it only crawls over 60"*) — that is the bar. ⚠ `tiktoken` is NOT preinstalled: `pip install tiktoken --break-system-packages`. ★ **READ THE CHAIN ONLY — header + ★ LATEST + the LS LATEST delta, 3.51K tape.** A #35 that reads the whole file has ignored the contract. **A claim about repo state is verified against `git log` / a real run, never against a banner** (#29→#34: six consecutive).
...
> **TITLE THE NEXT CHAT →** `Apollo — #35 MAKES THE USAGE DATA BITE (displaced from #34 by the ceiling, which is the ceiling working): PRIOR referenced-never-consumed 8/8, C2b/C3/C4b/C5 unused 5+ sessions, ~3,275 tape dead every window. #33 subdivided gm:A into 11 records + a router, so the DOOR is finer than the usage INSTRUMENT — decide whether testimony follows (ids minted in GM_A_SUBVOCAB). #34 ENACTED the whole throttle (ds-021 tape/bill · ds-022 roll_2f + continuity · ds-023 ceiling 45 / stop 60−wrap). Dave owes: the ≤45-vs-GREEN one-point slack · HOLE lines for #9/#10/#11/#19 (NOT backfilled — evidence of absence only) · the M10 numbers + the disarmed 28,000 trigger. ~12-15 pts. THE CEILING IS GATED NOW — price it before you start.`
...
> ## ★ LATEST — 2026-07-28 (Tue **evening #34**, OPUS solo, Dave live — ★★ **THE THROTTLE IS ENACTED: ds-021 · ds-022 · ds-023, ALL THREE, in one window, after three sessions unbuilt** · ★★ **`roll_2f` — the 2f split can no longer be half-done, and the prepend is no longer EXPRESSIBLE** · ★ **the ceiling FAILED the real repo on first run, on #33's own correct-but-differently-worded stamp** · 🟡 48 declared over a 45 ceiling and ruled a spend by Dave IN ADVANCE): **"The rule existed, was correct, was ratified — and nothing checked it."**
...
> - **⬛ OPEN, Dave's:** the **one-point slack** — the ruling says `≤ 45`, but 45 reads AMBER (GREEN is `< 45`); the LITERAL ruling shipped · **HOLE lines for #9/#10/#11/#19** — NOT backfilled, because a HOLE asserts *"wrote no stratum"* and what exists is evidence of **absence** only · whether the strict `RESERVE SPEND` marker stays strict · the **M10 numbers** + whether the 28,000 trigger stays disarmed (untouched from #33) · **★ JOB 1 — MAKE THE USAGE DATA BITE — DISPLACED TO #35, not abandoned**; it was dropped at the door rather than squeezed in, which is the ceiling working · everything on #32's list below.
...
**⬛ DAVE'S CALLS — copied up #33 by the 2c EXIT CHECK (they lived ONLY in the #31 banner, rolling this wrap, and the #32 banner, rolling next):**
**#31's THREE DELEGATED PICKS — ★ ALL THREE ENACTED #34, veto window CLOSED by enactment.** **ds-021** units `tape`/`bill`, caps bind on `bill`, three homes · **ds-022** `roll_2f` + BLOCKING N−1-or-HOLE continuity check · **ds-023** ceiling 45 (FAILS) + stop `60 − priced wrap`. Verbatim + provenance + what each left open: `_DS-IMPROVEMENTS.md`; arc: `_DECISION-HISTORY/2026-07-28-enacting-the-throttle-three-rules-nothing-checked.md`. ⚠ **STILL DAVE'S, three small things:** the `≤ 45` vs GREEN `< 45` **one-point slack** (literal ruling shipped) · **HOLE lines for #9/#10/#11/#19 — NOT backfilled**, because a HOLE asserts *"wrote no stratum"* and only **absence** is evidenced · whether the strict `RESERVE SPEND — forked to Dave` marker stays strict (it failed #33's correct-but-differently-worded stamp). [born #33 · ENACTED #34 · guards: this line · until: the three residuals are ruled]
...
> **pre-flight:** fill 20% + job 18% + wrap 10% = 48% — AMBER projected · reserve 15% ring-fenced · **RESERVE SPEND — forked to Dave** *(declared IN ADVANCE, before any enactment work: "that is over the 45 ceiling I'm enacting for you in this very window". Dave ruled it: "I can live with 48 if it only crawls over 60." **Job 1 (make the usage data bite) was DROPPED at the door rather than squeezed in** — the ceiling working as designed on the window that built it. fill = the read chain + boot; ⚠ **boot UNMEASURED and marked as an estimate, never dressed as a measurement** — no session here has ever measured it.)*
> **CLOSED: 🟡 AMBER, within the declared spend** — first in five not to exceed its own projection. The mechanism was **cutting the job at the door**, not better estimating: ds-023's whole thesis.
...
> **consult-receipts #34:** "ds-021 ds-022 ds-023 throttle dual unit bind on charged mover fold stop line" → gm:C4 · gm:HDR · gm:LATEST · ls:DELTAS · lane:lane-1-memento *(⚠ the three ds- entries themselves were reached by direct `awk` on `knowledge/_DS-IMPROVEMENTS.md`, not by the door — **the ledger is not indexed**, the same ds-016 shape #27 recorded about `docs/decisions/`. Worth a line in #35.)*
> **★ THE USAGE DATA PRODUCED ITS OWN EVIDENCE (this is #35's job):** **`A:U` — §A was never opened, the first on the board**; C1–C5 all U. **~9,600 tape of GM untouched that the pre-#33 contract would have charged in full.**

> **COMMIT STATE (stamped 2026-07-28 21:5x BST from `date`).**
> **Context gauge at authoring: 🟡 AMBER ~50% (ESTIMATE — the boot term is unmeasurable from inside the window and is NOT defaulted). Dual-unit per ds-021, which this session BUILT: all figures above in `tape`, `bill` derived ×1.57 PROVISIONAL.**
