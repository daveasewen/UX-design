...
> **size:** GM **15.7K tape / ~24.7K bill** · §A 4.21K (EXEMPT) · compactable **11.5K tape / ~18.1K bill — 433 tape to BLOCK** · chain **4.1K tape / ~6.5K bill** · corpus 34.8K tape · measured #36 (cl100k; `bill` DERIVED ×1.57, PROVISIONAL n=2). ★ **`tape` = tiktoken counts · `bill` = what the window charges — THE TAPE IS NOT THE BILL.** ⚠ **#36: GM's offload relief was SPENT at #35 and there is no more of it in this file. 433 tape of headroom, and the DEFERRED REGISTER still has to land in §C. Plan that headroom BEFORE starting, not at the gate — discovering it at the door is what cost #35 six trimming rounds.** ⚠ **The gate SILENTLY ESTIMATES without `tiktoken` and UNDER-reports by ~414 tape — install it before believing any number on this line.**
> **STATE: ★★ THE READ CHAIN IS CUT (#33) and it HELD — #36 read it and nothing else, third session running.** You are reading the contract; it is four paragraphs below the banners. §A and §C are STILL IN THIS FILE, reached by `_memento_search.py`, not by reading top-to-bottom. Build **72/72 exit 0**. ★★ **#36 enacted NOTHING, deliberately, and that is the session: it found that ds-023's `< 45` had been enforced as a BLOCK Dave never ruled — the gate was FAILING THE BAND it existed to steer sessions into — and that the wrap gate degrades to an estimator that under-reports pressure. Both found by PRICING a job rather than building one. The ruling is inscribed; the code still disagrees with it.**
> **★ #37's JOB, in order:** **(1) ENACT DAVE'S #36 BAND RULING** — `_capture_gate.py::check_preflight` still **FAILS at ≥45**, which is the opposite of what he ruled; `45–60` is a PREFERRED BAND for the FULL price, ~60 the line, 63 rare+marked. ⚠ The wrap output also prints `≤ 45` while the code enforces `< 45` — fix the prose in the same pass. **(2) THE #35 OFFLOAD, LS HALF** — `LS:DEAD` `SPINOFFS` `TARGETS` → archives + **DE-MATERIALISE `LS:LIFECYCLE`** (generated from `_decision-graph.json`) + the **DEFERRED REGISTER, the condition not a nicety** (§C·4). ⚠ **GM's four moved at #35 — that relief is SPENT. PLAN THE GM HEADROOM BEFORE YOU START: 647 tape to block, and the register lands in GM §C.** **(3) ★ MEASURE THE BOOT** — never measured by any session here, and now **LOAD-BEARING**: the band prices the FULL price, so the floor is inside the ruling. **(4) M10 · (5) charts.**
...
> **pre-flight #37: ds-023 IS A BAND — RULED #36, ENACTED NOWHERE.** Dave: *"the whole 45% was a misunderstanding, it just creates the band, between 45 and 60, that I'd prefer the full price with wrap to sit in"* · *"hitting around 60% every time for the full price I can live with, it's safe-ish"* · 63 tolerable, **rare + marked**. **The unit is the FULL price: front-load + context GM + job + wrap.** ⚠ **THE CODE STILL DISAGREES WITH THE RULING** — `check_preflight` FAILS at ≥45 (an enforcement a *delegated agent* picked at #31; Dave ruled only the number). Until #37 enacts it, an honest stamp in the 45–60 band **will fail its own gate** — do not under-price to fit, that is the thing the escape hatch exists to prevent. ⚠ `tiktoken` is NOT preinstalled: `pip install tiktoken --break-system-packages` — **and the gate SILENTLY ESTIMATES without it, UNDER-reporting by 414 tape (#36, measured).** ★ **READ THE CHAIN ONLY — header + ★ LATEST + the LS LATEST delta.** ★ **A claim about repo state is verified against `git log` / a real run, never against a banner** (#29→#36: eight consecutive).
...
> **⛔ SPENT — #37 RAN AND DID THIS. DO NOT FOLLOW THE LINE BELOW; you would redo landed work (commits `3488332` + `e89c06c`). This banner is #36's and is SUPERSEDED — the live record is `_LIVE-STATE.md` § ⏱ LATEST DELTA #37, which also lists what #37's wrap still owes. Marked at #37 by a surgical edit under an Amber flush, deliberately the only change made to this file.**
> *(spent)* **TITLE THE NEXT CHAT →** `Apollo — #37 ENACTS THE BAND: Dave ruled #36 that ds-023's 45 was NEVER a ceiling — it is the floor of a 45-60 preferred band on the FULL price (front-load + GM + job + wrap), ~60 the line and livable, 63 rare and marked. The gate still FAILS at >=45, which is the opposite, and its wrap output prints <=45 while the code enforces <45. Enforcement was picked by a delegated agent at #31, never ruled by Dave - warn != block, third appearance. THEN the LS half of the #35 offload (LS:DEAD/SPINOFFS/TARGETS + de-materialise LS:LIFECYCLE + the DEFERRED REGISTER, which is the condition) - but PLAN THE GM HEADROOM FIRST: #35 spent GM's relief at its own wrap, 647 tape to block, and the register lands in GM SecC. THEN measure the BOOT: never measured by anyone here and now load-bearing, because the band prices the FULL price. Also RAISED not ruled: the size checks should REFUSE without tiktoken rather than estimate - it under-reports by 414 tape and fails UNSAFE.`
...
**⬛ #35's PROPOSAL, Dave's call:** **MEASURE THE BOOT** — ~17 of a ~20-pt floor, never measured by any session; the chain is 3. Every pre-flight figure here rests on that estimate. [born #35 · until: measured or declined]
