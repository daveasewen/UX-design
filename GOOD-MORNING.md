...
> ## ★ PRIOR — 2026-09-22 (Tue **#295**, Fable 5.1, **4 Opus lanes**, DELEGATED wrap — ★★ **THE PUSH LANDED, CI WAS READ BACK, AND THE WRAP GATE REACHED ZERO**)
>
...
> **pre-flight #296:** ⛔ NOT CAPTURED — UNMEASURED. no top-level transcript matched `/sessions/*/mnt/.claude/projects/*/*.jsonl` at this seat, so the conductor's window is unreadable from here — an ABSENCE OF A MATCH, named as such, not a proven absence. ⛔ An UNKNOWN is declared, never defaulted to a number [[feedback-measuring-tool-must-not-guess]].

> **POST-MORTEM #296 — measured, not narrated.** ⛔ **NOTHING WAS MEASURED, AND THAT IS THE READING.** `_checkin.py` found no top-level transcript under `/sessions/*/mnt/.claude/projects/*/*.jsonl` at the conductor's boot, at any later call, or at this wrap seat (the arm's own words are the pre-flight line above). ⇒ **FILL, peak, first-turn figure, turn count, compaction records and drops are ALL UNMEASURED for #296 — a declared absence, never a number** [[feedback-measuring-tool-must-not-guess]]. The conductor's figure at the brief's cut, **~185,000**, is HIS OWN COUNT, DECLARED in `notes/_lanes/296/WRAP-BRIEF.md`'s header; it is quoted here as a declaration and is never graded against a band. ⚠ **No band is claimed crossed or clear.** He called the wrap himself (*"we must be hot now"* · *"wrap"*), so this session stopped ON HIS WORD, not on an instrument; `s283-D1`'s override shape is untouched (n=2). Whether the transcript path MOVED or the seat CHANGED is not diagnosed from here; it is carried as ⑪ of `residual → #297`.

> **⚠ NO `subs` LINE IS WRITTEN, AND THAT IS THE RUNBOOK'S LEGAL FORM, NOT AN OMISSION.** No lane transcript was readable at this seat, so no sub figure was MEASURED. The conductor DECLARED per-lane spend in the brief — A **162,526** · B **155,346** · B2 **130,722** · C **126,250** (+4 continuations, unpriced) · E **207,035** (+1, unpriced) — and those declarations are published here as declarations; ⛔ **they are not summed into a `subs` line, because five continuations carry no figure and a total over them would be a guess.**

> **wrap-handover: brief-cut ~185,000 real (conductor, DECLARED — his own count, no transcript at his seat) · sub-cut unobservable (no transcript matched at this seat) · delta unobservable (there are not two readings of one window to subtract) · replay unobservable (conductor-side, post-wrap)**

> **⚠ DECLARED SKIPS AND NOT-DONE AT THIS WRAP, EACH WITH ITS SIZE.** **2c, 2d, 2f, 2g ALL RAN** — #294's ★ PRIOR banner (11 lines) to `_GM-ARCHIVE.md` § `Batch 2026-09-22 #296`; #293's ⏱ delta (34 lines) and #291's `Previous:` segment (6,527 chars) to `_LIVE-STATE-ARCHIVE.md` § `Rolled 2026-09-22 #296`; #295's stratum (30 lines) to `notes/_GAUGE-LOG.md` by `roll_2f`. **2e moved nothing** — DO-FIRST still carries no session-keyed stratum. The `size:` stamp is THIS wrap's, GENERATED by `_gen_size_stamp.py --write`. **No date-split header line** — one day. **NOT DONE, and his:** which deck carries Friday · the brain's angle · the brain slide's words · the catalogue plate · the proficiency image · the two footnotes · the plant port · which cold brief is the demo's — all in `_CARRIES.md` § `residual → #297`. **NOT DONE, and mechanical:** the `_git_commit.sh` first-add lock variant (recorded, not repaired) and the missing transcript (not diagnosed). ⚠ **The three other-seat dirty paths** (`notes/_dream/_GRADE-DECISIONS.jsonl` · `notes/_lanes/293/J7-IDEA-jev-selects-over-the-kg.md` · `notes/_lanes/294/WRAP-MEMORY-HOOK.md`) stay UNCOMMITTED by declaration (#70).

> **section-usage #296 (self-report, delegated OPUS 5.5 wrap sub):** GM HDR:C LATEST:C PRIOR:R DOFIRST:U A:U C1:U C2:U C4:U STRATA:C · LS HDR:C LANES:U SPIN:U DELTAS:C WEBFONT:U LIVE:U LIFECYCLE:U DEAD:U OPEN:U TARGETS:U SPINOFFS:U

> The wrap WROTE the ★ LATEST banner, the ⏱ LATEST delta, the `Last refreshed` stamp and this stratum; READ #295's banner (to rename it ★ PRIOR), #294's to roll it, #293's delta and #291's `Previous:` segment for 2d, and #295's stratum for 2f; and left §A, §C, DO-FIRST and every un-named `_LIVE-STATE.md` section UNREAD. ⚠ **`U` means UNREAD AT THIS SEAT** — a claim about this window, not about the file.


> **section-sizes #296 (real):** GM HDR:4078 LATEST:764 PRIOR:1200 DOFIRST:10140 A:6957 C1:2927 C2:1769 C4:11926 STRATA:6204 · LS HDR:11366 LANES:1657 SPIN:6661 DELTAS:11898 WEBFONT:939 LIVE:7886 LIFECYCLE:1221 DEAD:678 OPEN:25131 TARGETS:896 SPINOFFS:728 · totals GM:45965 LS:69061

> **⚠ THE `section-sizes` LINE ABOVE IS `_gm_usage.py`'s INSTRUMENT, A DIFFERENT OBJECT FROM THE `size:` STAMP'S WHOLE-FILE MEASUREMENT** — never reconciled, never averaged [[measure-dont-convert-units]]. Taken BEFORE the `s214-D6` line and step 5b landed; NOT re-taken (the #241 rule). **LATEST:764 is this instrument's region, which includes the generated residual line; the gate's own banner arm is quoted in the ⏱ delta's 5b line.**

> **consult-receipts #296:** none — `_memento_search.py` was not run at this seat. Every artefact the ritual needed was named BY PATH in the wrap brief (the runbook, `_HANDOFF-146`, the five lane reports, the three commits, the cold-run brief), so there was no query to put; a receipt invented to satisfy a probe would be worse than a declared absence. **Tenth consecutive honest negative** — a reading about the instrument, which only the cold opener consumes [[instrument-without-a-consumer]].

> **⚠ `s214-D6` BANNER-DISCIPLINE MEASUREMENT — QUOTED FROM `_CHAIN.md`'s GENERATED FOOTER, NEVER RE-DERIVED, AND TAKEN AFTER THE DECLARE-LAST `size:` STAMP** (the #241 rule): **`_CHAIN.md` = 7,010 tape (cl100k) = slice 6,155 + the generator's own 855 wrapper**, `GOOD-MORNING.md` = **33,227 tape**, **ratio 21%** against the generator's `<40%` floor. ★ **INSIDE the ~10–12K target and DOWN from #295's 13,035 at 5b** — the causes are nameable: a short ★ LATEST banner (a deck day, few machine findings), a ⏱ LATEST delta written as pointers, and the `size:` narration in the header cut from 3,544 to ~1,300 characters. ⚠ **Step 5b adds to the ⏱ delta, which is inside the slice, so the figure #297 actually pays is re-read after 5b and published in the 5b line — both figures reported, no third invented (the #240 instruction).** ⛔ **No literal was moved.** ⛔ **The #240 discrepancy, re-measured:** `grep -c 'BANNER-DISCIPLINE MEASUREMENT' notes/_GAUGE-LOG.md` reads **84** (#295's stratum rolled in); the newest ordinal NAMED there is #295's **FOURTEENTH**, making this the **FIFTEENTH** — **84 − 14 = 70, the same gap #295 measured: stable, not growing.** Both reported; nothing renumbered.
