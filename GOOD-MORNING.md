...
> **size:** GM 14.6K tk · §A 4.21K tk · chain 32.2K tk · measured 2026-07-28 (tiktoken cl100k_base) · budget = the COMPACTABLE region (all but §A),
> 8K warn / 12K block — `_RUNBOOK-capture-ritual.md` step 2. Compactable 10,353 tk = WARN (down from 10,403 — the §C·1(a) STEP-1 correction bought back what the #26 banner spent; block is 12K, do not chase below 8K). Chain cap 24K is ADVISORY (M10, #18); ⚠ the 28,000 promotion threshold is still CROSSED (chain 32,154 tk at this wrap); the payer remains LS's standing body (§C·4 M10 line), not the deltas.
> **STATE: ★ LANE 2 IN FLIGHT — DV-J2 ACTIVE (chart-table toggle, was ex-M4b; keys DV-J = Apollo
> dataviz Jobs, minted #26 — "M stood for Memento"). ADR-0015-A2 ENACTED #26: consumes-manifest —
> universal default · individual opt-out · **TENTATIVE (Dave: "flexible… might be neater global")**;
> machinery mutation-proven, zero declarations yet ⇒ zero behaviour change. **DV-J2's ENACTMENT =
> #27's first job** (scatter joins narrow + data-tip; sparkline toggle markup+CSS). DV-J1 queued ·
> §C·1(a) STEP-1 corrected — scatter exemplar was DONE 07-22 (`00abdf3`), the line sat stale six days.
...
> **pre-flight #27 guide:** cold floor **MEASURE IT — never inherit this number.** #26 measured
> **32,043 tk = 16.0%** at its opener (32.0K stamped at #25 wrap — the delta = GM lines authored
> after their own stamp, the known class); harness ~7 pts inferred on top, Read-render overhead
> reads the floor LOW (sizes = FILE cost only).
> **#27 = DV-J2 ENACTMENT FIRST** (item 0; machinery proven `[72/72]` #26 — do NOT re-prove; the
> job = the first NARROW DECLARATION + adoption + render-verify) · **retrieval FIRST:** `python3
> knowledge/_memento_search.py "<q>"` → `--fetch <id>` — quote the record, never reconstruct ·
> **wrap moves RUN THROUGH `_gm_move.py`** · stratum carries **usage + sizes + consult-receipts**
> lines (receipts ADVISORY; usage/M9/lane-routing BLOCKING) · a routing edit = records FIRST,
> regen view, then §C·1's line · M1: RED = wrap-only, announce crossings · 15% reserve · panel Tue 13:35 — ask fresh.
> **RENAME THE WRAPPED CHAT →** `Apollo — DV-J KEYS + ADR-0015-A2 (#26, Fable solo): consumes-manifest (universal default · opt-out by declaration · TENTATIVE) machinery landed mutation-proven [72/72]; the queue's scatter STEP-1 was six days stale — measured, corrected, re-picked ("a pick from a stale set is not a ruling"); DV-J2 ACTIVE — M was Memento's letter, J is Job's`
> **TITLE THE NEXT CHAT →** `Apollo — DV-J2 ENACTMENT FIRST (#27): scatter joins consumes:["dv-behaviour"] + 28 title→data-tip + toolbar/table spine · sparkline toggle markup+CSS · render-verify — the manifest's first NARROW declaration is its first live fire; MEASURE the floor (do not inherit 32.0K); retrieval first — _memento_search.py "<q>" then --fetch <id>; M12 fires unattended Sun 08-02 07:10; panel last Tue 13:35`
...
> ## ★ LATEST — 2026-07-28 (Tue **mid-afternoon #26**, FABLE solo, Dave live — ★★ **ADR-0015-A2 ENACTED: consumes-manifest — universal default · individual opt-out · TENTATIVE — machinery mutation-proven, build `[72/72]` exit 0** · ★ **DV-J KEYS MINTED (J = Job; "M stood for Memento")** · ★ **§C·1(a) STEP-1 was SIX DAYS STALE — measured, corrected** · 🟡 ~57% est at wrap): **"The pick was made on a stale queue line; the measurement un-made it."**
...
**⛔ ROUTING (records: `knowledge/_lanes.json` · view: `_LIVE-STATE.md` §🛤 LANES · why: `notes/_MEMENTO-DECISIONS.md` § TWO LANES + § ★ #25): ACTIVE lane-2-apollo-charts (unblocked #25 — DV-J1 table-idiom · DV-J2 toggle accretion ACTIVE (keys #26, was ex-M4a/b) · §C·1 strands (a)–(d)) · LANDED lane-1-memento (#25 — O2′ memento-search enacted; retrieval = `_memento_search.py`) · STEADY lane-dream-pass (weekly Sun 07:10 — M12 unattended fire 08-02).** [born #19 · two-lanes #20 · records #24 · landed #25 · keys #26 · guards: `lane_routing_check` BLOCKING at wrap · until: lane 2 lands]
...
this strand.)* **STEP 2 (wave) — DIVVY PLAN, the other 8 as fenced worker lanes** (NEW snippet
