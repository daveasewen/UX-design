...
> **pre-flight #97 (RETROSPECTIVE, declared):** ⛔ NOT CAPTURED — UNMEASURED. No priced job estimate at the opener; only boot is real: boot 64,728 real (`message.usage` first turn; n=8). The opener declared routing + the 150,929 stop line but no job term — the gap is the pre-flight, not the measurement.
