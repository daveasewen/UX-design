...
> **#31's candidates, his pick:** **rule ds-021** (three options; unblocks every future pre-flight and every cap in the programme) · **rule ds-022** (three options; (a) is cheap, and needs a HOLE escape hatch) · then #29's list, unmoved: **the scatter GEOMETRY** (`--control` already specifies `x=2→46` + `y=9→11` — **the numbers are HIS**, one ruling then a small enact) · **the two instrument fixes** (ancestor transforms + ancestor visibility) then re-run `--all` — ⚠ **no corpus debt figure may be quoted until this lands; the 78 is UNTRUSTED** · **DV-J2b** (sparkline markup+CSS, JS dormant) · **ds-020** (⚠ scatter has **0** `data-fx` vs bar's 167).
> ⚠ **Render sandbox is FRESH every session** (~4 calls before a pixel — price it) · **retrieval FIRST:** `_memento_search.py "<q>"` → `--fetch <id>` · **wrap moves RUN THROUGH `_gm_move.py`** (⚠ **banner headings are BLOCKQUOTED — anchor `> ## ★ PRIOR …`**) · stratum carries usage + sizes + consult-receipts, **and its post-mortem half MUST reach `notes/_GAUGE-LOG.md` — no gate checks this (ds-022)** · M1: RED = wrap-only · 15% reserve · **PANEL IS STALE: last reading Tue 17:40. #30 asked and the window closed before one arrived — ASK AGAIN, FIRST THING.**
> *(**RENAME is delivered IN CHAT at wrap, not here — RULED #28, ENACTED in runbook step 4b by #30.**)*
> **TITLE THE NEXT CHAT →** `Apollo — #31 OPENS WITH TWO UNIT/GATE RULINGS, NOT A CHART: ds-021 says every cap and every floor figure in the programme is denominated in cl100k while the window charges ~1.57× (GM 16,107 measured / 25,355 charged; floor 27.2% not 17.5%) — three remedies, the constant is UNSET not re-dialled, and re-dialling is DAVE'S; ds-022 says capture-ritual step 2f was never being run, notes/_GAUGE-LOG.md never got #26/#28/#29 and NO gate reads it — repaired, three remedies, (a) needs a HOLE escape hatch. Then the chart list is exactly where #29 left it: scatter GEOMETRY (--control specifies x=2→46 / y=9→11, the numbers are HIS) then the two instrument fixes before ANY corpus figure is quoted (the 78 is UNTRUSTED), then DV-J2b, then ds-020. set_content() BANNED in render-proofs. tiktoken needs installing. Delete the stray untracked __tmp_moved.html. M12 fires UNATTENDED Sun 08-02 07:10. ASK FOR A FRESH PANEL — #30 never got one.`
