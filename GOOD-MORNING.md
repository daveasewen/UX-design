...
> **size:** GM 16.2K tk · §A 4.21K tk · chain 34.5K tk · measured 2026-07-28 (tiktoken cl100k_base) · budget = the COMPACTABLE region (all but §A),
> 8K warn / 12K block — `_RUNBOOK-capture-ritual.md` step 2. ⚠ **#28 closed at ~24 tk of headroom after SEVEN trim passes — BUDGET THE BANNER BEFORE WRITING IT, not after.** §C 158 charged vs warn 150 / block 225 — WARN, do NOT chase (the +5 are the 2c EXIT CHECK working). Chain 24K ADVISORY (M10); 28,000 still CROSSED; payer = LS standing body (§C·4).
> **STATE: ★ LANE 2 IN FLIGHT — DV-D02 AMENDED + ITS FIRST GATE LIVE (#28); DV-J2b STILL QUEUED.**
> The encoding brief is **PART-CONSUMED** — state + what remains live in **DO-FIRST 0b**, not here.
> **ds-020 is now a gate-enforced obligation** (the DV-D02-A waiver on scatter names it as clears-when).
> Memento steady: retrieval `_memento_search.py` → `--fetch` · gates BLOCKING (usage · M9 · lane-routing)
> · consult-receipts ADVISORY · M12's UNATTENDED Sun 08-02 07:10 fire = the proof owed.
> **pre-flight #29 guide:** cold floor **MEASURE IT — never inherit this number.** #28 measured **33,962 tk = 17.0%** at its opener (GM 15,985 · LS 17,977 · **3.53 B/tk — measure, never convert**); harness ~7 pts on top; sizes are FILE cost only, so the floor reads LOW.
> **#29's candidates, his pick:** **brief finding 1 — the text-cropping render-proof** (`getBBox()` intersection + viewBox containment; render-proof family, NOT a `_validate_*.py`; **Dave has SEEN this defect twice and named it**) · **DV-J2b** (sparkline markup+CSS, JS dormant) · **ds-020** (⚠ bigger than it looks — scatter has **0** `data-fx` vs bar's 167).
> ⚠ **Render sandbox is FRESH every session** (~4 calls before a pixel — price it) · **retrieval FIRST:** `_memento_search.py "<q>"` → `--fetch <id>` · **wrap moves RUN THROUGH `_gm_move.py`** (⚠ **banner headings are BLOCKQUOTED — anchor `> ## ★ PRIOR …`**; cost two failed dry-runs) · stratum carries usage + sizes + consult-receipts · M1: RED = wrap-only · 15% reserve · **panel last read Tue 14:25 — ASK FRESH, it was already stale at #28's opener.**
> *(**RENAME is delivered IN CHAT at wrap, not here — RULED #28.** See the ★ LATEST addendum.)*
> **TITLE THE NEXT CHAT →** `Apollo — #29 FIRST JOB IS DAVE'S PICK: brief finding 1, the text-cropping render-proof (he has now seen it twice and named it — getBBox intersection + viewBox containment, render-proof family not a validator, price the FRESH sandbox at ~4 calls) · DV-J2b sparkline markup+CSS · ds-020 (bigger than it looks: scatter has 0 data-fx vs bar's 167); MEASURE the floor (do not inherit 33,962 tk / 17.0%); retrieval first — _memento_search.py "<q>" then --fetch <id>; mover anchors on banners are BLOCKQUOTED "> ## ★ PRIOR"; M12 fires UNATTENDED Sun 08-02 07:10; ASK FOR A FRESH PANEL at the opener`
