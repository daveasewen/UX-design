...
> **RENAME THIS CHAT →** `Apollo — scatter exemplar BUILT (lock-up, build 51/51, library 65, census 32→32, DataViz 0/0) + chart-revisit programme scoped: proforma = interaction gold-standard → all 5 charts to Layer-2 (value popover · responsive fit · table-view popover · legend-filter · optional title · bar/line-overlay combo) · Fable brief cut · Opus, wrapped ~Amber`
> **TITLE THE NEXT (Fable) CHAT →** `Apollo charts — revisit all 5 to the proforma bar (Fable): value popover + responsive fit + table-view popover + legend-as-filter + optional title + net-new bar/line-overlay combo · Decision #1 = behaviour-partial · prove on Chart-line, then wave · brief 2026-07-23`
...
> **1. ★ CHART REVISIT — read `notes/_briefs/2026-07-23-chart-revisit-fable-brief.md` (Fable).** Lift
> ALL 5 canon charts to the **proforma** interaction bar (value popover · responsive fit · table-view
> POPOVER not drawer · legend-as-filter · optional title · full motion) + the net-new **bar/line-overlay
> combo**. Resolve **Decision #1** (behaviour home = generated JS partial, recommend) with Dave FIRST,
> then **prove the Layer-2 on ONE chart (Chart-line)** end-to-end, eyeball, THEN fan out (divvy in the
> brief). The scatter exemplar is BUILT (`snippets/Chart-scatter.reference.html`) — it is the LAYOUT
> reference; the proforma is the INTERACTION reference. Do as LITTLE Dave-review as possible (self-verify
> checklist in the brief; review overlay rides the panes for async pins).
