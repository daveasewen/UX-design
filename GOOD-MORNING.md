...
> **size:** GM 15.8K tk cl100k · §A 4.21K tk · compactable 11.6K tk · chain 34.7K tk · measured 2026-07-28 (tiktoken cl100k_base) · budget = the COMPACTABLE region (all but §A), 8K warn / 12K block — `_RUNBOOK-capture-ritual.md` step 2.
> ⚠⚠ **THOSE CAPS ARE STATED IN A UNIT THE WINDOW DOES NOT CHARGE IN — ds-021, UNRULED.** cl100k says this file is 15,781; the read charges **~24,776 (×1.57)**. So 8K/12K really bind at **~12.6K / ~18.8K charged**, and the chain floor is **27.2%, not 17.5%**. **Do not chase the cl100k number and do not trust it either — read ds-021 before you price anything.** §C 158 vs warn 150 — WARN, and its real cost is ~1.57× that too.
> **STATE: ★ THE RITUAL ITSELF WAS THE DEFECT THIS WINDOW — ds-021 (wrong unit) + ds-022 (2f ungated, `_GAUGE-LOG` starved) both MEASURED, both REPAIRED-OR-FORKED, both UNRULED. Chart work UNTOUCHED — #29's whole open list stands.**
> ⚠ **#29's commit had never run** (11 orphaned paths, two of them untracked files its own COMMIT STATE claims were committed). #30 landed both. **Verify a claim against `git log`, never against a banner** — that is how this was found.
> ⚠ **`set_content()` IS BANNED IN RENDER-PROOFS** — it drops `type.css` silently (14px→16px) and
> `document.fonts.check` passes anyway. Use `goto("file://…")`. (#29, measured.)
> **ds-020 is a gate-enforced obligation** (the DV-D02-A waiver on scatter names it as clears-when).
> Memento steady (gates + retrieval: §C·1 ROUTING). **M12's UNATTENDED Sun 08-02 07:10 fire = the proof owed.**
> **pre-flight #31: MEASURE THE FLOOR — then read ds-021 BEFORE you believe your own number.** #30 measured **34,925 tk by the gate's unit and 54,458 CHARGED**; harness on top. ⚠ **`tiktoken` is NOT preinstalled — `pip install tiktoken --break-system-packages` first.**
> ⚠⚠ **ds-023 — THE RESERVE IS BEING READ AS ADDITIVE HEADROOM (Dave, #30): "60 should be a hard stop."** The canon ALREADY rules it ring-fenced-not-additive and names `fill+job+wrap+15%` as a false fix — so this is an ENFORCEMENT gap, the third this session. **The implied ceiling is `fill+job+wrap ≤ 45%` — a job must be projected to finish GREEN — and that number is DAVE'S to confirm.** #29 stamped 50% and #30 stamped 52%; both were over at the opener and both passed every gate. ⚠ Compose with ds-021 and #29's real pre-flight was **60% before a single tool call**. **DO NOT PRICE A JOB IN #31 UNTIL THIS IS RULED.**
> **#31's candidates, his pick:** **rule ds-023 FIRST** (it governs how every other job may be priced) · **rule ds-021** (three options; unblocks every future pre-flight and every cap in the programme) · **rule ds-022** (three options; (a) is cheap, and needs a HOLE escape hatch) · then #29's list, unmoved: **the scatter GEOMETRY** (`--control` already specifies `x=2→46` + `y=9→11` — **the numbers are HIS**, one ruling then a small enact) · **the two instrument fixes** (ancestor transforms + ancestor visibility) then re-run `--all` — ⚠ **no corpus debt figure may be quoted until this lands; the 78 is UNTRUSTED** · **DV-J2b** (markup+CSS) · **ds-020** (⚠ scatter has **0** `data-fx` vs bar's 167).
> ⚠ **Render sandbox is FRESH every session** (~4 calls — price it) · **retrieval FIRST:** `_memento_search.py "<q>"` → `--fetch <id>` · **wrap moves RUN THROUGH `_gm_move.py`** (⚠ **banner headings are BLOCKQUOTED — anchor `> ## ★ PRIOR …`**) · stratum's post-mortem half MUST reach `notes/_GAUGE-LOG.md` — no gate checks this (ds-022)** · M1: RED = wrap-only · 15% reserve · **PANEL STALE (Tue 17:40) — ASK FIRST THING.**
> *(**RENAME is delivered IN CHAT at wrap, not here — RULED #28, ENACTED in runbook step 4b by #30.**)*
> **TITLE THE NEXT CHAT →** `Apollo — #31 RULES THE THROTTLE BEFORE IT PRICES ANYTHING: ds-023 first (Dave: 60 is a HARD STOP, the reserve is NOT additive — the canon already said both, so the implied ceiling is fill+job+wrap <= 45%, a job must be projected to finish GREEN; HIS number to confirm. #29 stamped 50%, #30 stamped 52%, both over at the opener, both gate-green; composed with ds-021 #29's real pre-flight was 60% before a single tool call). Then ds-021 (every cap denominated in cl100k, charged x1.57) and ds-022 (step 2f unrun three wraps, _GAUGE-LOG starved, no gate reads it — and it BLOCKS ds-023 remedy (c), which needs the overrun dataset). Only then the charts, exactly where #29 left them: scatter GEOMETRY (x=2->46 / y=9->11, HIS numbers), the two instrument fixes before ANY corpus figure (the 78 is UNTRUSTED), DV-J2b, ds-020. FIRST COMMAND: git rm --cached __tmp_moved.html. ASK FOR A PANEL.`
