...
> **pre-flight #298:** ⛔ NOT CAPTURED — UNMEASURED. no top-level transcript matched `/sessions/*/mnt/.claude/projects/*/*.jsonl` at this seat, so the conductor's window is unreadable from here — an ABSENCE OF A MATCH, named as such, not a proven absence. ⛔ An UNKNOWN is declared, never defaulted to a number [[feedback-measuring-tool-must-not-guess]].

> **POST-MORTEM #298 — measured, not narrated, and NOT by `_checkin.py`.** ⚙ The arm's line above is true of the DEVICE: the conductor ran in the CLOUD and its transcript is `/root/.claude/projects/-home-claude/0d7deb6a-f7d5-5387-b274-836fd58074fa.jsonl`, which the device glob cannot see. ⇒ **Every figure here is a HAND SUM of `input_tokens + cache_creation_input_tokens + cache_read_input_tokens` per distinct `message.id`, main thread only — the check-in's fields, never a `_checkin.read_fill` run.** **DECLARED by the conductor** (`notes/_lanes/298/WRAP-BRIEF.md`): boot **127,661** (create 62,465 · read 65,194) · **176,116** after the opener reads (message 14) · **+25,021** on message 18 (his 11:18 message and screenshot, with the memory snapshot and the skills list re-sent after the opener's store writes) · **242,391** at message 33 · **~243,000** at the brief. **RE-READ FIRST-HAND at this seat, cloud `Bash`, the same transcript and method:** each of those reproduces to the token; the 34th distinct message (10:53:23 UTC) reads **243,057**; the 36th (10:58:50 UTC), the one that launched this seat, **247,464**. **43,057 past 200,000 and 12,943 under 256,000 at the brief — the conductor CALLED the wrap** (his 11:58 message asked for work this chat could not hold); no band claimed, no literal moved. ⛔ **Boot 127,661 vs `BOOT_CEILING_TK` 72,768 (`s295-D3`): over by 54,893.** The conductor's reading (not a ruling): the step is the CLOUD SEAT — three cloud chats within 61 tokens of each other; what is inside it is not attributed.

> **subs 361,957 tokens (n=2)**

> **⚠ THE `subs` FIGURE ABOVE IS A HAND SUM AT THIS SEAT** — the final FILL of each lane's own transcript under `…/0d7deb6a-…/subagents/`, same fields, same per-message dedupe: **A 191,130 · B 170,827.** The conductor DECLARED the harness's figures, **A 192,163 · B 171,677** — two instruments, both reported, never reconciled. The wrap seat's own transcript is EXCLUDED. ⚠ **UNIT: REAL tokens, QUOTA not window FILL** — never added to the figures above [[budget-vs-quota-vocabulary]].

> **wrap-handover: brief-cut 243,057 real (conductor, DECLARED as "~243,000 at this brief"; the figure is the 34th distinct message, 10:53:23 UTC, by the same hand sum) · sub-cut 247,464 real (first-hand at this seat, hand sum over the SAME cloud transcript, 36 distinct messages) · delta 4,407 real · replay unobservable (conductor-side, post-wrap)**

> **⚠ DECLARED SKIPS AND NOT-DONE AT THIS WRAP, EACH WITH ITS SIZE.** **2c, 2d, 2f, 2g ALL RAN** — #296's ★ PRIOR banner to `_GM-ARCHIVE.md` § `Batch 2026-09-23 #298`; #295's ⏱ delta to `_LIVE-STATE-ARCHIVE.md` § `Rolled 2026-09-23 #298`; #297's stratum to `notes/_GAUGE-LOG.md` by `roll_2f`. ⚠ **2d's `Previous:` trim moved nothing:** no `Previous:` segment is left in the stamp lines (#292's, the last, moved at #297); the #293–#297 stamps are five separate lines, 28,536 chars, not a `Previous:` chain, and were not rolled. **2e moved nothing** — DO-FIRST carries no session-keyed stratum. **No date split** — session, lanes and ritual are all 2026-09-23, so no header line. The `size:` stamp is THIS wrap's, GENERATED by `_gen_size_stamp.py --write`. **Step 3 is a seat limit, not a skip**: the hook is `notes/_lanes/298/WRAP-MEMORY-HOOK.md`, owed to #299's opener. ⚠ **The wrap gate REFUSED at open on `s294-D12`** — `_to_delete/.DS_Store` (16,388 B, #294 by commit, 4 sessions) — cleared by deleting that ONE file after the delete-permission request was granted; nothing else was deleted. **NOT DONE, and his:** reconciling and enacting his decided copy and diagram changes · the ask · the 07/08 headlines · the demo brief — all in `_CARRIES.md` § `residual → #299`. ⚠ **The three other-seat dirty paths** (`notes/_dream/_GRADE-DECISIONS.jsonl` · `notes/_lanes/293/J7-IDEA-jev-selects-over-the-kg.md` · `notes/_lanes/294/WRAP-MEMORY-HOOK.md`) stay UNCOMMITTED by declaration (#70).

> **section-usage #298 (self-report, delegated OPUS 5.5 wrap sub):** GM HDR:C LATEST:C PRIOR:R DOFIRST:U A:U C1:U C2:U C4:U STRATA:C · LS HDR:C LANES:U SPIN:U DELTAS:C WEBFONT:U LIVE:U LIFECYCLE:U DEAD:U OPEN:U TARGETS:U SPINOFFS:U

> The wrap WROTE the ★ LATEST banner, the ⏱ LATEST delta, the `Last refreshed` stamp and this stratum; READ #297's banner (to rename it ★ PRIOR), #296's to roll it, #295's delta for 2d and #297's stratum for 2f; and left §A, §C, DO-FIRST and every un-named `_LIVE-STATE.md` section UNREAD. ⚠ **`U` means UNREAD AT THIS SEAT** — a claim about this window, not about the file.

> **section-sizes #298 (real):** GM HDR:3558 LATEST:741 PRIOR:841 DOFIRST:10140 A:6957 C1:2927 C2:1769 C4:11926 STRATA:6583 · LS HDR:10060 LANES:1657 SPIN:6661 DELTAS:5037 WEBFONT:939 LIVE:7886 LIFECYCLE:1221 DEAD:678 OPEN:25131 TARGETS:896 SPINOFFS:728 · totals GM:45442 LS:60894
...
> **consult-receipts #298:** none — `_memento_search.py` was not run at this seat. Every artefact the ritual needed was named BY PATH in the wrap brief (the runbook, `_HANDOFF-148`, the two lane reports, his words file, #297's wrap), so there was no query to put; a receipt invented to satisfy a probe would be worse than a declared absence. **Twelfth consecutive honest negative** [[instrument-without-a-consumer]].

> **⚠ `s214-D6` BANNER-DISCIPLINE MEASUREMENT — QUOTED FROM `_CHAIN.md`'s GENERATED FOOTER, NEVER RE-DERIVED, AND TAKEN AFTER THE DECLARE-LAST `size:` STAMP** (the #241 rule): **`_CHAIN.md` = 6,797 tape (cl100k) = slice 5,947 + the generator's own 850 wrapper**, `GOOD-MORNING.md` = **33,257 tape**, **ratio 20%** against the generator's `<40%` floor. ★ **INSIDE the ~10–12K target**, DOWN 356 on #297's 7,153 at its wrap and down from its 7,840 after 5b — a banner of four items and a ⏱ delta of four paragraphs. ⚠ **Step 5b adds to the ⏱ delta, which is inside the slice, so the figure #299 actually pays is re-read after 5b and published in the 5b line — both figures reported, no third invented (the #240 instruction).** ⛔ **No literal was moved.** ⛔ **The #240 discrepancy, re-measured:** `grep -c 'BANNER-DISCIPLINE MEASUREMENT' notes/_GAUGE-LOG.md` reads **86** (#297's stratum rolled in); the newest ordinal NAMED there is #297's **SIXTEENTH**, making this the **SEVENTEENTH** — **86 − 16 = 70, the same gap: stable, not growing.** Both reported; nothing renumbered.
