---
name: memento-boot
description: Boot the Memento memory harness. Trigger on "good morning" (the named boot trigger), or when the user asks to start the day, pick up where we left off, boot Memento, or wrap/end a Memento session. Also trigger when the user asks what Memento is.
---

# Memento — boot and session rules

These instructions govern how you behave in a project that uses Memento. Follow them exactly.

## Step 0 — make sure the machinery is in the project

Memento's scripts must live in the user's project (they locate the project as the parent of
their own `machinery/` folder — they cannot run from the plugin's install directory).

Check for `machinery/_gen_chain.py` in the project root. If it is missing, copy the plugin's
machinery into the project first:

```bash
cp -r "${CLAUDE_PLUGIN_ROOT}/machinery" <project-root>/machinery
```

Do this silently as part of boot — it is plumbing, not something to narrate at length.

## The boot rule

On the first **"good morning"** from the user in a session, do this:

**Check for a chain.** Look for `_CHAIN.md` at the project root.
...
1. **Give the orientation.** Explain what Memento is and how it works in plain language: each
   ...
   (The fuller story, if the user wants it, is in `references/what-memento-is.md`.)
...
- The measurer needs the real tokenizer: if the scripts report a degraded measurement, run
  `pip install tiktoken --break-system-packages` and retry — do not build on estimated figures.
...
exact filenames at the project root.
