#!/usr/bin/env python3
"""#290 wrap — build the `_LIVE-STATE.md` ops file (ritual step 1 + 2d).

FOUR things happen and every one is a MOVER op:
  (1) the #287 ⏱ PRIOR DELTA block MOVES verbatim to `_LIVE-STATE-ARCHIVE.md` (2d keeps
      LATEST + 2 PRIOR; adding #290 makes #287 the fourth);
  (2) the #289 delta heading is DEMOTED from ⏱ LATEST to ⏱ PRIOR;
  (3) the #290 ⏱ LATEST DELTA is inserted above it;
  (4) the `Last refreshed` chain line gains the #290 stamp and SHEDS its oldest segment
      (#286's), which is moved verbatim into the same archive section.
...
DELTA = os.path.join(HERE, "delta290.md")

ls = open(LS, encoding="utf-8").read().split("\n")

# ---- locate the chain line -------------------------------------------------------------------
chain_idx = [i for i, l in enumerate(ls) if l.startswith("*Last refreshed: 2026-09-20 (Sun from `date` — **#289 wrap**")]
...
SEG_START = " Previous: *Last refreshed: 2026-09-18 (Fri from `date` — **#286 wrap**"
TRIM_MARK = " *Last refreshed (#285, trimmed at the #289 wrap)"
a = chain.index(SEG_START)
b = chain.index(TRIM_MARK)
assert a < b, (a, b)
segment = chain[a:b]
assert "#286 wrap" in segment and "#287 wrap" not in segment, "slice caught the wrong segment"
assert len(segment) > 4000, f"segment implausibly small ({len(segment)} B)"

TRIM_NOTE = (" *Last refreshed (#286, trimmed at the #290 wrap): #286's `Previous:` chain segment "
             "was moved VERBATIM to `_LIVE-STATE-ARCHIVE.md` § Rolled 2026-09-20 #290, in the same "
             "section as the #287 ⏱ delta block this wrap rolled. Nothing was deleted — moved.*")

STAMP = open(os.path.join(HERE, "stamp290.txt"), encoding="utf-8").read().strip()
...
ARCH_HEAD = "## Rolled 2026-09-20 #290 (2d, at the #290 wrap) — via the mover"

ops = [
    # (e) the archive section heading, newest-first, BEFORE #288's
    {"op": "insert", "file": "_LIVE-STATE-ARCHIVE.md",
     "at": "## Rolled 2026-09-20 #289 (2d, at the #289 wrap) — via the mover",
     "where": "before",
     "lines": [ARCH_HEAD, "",
               "### #286's `Previous:` chain segment — moved VERBATIM at the #290 wrap (`Last refreshed` trim)",
               "", segment.strip(), ""]},
    # (1) the #286 delta block moves under it
    {"op": "move", "src": "_LIVE-STATE.md",
     "start": "## ⏱ PRIOR DELTA — 2026-09-19 (Sat from `date`) (**#287**",
     "end": "## 🕓 OPEN — Latin Univers **WEBFONT**",
     "dst": "_LIVE-STATE-ARCHIVE.md", "at": ARCH_HEAD, "where": "after"},
    # (2) demote #288
    {"op": "replace", "file": "_LIVE-STATE.md",
     "find": [ls[[i for i, l in enumerate(ls) if l.startswith("## ⏱ LATEST DELTA — 2026-09-20")][0]]],
     "replace": [ls[[i for i, l in enumerate(ls) if l.startswith("## ⏱ LATEST DELTA — 2026-09-20")][0]]
                 .replace("## ⏱ LATEST DELTA —", "## ⏱ PRIOR DELTA —", 1)]},
    # (3) the new #289 delta above it
    {"op": "insert", "file": "_LIVE-STATE.md",
     "at": "## ⏱ PRIOR DELTA — 2026-09-20 (Sun from `date`) (**#289**", "where": "before",
     "lines": delta + [""]},
    # (4) the chain line
    {"op": "replace", "file": "_LIVE-STATE.md", "find": [chain], "replace": [new_chain]},
]

out = os.path.join(HERE, f"ops-290-ls-{int(time.time())}.json")
