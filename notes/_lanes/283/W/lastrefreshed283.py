#!/usr/bin/env python3
"""2d boundary at the #283 wrap: MOVE #279's `Previous:` chain segment VERBATIM out of
_LIVE-STATE.md into _LIVE-STATE-ARCHIVE.md § Rolled 2026-09-18 #283. Span reconstruction
ASSERTED in this process before anything is written. The #279…#282 shape, one session on."""
import os, sys
...
START = "Previous: 2026-09-16 (Wed from `date` — **#279 wrap**."
END = "*Last refreshed (#278, trimmed at the #282 wrap)"
...
note = ("\n### `Last refreshed` chain segment — #279, moved VERBATIM at the #283 wrap "
        "(2d boundary, one behind the delta roll). Nothing was deleted.\n\n*" + span.rstrip() + "\n")
ar = open(AR, encoding="utf-8").read()
H = "## Rolled 2026-09-18 #283 (2d, at the #283 wrap) — via the mover\n"
k = ar.find(H)
...
new = new.replace("*Last refreshed (#278, trimmed at the #282 wrap)",
  "*Last refreshed (#279, trimmed at the #283 wrap): #279's `Previous:` chain segment was moved "
  "VERBATIM to `_LIVE-STATE-ARCHIVE.md` § Rolled 2026-09-18 #283, in the same section as the #280 "
  "⏱ delta block this wrap rolled. Nothing was deleted — moved.*  "
  "*Last refreshed (#278, trimmed at the #282 wrap)", 1)
