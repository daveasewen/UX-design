#!/usr/bin/env python3
"""#282 wrap — write a UNIQUELY-NAMED, SESSION-OWNED ops file, assert it exists with a size
...
Here it is imported by the per-step scripts instead; see `ops282.py`.
"""
...
def run(name, ops, write=False, min_bytes=100):
    path = os.path.join(HERE, f"ops-282-{name}-{int(time.time())}.json")
