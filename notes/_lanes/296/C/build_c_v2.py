...
/* THE CURRENT CHAPTER: 44px circle, number in the display face, title x1.5 label size, red rule */
#chrail .cr-ch.is-cur .cr-c{width:44px;height:44px;font-size:22px;font-weight:300;letter-spacing:-.01em;border-color:var(--rk);}
#chrail .cr-ch.is-cur > .cr-row .cr-t{font-size:18px;letter-spacing:.14em;color:var(--racc);}
...
/* #296 lane C - the chapter rail, v2. RAIL is the only thing to edit:
   ...
   circle sits at anchor x viewport height on every slide; everything
   before it shares the space from topInset to the anchor, everything
   after shares the space from the anchor to bottomInset, evenly. */
var RAIL = {
...
(function(){
  ...
  function layout(curLi){
    ...
    if (k < 0){ ys = spread(items.length, top, bot); }
      ...
      var A = Math.round(H * RAIL.anchor), up = spread(k, A - RAIL.clear, top), dn = spread(items.length - k - 1, A + RAIL.clear, bot);
      ys = up.slice(0, k).reverse().concat([A], dn.slice(0, items.length - k - 1));
    }
    items.forEach(function(li, i){ li.style.top = Math.round(ys[i]) + 'px'; });
  }
  var last = null;
  function current(){
