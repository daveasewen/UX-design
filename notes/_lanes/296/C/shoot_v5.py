...
MJS = '''() => {
  ...
  if (getComputedStyle(nav).opacity === '0') return {hidden: true, cls: nav.className, opacity: getComputedStyle(nav).opacity, pointer: [...nav.querySelectorAll('.cr-row')].map(n=>getComputedStyle(n).pointerEvents).filter(x=>x!=='none').length};
  ...
  return {slide: sl.id, cls: nav.className, cur: cur ? cur.textContent : null, subLit: !!sub,
    ...
    circleBg: [...new Set(bgs)], slideBg: getComputedStyle(sl).backgroundColor, railClash: clash, colours: (()=>{ const q=(sel)=>nav.querySelector(sel);
    const bg=n=>n?getComputedStyle(n).backgroundColor:null, bc=n=>n?getComputedStyle(n).borderTopColor:null, fc=n=>n?getComputedStyle(n).color:null;
    return {line: line.backgroundColor, lineOpacity: line.opacity, otherCircle: bc(q('.cr-ch:not(.is-cur) .cr-c')),
      otherSubDot: bg(q('.cr-sub:not(.is-grp) .cr-d')), grpSubDot: bg(q('.cr-sub.is-grp:not(.is-cur) .cr-d')), curSubDot: bg(q('.cr-sub.is-cur .cr-d')),
      curStroke: bc(q('.cr-ch.is-cur .cr-c')), curNumber: fc(q('.cr-ch.is-cur .cr-c')), curTitle: fc(q('.cr-ch.is-cur .cr-t')), curRule: getComputedStyle(q('.cr-ch.is-cur .cr-t'),'::before').backgroundColor}; })(), eyebrow: (()=>{const e=sl.querySelector('.label'); if(!e) return null; const c=getComputedStyle(e), r=getComputedStyle(e,'::before'); return [c.fontSize,c.fontWeight,c.letterSpacing,c.textTransform,r.width];})(), railTitle: cur ? (()=>{const c=getComputedStyle(cur.querySelector('.cr-t')), r=getComputedStyle(cur.querySelector('.cr-t'),'::before'); return [c.fontSize,c.fontWeight,c.letterSpacing,c.textTransform,r.width];})() : null, tinyShown: [...nav.querySelectorAll('.cr-t')].filter(n=>getComputedStyle(n).display!=='none').map(n=>n.textContent), centres: [...nav.querySelectorAll('.cr-mk')].map(n=>{const b=n.firstChild.getBoundingClientRect(); return rr(b.top+b.height/2);})};
}'''
with sync_playwright() as p:
    ...
    for dpr in (2, 3):
        ...
        for i in (IDS if dpr == 2 else ['s3','s6b','s12']):
            ...
            if dpr == 2:
                pg.screenshot(path=D + 'v5-' + i + '.png'); meas[i] = pg.evaluate(MJS)
            else:
                pg.screenshot(path=D + 'v5-rail-' + i + '.png', clip={'x': 0, 'y': 0, 'width': 320, 'height': 900})
        if dpr == 2:
    ...
    pg.goto('file://' + F, wait_until='load'); time.sleep(2.0)
    order = pg.evaluate("() => [...document.querySelectorAll('section.slide')].map(s=>s.id)")
    for i in ['s3','s12']:
        pg.evaluate('n => window.deckGo(n)', order.index(i)); time.sleep(1.2)
        meas['tall1200_' + i] = pg.evaluate("() => [...document.querySelectorAll('#chrail li')].map(l => parseFloat(l.style.top))")
    pg.close()
    b.close()
# line-through check: pixel on the line's x inside the grown circle, above the numeral
for i in IDS:
    m = meas[i]
    if m.get('hidden') or not m['grownCy']: continue
    im = Image.open(D + 'v5-' + i + '.png').convert('RGB')
...
json.dump({'errors': errs, 'measured': meas}, open(D + 'v5-measured.json', 'w'), indent=1)
print('errors', errs)
for k, v in meas.items():
    if isinstance(v, dict) and v.get('hidden'): print(k, v); continue
    if isinstance(v, dict):
        c = v['centres']; d = [round(c[j+1]-c[j],1) for j in range(len(c)-1)]
        v['gaps'] = d
        print(k, 'cy', v['grownCy'], 'gaps', d, 'extent', v['itemsTop'], v['itemsBottom'], 'titles', v['tinyShown'], 'line', v['lineTop'], v['lineBottom'], 'clash', v['railClash'], 'mask', v.get('pxInsideAboveNum'), 'colours', v['colours'], v['cls'], 'sub', v['subLit'])
    elif isinstance(v, list): print(k, 'centres', v[:3], '...', v[-2:], 'gaps', sorted(set(round(v[j+1]-v[j],1) for j in range(len(v)-1))))
    else: print(k, v)
json.dump({'errors': errs, 'measured': meas}, open(D + 'v5-measured.json', 'w'), indent=1)
fp = '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf'
fnt = ImageFont.truetype(fp, 30) if os.path.exists(fp) else ImageFont.load_default()
CROP = ['s3','s6b','s12']
ims = [Image.open(D + 'v5-rail-' + i + '.png').convert('RGB') for i in CROP]
...
sh.save(D + 'v5-rail-3up.png'); print('done')
