...
with sync_playwright() as p:
    ...
    for dpr in (2, 3):
        ...
        for i in (IDS if dpr == 2 else ['s3','s6b','s12']):
            ...
            if dpr == 2:
                pg.screenshot(path=D + 'v4-' + i + '.png'); meas[i] = pg.evaluate(MJS)
            else:
                pg.screenshot(path=D + 'v4-rail-' + i + '.png', clip={'x': 0, 'y': 0, 'width': 320, 'height': 900})
        if dpr == 2:
...
for i in IDS:
    ...
    im = Image.open(D + 'v4-' + i + '.png').convert('RGB')
...
json.dump({'errors': errs, 'measured': meas}, open(D + 'v4-measured.json', 'w'), indent=1)
print('errors', errs)
for k, v in meas.items():
    ...
    if isinstance(v, dict):
        ...
        v['gaps'] = d
        print(k, 'cy', v['grownCy'], 'gaps', d, 'extent', v['itemsTop'], v['itemsBottom'], 'titles', v['tinyShown'], 'line', v['lineTop'], v['lineBottom'], 'clash', v['railClash'], 'mask', v.get('pxInsideAboveNum'), 'eyebrow', v['eyebrow'], 'railTitle', v['railTitle'], v['cls'], 'sub', v['subLit'])
...
CROP = ['s3','s6b','s12']
ims = [Image.open(D + 'v4-rail-' + i + '.png').convert('RGB') for i in CROP]
...
sh.save(D + 'v4-rail-3up.png'); print('done')
