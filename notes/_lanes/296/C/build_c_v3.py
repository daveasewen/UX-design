...
var RAIL = {
  ...
  step:        22,         // px, v3: fixed item-to-item spacing, above and below, on every slide
                           //  (= the v2 spacing above the anchor on s12 at 900px: (342-40-40)/12 = 21.8)
  chapters: [
...
(function(){
  ...
  function layout(curLi){
    ...
    if (k < 0){ ys = spread(items.length, top, bot); }
      ...
      var A = Math.round(H * RAIL.anchor), up = [], dn = [], i;
      /* v3: fixed spacing - stack upward and downward from the anchor at RAIL.step */
      for (i = 0; i < k; i++) up.push(A - RAIL.clear - i * RAIL.step);
      for (i = 0; i < items.length - k - 1; i++) dn.push(A + RAIL.clear + i * RAIL.step);
