#!/usr/bin/env python3
"""#291 wrap — build the `_LIVE-STATE.md` ops file (ritual step 1 + 2d).

FOUR things happen and every one is a MOVER op:
  (1) the #288 ⏱ PRIOR DELTA block MOVES verbatim to `_LIVE-STATE-ARCHIVE.md` (2d keeps
      LATEST + 2 PRIOR; adding #291 makes #288 the fourth);
  (2) the #290 delta heading is DEMOTED from ⏱ LATEST to ⏱ PRIOR;
  (3) the #291 ⏱ LATEST DELTA is inserted above it;
  (4) the `Last refreshed` chain line gains the #291 stamp and SHEDS its oldest segment
      (#287's), which is moved verbatim into the same archive section.
...
DELTA = os.path.join(HERE, "delta291.md")

ls = open(LS, encoding="utf-8").read().split("\n")

# ---- locate the chain line -------------------------------------------------------------------
chain_idx = [i for i, l in enumerate(ls)
             if l.startswith("*Last refreshed: 2026-09-20 (Sun from `date` — **#290 wrap**")]
...
SEG_START = " Previous: *Last refreshed: 2026-09-19 (Sat from `date` — **#287 wrap**"
TRIM_MARK = " *Last refreshed (#286, trimmed at the #290 wrap)"
a = chain.index(SEG_START)
b = chain.index(TRIM_MARK)
assert a < b, (a, b)
segment = chain[a:b]
assert "#287 wrap" in segment and "#288 wrap" not in segment, "slice caught the wrong segment"
assert len(segment) > 3000, f"segment implausibly small ({len(segment)} B)"

TRIM_NOTE = (" *Last refreshed (#287, trimmed at the #291 wrap): #287's `Previous:` chain segment "
             "was moved VERBATIM to `_LIVE-STATE-ARCHIVE.md` § Rolled 2026-09-21 #291, in the same "
             "section as the #288 ⏱ delta block this wrap rolled. Nothing was deleted — moved.*")

STAMP = open(os.path.join(HERE, "stamp291.txt"), encoding="utf-8").read().strip()
...
ARCH_HEAD = "## Rolled 2026-09-21 #291 (2d, at the #291 wrap) — via the mover"

latest = [l for l in ls if l.startswith("## ⏱ LATEST DELTA — 2026-09-20")]
assert len(latest) == 1, latest

ops = [
    # (e) the archive section heading, newest-first, BEFORE #290's
    {"op": "insert", "file": "_LIVE-STATE-ARCHIVE.md",
     "at": "## Rolled 2026-09-20 #290 (2d, at the #290 wrap) — via the mover",
     "where": "before",
     "lines": [ARCH_HEAD, "",
               "### #287's `Previous:` chain segment — moved VERBATIM at the #291 wrap (`Last refreshed` trim)",
               "", segment.strip(), ""]},
    # (1) the #288 delta block moves under it
    {"op": "move", "src": "_LIVE-STATE.md",
     "start": "## ⏱ PRIOR DELTA — 2026-09-19 (Sat from `date`) (**#288**",
    ...
    {"op": "insert", "file": "_LIVE-STATE.md",
     "at": "## ⏱ PRIOR DELTA — 2026-09-20 (Sun from `date`) (**#290**", "where": "before",
     "lines": delta + [""]},
    # (4) the chain line
    {"op": "replace", "file": "_LIVE-STATE.md", "find": [chain], "replace": [new_chain]},
]

out = os.path.join(HERE, f"ops-291-ls-{int(time.time())}.json")
