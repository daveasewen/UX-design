#!/usr/bin/env python3
"""#291 wrap — measure the ★ LATEST banner against `s241-D2` with the GATE'S OWN instrument.

The banner block = `banner291.md` + the two residual lines. The GENERATED residual line is
substituted from `_roll_state.py` BEFORE measuring, because that line sits inside the block the
cap charges (the #288/#289 lesson, re-applied rather than re-learned).
"""
import os
import subprocess
import sys
