#!/usr/bin/env python3
"""_drive_page_v2.py — DRIVE the #277 lane RIF v2 review page, then screenshot it.

Lane RI's _drive_page.py is NOT touched (it still drives RI's v1 page). This is its
copy, re-aimed at REVIEW-icons-2026-09-16-v2.html, with the three things VERIFY.md
ruled:

  A2 — localStorage is CLEARED at first load and the screenshot pass runs in a FRESH
       CONTEXT, so the driver's own clicks and its reload-test note cannot ship inside
       the PNGs the way they did in ri-decisions-*.png and ri-RI3-*.png.
  15. THE SHEET — the RI-3 card carries at least one inline <svg> per multi-variant
       glyph and its bare sibling (the count is read from the page's own measured
       block, never typed), and the sheet sits INSIDE the RI-3 card.
  16. NOTHING PRE-TICKED — on a first load with empty storage, no radio anywhere on
       the page is checked and no notes box carries text.
...
Screenshots: light and dark, full page, 1280 and 390.
    ...
    source knowledge/_render/seat_env.sh && \
      python3 notes/_lanes/277/icons-propose/_drive_page_v2.py
"""
import json
import re
import sys
...
PAGE = LANE / "REVIEW-icons-2026-09-16-v2.html"
URL = "file://" + str(PAGE)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # A2 (VERIFY.md): whatever a previous run left in storage is NOT this page's state.
        # Clear it and reload, so every check below and every screenshot is a first load.
        ...
        # ---- 16. NOTHING PRE-TICKED (VERIFY A2) -----------------------------
        virgin = pg.evaluate("""() => ({
          checked: [...document.querySelectorAll('input[type=radio]')].filter(r => r.checked)
                     .map(r => r.id),
          radios: document.querySelectorAll('input[type=radio]').length,
          notes: [...document.querySelectorAll('textarea.note')].filter(x => x.value.trim())
                     .map(x => x.dataset.id),
          said: document.getElementById('said').textContent})""")
        check("first load with empty storage: no radio checked, no note written",
              not virgin["checked"] and not virgin["notes"]
              and virgin["said"].startswith("0 of"),
              f"{virgin['radios']} radios, {len(virgin['checked'])} checked · "
              f"{len(virgin['notes'])} note(s) with text · said {virgin['said']!r}")

        # ---- 15. THE SHEET IS ON THE RI-3 CARD (VERIFY R1) ------------------
        want_svg = n["n_sheet_glyphs"]
        sheet = pg.evaluate("""() => {
          const card = document.querySelector('.dec[data-id="RI-3"]');
          const sh = card && card.querySelector('#RI-3-sheet');
          return {inCard: !!sh,
                  svg: sh ? sh.querySelectorAll('svg').length : 0,
                  cardSvg: card ? card.querySelectorAll('svg').length : 0,
                  rows: sh ? sh.querySelectorAll('.srow').length : 0,
                  painted: sh ? [...sh.querySelectorAll('svg')]
                      .filter(s => s.getBoundingClientRect().width >= 24).length : 0,
                  optC: !!document.getElementById('RI-3-c')};}""")
        check("RI-3 carries the sheet: one inline <svg> per glyph, all painted, and option (c)",
              sheet["inCard"] and sheet["cardSvg"] >= want_svg
              and sheet["painted"] == sheet["svg"] and sheet["rows"] == n["n_multi"]
              and sheet["optC"],
              f"{sheet['cardSvg']} <svg> in the card (want >= {want_svg}) · "
              f"{sheet['rows']} rows · {sheet['painted']}/{sheet['svg']} painted >= 24px · "
              f"option (c) {'present' if sheet['optC'] else 'MISSING'}")

        # ---- 15b. THE RENDERS ARE ON THE RI-1 CARD (VERIFY A1) --------------
        imgs = pg.evaluate("""() => [...document.querySelectorAll('.dec[data-id="RI-1"] img')]
              .map(i => ({src: i.getAttribute('src'), w: i.naturalWidth, h: i.naturalHeight,
                          alt: (i.alt || '').length,
                          shown: i.getBoundingClientRect().width > 0}))""")
        vis = [i for i in imgs if i["shown"]]
        check("RI-1 carries two explorer renders, in this theme, that actually loaded",
              len(vis) == 2 and len(imgs) == 4
              and all(i["w"] > 400 and i["h"] > 300 and i["alt"] > 20 for i in imgs)
              and "screenshots below" not in pg.inner_text("body"),
              f"{len(imgs)} render(s), {len(vis)} shown in this theme · "
              + " · ".join(f"{i['src']} {i['w']}x{i['h']}" for i in vis))
        ...
        # ---- A2 (VERIFY.md): THE SCREENSHOT PASS RUNS IN A FRESH CONTEXT ------
        # RI's driver cleared localStorage here and still shipped PNGs with (a) ticked on
        # RI-1 and the reload-test string in RI-3's notes box: the page saves the DOM back
        # to storage on its way out, so a clear() followed by reload() restores the very
        # residue it just removed. The only honest fix is a context that has never been
        # driven. Everything from here down is shot in one, and the first thing it does is
        # assert that nothing is ticked and nothing is written.
        pg.close()
        ctx = b.new_context(viewport={"width": 390, "height": 900}, color_scheme="light")
        pg = ctx.new_page()
        pg.goto(URL)
        pg.wait_for_timeout(700)
        fresh = pg.evaluate("""() => ({
          checked: [...document.querySelectorAll('input[type=radio]')].filter(r => r.checked).length,
          notes: [...document.querySelectorAll('textarea.note')].filter(x => x.value.trim()).length,
          said: document.getElementById('said').textContent})""")
        check("the screenshot context is virgin: 0 ticked, 0 notes, before a single PNG",
              fresh["checked"] == 0 and fresh["notes"] == 0 and fresh["said"].startswith("0 of"),
              f"{fresh['checked']} ticked · {fresh['notes']} note(s) · said {fresh['said']!r}")
        ...
        for theme in ("light", "dark"):
            ...
            pg.screenshot(path=str(LANE / f"ri2-390-{theme}.png"), full_page=True)
        ...
        for theme, want_c in (("light", "rgb(218, 26, 0)"), ("dark", "rgb(246, 96, 76)")):
            ...
            pg.screenshot(path=str(LANE / f"ri2-{theme}-1280.png"), full_page=True)
            pg.evaluate("() => document.querySelector('#decisions').scrollIntoView()")
            pg.wait_for_timeout(300)
            pg.screenshot(path=str(LANE / f"ri2-decisions-{theme}.png"))
            pg.evaluate("() => document.querySelector('.dec[data-id=\\'RI-3\\']').scrollIntoView()")
            pg.wait_for_timeout(300)
            pg.screenshot(path=str(LANE / f"ri2-RI3-{theme}.png"))
            # the sheet is the point of (c), so it gets its own full-height frame
            pg.evaluate("() => document.getElementById('RI-3-sheet').scrollIntoView()")
            pg.wait_for_timeout(300)
            el = pg.query_selector("#RI-3-sheet")
            el.screenshot(path=str(LANE / f"ri2-RI3-sheet-{theme}.png"))
            pg.evaluate("() => document.getElementById('RI-1-renders').scrollIntoView()")
            pg.wait_for_timeout(300)
            pg.query_selector("#RI-1-renders").screenshot(
                path=str(LANE / f"ri2-RI1-renders-{theme}.png"))
    ...
    for p_ in sorted(LANE.glob("ri2-*.png")):
