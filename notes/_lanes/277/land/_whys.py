...
LINE = [
    ...
    ("rule:dv-line-001",
     "The zero y-axis is OPTIONAL for a line and MANDATORY for a bar, and this meta records only the mandatory side's absence - no data-domain-min antiPattern where chart-bar carries one. The engine it names in `behaviour.script` (dv-render.js) still floors every registered type at zero by default (a `zeroBaseline` opt-out exists; no type takes it today), and dv-render-line.js reports that clamp as a CORE REQUEST against this very rule - so the edge records a rule the line is built to honour and does not yet enact.",
     ["antiPatterns", "!data-domain-min", "\"script\": \"knowledge/canon/dv-render.js\""]),
    ("rule:dv-line-002",
    ...
    ("rule:dv-001",
     ...
     ["\"script\": \"knowledge/canon/dv-render.js\"", "REWRITES those same rows from the spec"]),
    ("rule:dv-002",
    ...
    ("rule:dv-007",
     ...
     ["to the container width on a single rAF-debounced resize", "TEXT NEVER SCALES (viewBox pinned 1:1)"]),
    ("rule:dv-008",
