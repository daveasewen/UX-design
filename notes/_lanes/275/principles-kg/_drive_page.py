#!/usr/bin/env python3
"""_drive_page.py — DRIVE the #275 lane RP review page, then screenshot it.
...
Then it writes screenshot.png (full page, desktop, LIGHT) beside the HTML.
    ...
    source knowledge/_render/seat_env.sh && \
      python3 notes/_lanes/275/principles-kg/_drive_page.py
"""
import json
import re
import sys
...
REPO = LANE.parents[3]
PAGE = LANE / "REVIEW-principles-into-graph-2026-09-15-v1.html"
...
def main():
    ...
    with sync_playwright() as p:
        ...
        pg.click("#RP-1-a")
        pg.fill('textarea.note[data-id="RP-3"]', "a note that must survive a reload")
        pg.click("#btnExport")
        ...
        shape_ok = (set(exp) == {"page", "at", "decisions"}
                    ...
                    and [x["id"] for x in exp["decisions"]] == [f"RP-{i}" for i in range(1, 7)]
        ...
        back = pg.evaluate("""() => ({
          a: document.querySelector('#RP-1-a').checked,
          n: document.querySelector('textarea.note[data-id="RP-3"]').value,
