#!/usr/bin/env python3
"""#292 wrap — fill this wrap's OWN placeholders and iterate the self-measuring lines to a
...
usage:  python3 fill292.py [--write]
"""
import os
import re
import subprocess
...
def sizes_line():
    p = subprocess.run(["python3", os.path.join(ROOT, "knowledge", "_gm_usage.py"),
                        "--sizes", "--session", "292"], capture_output=True, text=True, cwd=ROOT)
...
STAMP_TMPL = (
    ...
    "BYTE-IDENTICAL TO ITS STATE AT THE #272\u2026#291 WRAPS** (`sha256 {dig}\u2026` over the gate's OWN "
    "pinned probe shape `_capture_gate.section_a_digest`, returning exactly what all twenty of "
    "those stamps recorded \u2014 **\u00a7A has not moved in TWENTY-FOUR sessions**; the gate reports it "
    ...
    "`sha256 b9aef5f5\u2026` over 169 lines and #272\u2026#292 record `{dig}\u2026` over 198 \u2014 two probe "
    "shapes, not a change in \u00a7A, and **which shape the stamp should quote is ruling-shaped and "
    "remains Dave's, now at its TWENTY-FOURTH session, standing in `_CARRIES.md` \u00a7 "
    "`residual \u2192 #293`.** \u00b7 LS **{lsk}K tape** ({ls:,} real) \u00b7 corpus GM+LS **{ck}K tape** "
    ...
    "construction and `--check`-blocked** \u00b7 measured 2026-09-21 at the #292 wrap, DECLARE-LAST. "
    "**AGAINST #291's STAMP (35,404 / 68,471 / 103,875 tape) GM IS {dgm:+,} tape, LS {dls:+,} AND "
    "THE CORPUS {dc:+,}** \u2014 and the cause is nameable rather than mysterious: **#289's \u2605 PRIOR "
    "banner and #291's stratum rolled OUT of GM while #292 wrote a banner and a stratum of its "
    "own, and `s241-D2` puts gauge, declared-skip and 5b detail in the \u23f1 LATEST DELTA, which is "
    "`_LIVE-STATE.md`** \u2014 where #289's \u23f1 delta and #288's `Previous:` chain segment rolled out "
    "in the same pass, and where this session's own \u23f1 delta is the longest of the three. "
    "\u26d4 **#272\u2026#291's standing warning that the next wrap has no room in this idiom is NOT "
    "withdrawn, AND THIS WRAP MET IT HARDER THAN ANY BEFORE IT: the \u2605 LATEST banner was drafted "
    "OVER the cap and shortened SIX TIMES to fit, and it closed at {btk}/{blines} against the "
    "1,200/10 cap \u2014 **TWO tokens of headroom**, the tightest reading this record carries, and "
    "only by MERGING two bullets into one so the session's nine ruling-shaped things are "
    "represented by five.** \u26d4 **NEITHER FILE SIZE IS A TRIM ORDER; all of these are "
    "measurements, and what to do about them is Dave's.** \u26a0 **AND TWO DRIFTS ARE DECLARED "
    "RATHER THAN LEFT TO BE DISCOVERED.** (1) **Both this stamp and the `section-sizes` line "
    "measure regions that CONTAIN them**, so each was ITERATED TO A FIXED POINT rather than "
    "published at a reading taken before it was written \u2014 the #240 lesson, mechanised in "
    "`notes/_lanes/292/W/fill292.py`, and both are DECLARED whole-file writes rather than mover "
    "ops because the mover has no iterate-to-fixed-point op. (2) **The `s214-D6` chain figure and "
    "STEP 5b both land AFTER this stamp** (the stamp is itself inside the chain slice \u2014 the "
    "#241 rule), so they move GM and `_LIVE-STATE.md` by single-digit-percent tape against a 10% "
...
def fill_once():
    ...
    # 1 — section-sizes
    line = sizes_line()
    gm = re.sub(r"^> \*\*section-sizes #292 \(real\):\*\*.*$", lambda _: line, gm, count=1, flags=re.M)
    gm = gm.replace("> **section-sizes #292 (real):** {{SECTION_SIZES}}", line)
    # 2 — banner headroom
...
def stamp_once():
    ...
    new = STAMP_TMPL.format(gmk=f"{g/1000:.1f}", gm=g, ak=f"{a/1000:.1f}", a=a,
                            ...
                            dgm=g - 35404, dls=l - 68471, dc=(g + l) - 103875,
