#!/usr/bin/env python3
"""#269 wrap — demote the #268 ★ LATEST banner to ★ PRIOR and write #269's in front.
...
line270 = [l for l in carries if l.startswith("> **residual → #270:**")]
assert len(line270) == 1
count = len(cg._carry_items(line270[0]))

banner = io.open(os.path.join(HERE, "banner269.txt"), encoding="utf-8").read().rstrip("\n")
...
assert len(i) == 1, i
assert "**#268**" in lines[i[0]], lines[i[0]][:120]
...
assert len(t) == 1, t
TITLE = "`Apollo - #270: roles and desk fields into the graph`"
lines[t[0]] = "> **TITLE THE NEXT CHAT →** " + TITLE

# ⚠ The `> **YOU ARE #N. TITLE THIS CHAT →**` line is NOT in this file — `_gen_chain.py`
