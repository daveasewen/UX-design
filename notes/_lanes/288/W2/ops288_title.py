#!/usr/bin/env python3
"""#288 wrap — step 4b: write the FORWARD title into GOOD-MORNING.md, through the mover.
...
assert fwd.startswith("Apollo - #289"), repr(fwd)
assert rec["meta"]["declared_session"] == 288 and rec["meta"]["next_session"] == 289
...
OPSF = os.path.join(HERE, f"ops-288-title-{int(time.time())}.json")
