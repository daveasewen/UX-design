#!/usr/bin/env python3
"""#285 wrap — write a UNIQUELY-NAMED, SESSION-OWNED ops file, assert it exists with a size
...
⚠ #285 ADDITION, from lane V's carried note: `_seam.py` SCRATCH deletes a lane's own `/tmp`
working files mid-run. Every ops file this wrap writes is therefore session-owned under
`notes/_lanes/285/W/` — inside the repo, never `/tmp` or `/var/tmp` — which is also the
`s218-D7` clause 3 rule for evidence. The two reasons agree; the path is the same either way.
"""
...
def run(name, ops, write=False, min_bytes=100):
    path = os.path.join(HERE, f"ops-285-{name}-{int(time.time())}.json")
