#!/usr/bin/env python3
"""#281 lane CM — THE CENSUS RE-RUN, against the rebuilt page.
...
chip settings, the same three predicates. It is a SEPARATE FILE because lane CM may not write inside
lane OC's folder and running _census.py itself would rewrite lane OC's page and facts.json — a lane's
filed evidence is dated history.
...
MEASURES only. Writes facts-after.json beside this file and prints the table.
  ...
  python3 notes/_lanes/281/chip-map/_recensus.py [<explorer.html>] [<out.json>] [<template.html>]
...
def measure(famOn):
    ...
    eon = lambda e: famOn.get(FAMILY.get(e['type']), 0) and ((not DR(e)) or famOn['designrulings']) and not e.get('held')
...
WATCH = ['shape', 'intent', 'role', 'ux', 'rule', 'logo', 'component', 'snippet']
facts = {
...
print("the three dot sets the lane had to light (degree > 0 at 'every chip on'):")
for t in ('shape', 'intent', 'role'):
    c = facts['watch'][t]['all']
