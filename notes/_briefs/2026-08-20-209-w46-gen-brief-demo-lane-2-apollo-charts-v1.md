<!-- GEN-BRIEF BEGIN TITLE-GOVERNANCE · owner=machine · sha256=09a3c7ef70db0083d38c94208fcdf54f642736fde86afaf5dd16f82e77033105 · writer=knowledge/gen_brief.py -->
# PM brief — lane `lane-2-apollo-charts` (Apollo charts)

> ⛔ **THIS FILE IS A DATED PERIOD RECORD, NOT A LIVE HOME.** It was minted at `2026-08-20T07:39:01+01:00` by `knowledge/gen_brief.py`.
...
| this artefact | `notes/_briefs/2026-08-20-209-w46-gen-brief-demo-lane-2-apollo-charts-v1.md` |
...
| repo HEAD at mint | `eba1ef0 after #209 2026-08-20 — s198-D2 enacted in the state-snap gate: minted hover-light/hover-dark are legal snap targets …` |
...
*Human-owned. Written by hand at #209 by the W-46 build sub, as the demonstration that this
block survives a re-mint untouched.*

This brief is the FIRST REAL DRIVE of `knowledge/gen_brief.py` (W-46 proposal 2, `s204-D1` item 4),
minted on lane `lane-2-apollo-charts`. It exists so the generator was driven on a real lane in the
same wave it was built — [[instrument-without-a-consumer]] answered by a drive, not by a promise.

It is a DEMONSTRATION ARTEFACT. It does not brief anybody's live work and it settles nothing about
the Apollo charts lane; the lane facts it prints were read from `knowledge/_lanes.json` at mint.
<!-- GEN-BRIEF END THE-JOB -->

<!-- GEN-BRIEF BEGIN PREMISE-TABLE · owner=machine · sha256=678d3e3a4b348666d7eb2d3ce2d8e915b47c1ceb9d94d9c93c5d36cfa1490d4e · writer=knowledge/gen_brief.py -->
...
|---|---|---|---|---|---|
| `P-1` | `python3 knowledge/_probe_registry/probe_meta_schema.py --check` | `0` | `2026-08-20T07:39:01+01:00` | 0.19s | PROBE P-1 — findings=0 |
| `P-2` | `python3 knowledge/_probe_registry/probe_dup_ids.py --check` | `0` | `2026-08-20T07:39:01+01:00` | 0.14s | PROBE P-2 — findings=0 |
| `P-3` | `python3 knowledge/_probe_registry/probe_dangling_var_pixel.py --check` | `NOT RUN AT MINT` | `2026-08-20T07:39:01+01:00` | — | NOT RUN AT MINT — environment `sandbox-render`. This probe cannot be asked in a plain shell; see `s204-D1` item 5 (the CI pixel leg). A DECLARED GAP, never an omission. |
| `P-4` | `python3 knowledge/_probe_registry/probe_premise_store.py --check` | `0` | `2026-08-20T07:39:01+01:00` | 0.11s | PROBE P-4 — findings=0 |
| `P-5` | `python3 knowledge/_probe_registry/probe_stale_figure.py --check` | `0` | `2026-08-20T07:39:01+01:00` | 0.30s | PROBE P-5 — findings=0 |
...
<!-- GEN-BRIEF BEGIN OPEN-ITEMS · owner=machine · sha256=0939ca692edd49d64094a92ecdd5d620f22e6d5a73561a2d118145b469526281 · writer=knowledge/gen_brief.py -->
## OPEN ITEMS — read from `knowledge/_state.json` at mint

Counts are GENERATED, never typed: **78 items total · 54 live · 52 open · 1 blocked · 1 parked**, measured at `2026-08-20T07:39:01+01:00`.
...
<!-- GEN-BRIEF BEGIN DO-NOT-RULE · owner=machine · sha256=21f1fe7b84fd250fca0d271f2006a4035122f8e125f165ad9c36219755c99111 · writer=knowledge/gen_brief.py -->
...
**1 · Store items `state=open` AND `owner=dave` — 22 item(s):**
...
- ⬛ **Whether `gen_brief.py` is wired into `knowledge/_build_all.py` is DAVE'S** — the #209
  build deliberately did not wire it; a build step is PRICED in the #209 report and nothing else.
- ⬛ The five W-44 schema choices, W-49's repair-or-park, the twice-caught vocabulary question and
  PM-topology permanence stay untouched here — this generator routes around them, never through.
- ⬛ Promotion of any probe to a BLOCKING gate is Dave's under derivation governance. This brief
  RUNS probes; it never promotes one.
<!-- GEN-BRIEF END DO-NOT-RULE-APPEND -->

<!-- GEN-BRIEF BEGIN FENCES-ENVIRONMENT · owner=machine · sha256=b80c0475328226fc31cc9ef5c435d9345d711d36249cca9b766ce4526e603276 · writer=knowledge/gen_brief.py -->
...
|---|---|---|
| minted at | `2026-08-20T07:39:01+01:00` | the generator's clock |
...
| HEAD | `eba1ef0` | `git rev-parse --short HEAD` (rc=0) |
| dirty paths | `5` | `git status --short` (rc=0) |
...
*Human-owned. For this demonstration artefact the return contract is the #209 build report itself.*

A sub briefed from a minted brief owes back: a claim table with a probeable token per claim
(`s182-D1`) · every path it wrote, absolute · what it DROVE and every refusal it observed · the
commit hash and the subject READ BACK from `git log` as text · what stays UNPROVEN, named ·
the consequences and pitfalls of what it built.
