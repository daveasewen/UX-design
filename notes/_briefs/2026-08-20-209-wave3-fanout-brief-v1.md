<!-- GEN-BRIEF BEGIN TITLE-GOVERNANCE · owner=machine · sha256=2b9d08ef2a90f5df251f060c46dc8ac4ca989f6bed5c731af922954e68ba6ba5 · writer=knowledge/gen_brief.py -->
# PM brief — lane `lane-2-apollo-charts` (Apollo charts)

> ⛔ **THIS FILE IS A DATED PERIOD RECORD, NOT A LIVE HOME.** It was minted at `2026-08-20T12:25:20+01:00` by `knowledge/gen_brief.py`.
...
| this artefact | `notes/_briefs/2026-08-20-209-wave3-fanout-brief-v1.md` |
...
| repo HEAD at mint | `ed20b76 after #209 2026-08-20 — input leading-trim: the CANARY (probe P-6) + the 11 blockless snippets` |
...
WAVE 3 FAN-OUT (§C·1 strand b), Dave fired it live #209: "Fire as divvied". Three worker
lanes, nine NEW components, the wave-2 pattern (workers = NEW snippet + meta + receipt files
ONLY; the serial set — registry · MIGRATED_SNIPPETS · CATEGORIES · spine · git — is the
CONDUCTOR'S, one commit). Survey basis: reviews/ITINERARY-STATUS-2026-08-19-v1.json re-probed
at #209 — 6 of its 22 GAPs were already built at #204; the 16 real gaps are measured, and this
wave takes 9 (the heavy 7 — calendar, tree, cascader, carousel, qr-code, splitter, image-block
— are DEFERRED to wave 4, deliberately).

LANE A (Opus) — the fintech-row family, includes THE ONLY REMAINING P2:
  · Transaction / ledger row (itinerary row 91, slug transaction-row, P2)
  · Standing-order / mandate row (row 95, slug standing-order-mandate-row, P3)
  · Limits meter (row 96, slug limits-meter, P3)
  Start from the #204 fintech precedents on disk: document-row, runway-bar, payment-card-visual
  (COPY their idioms — masking, status chips, store rows, a11y group labelling — never re-draw).
  ⛔ FINTECH SEMANTICS ARE DAVE'S (the Kpi-tile precedent, s182-D2): every semantic binding
  (which status maps to which tint, delta conventions, monetary text colour) is authored
  PROPOSED-not-ruled in the meta's own $note, exactly as payment-card-visual does it.

LANE B (Sonnet) — selection controls:
  · Range slider (row 24, slug range-slider) — ACCRETE from the gated Slider, do not reinvent
  · Rating (row 25, slug rating)
  · Transfer / dual list (row 27, slug transfer-list) — List-items + Selection-controls idioms

LANE C (Sonnet) — action chrome:
  · Split button (row 6, slug split-button) — Button + Dropdown composition
  · FAB (row 7, slug fab) — Icon-button idiom, mobile placement notes
  · Back-to-top (row 38, slug back-to-top) — Icon-button idiom

EVERY new snippet: the CURRENT leading-trim block (copy line 36 of
knowledge/snippets/Command-palette.reference.html VERBATIM — it includes the #209 input
vocabulary) · .t-cm-*/.t-ed-* composites only, no raw font declarations · 4px grid (the
DEF-005 gate is absolute; 2px half-step for spacing only) · token-manifest script block ·
binds addresses in the DOTTED grammar (rag.success, never rag/success — the #209 lesson) ·
NO `intent` field (none of these is a chart; the W-58 parking is the precedent) · a11y per
the neighbour components (roles, aria, focus, 44px controls) · four-theme correctness via
tokens, verified in light AND dark.
<!-- GEN-BRIEF END THE-JOB -->

<!-- GEN-BRIEF BEGIN PREMISE-TABLE · owner=machine · sha256=0b3d8954ec1e02b5a621b20d290b51822e0fb5e7124df3ed2f7d98ba9a0b930d · writer=knowledge/gen_brief.py -->
...
|---|---|---|---|---|---|
| `P-1` | `python3 knowledge/_probe_registry/probe_meta_schema.py --check` | `0` | `2026-08-20T12:25:20+01:00` | 0.28s | PROBE P-1 — findings=0 |
| `P-2` | `python3 knowledge/_probe_registry/probe_dup_ids.py --check` | `0` | `2026-08-20T12:25:20+01:00` | 0.19s | PROBE P-2 — findings=0 |
| `P-3` | `python3 knowledge/_probe_registry/probe_dangling_var_pixel.py --check` | `NOT RUN AT MINT` | `2026-08-20T12:25:20+01:00` | — | NOT RUN AT MINT — environment `sandbox-render`. This probe cannot be asked in a plain shell; see `s204-D1` item 5 (the CI pixel leg). A DECLARED GAP, never an omission. |
| `P-4` | `python3 knowledge/_probe_registry/probe_premise_store.py --check` | `0` | `2026-08-20T12:25:21+01:00` | 0.11s | PROBE P-4 — findings=0 |
| `P-5` | `python3 knowledge/_probe_registry/probe_stale_figure.py --check` | `0` | `2026-08-20T12:25:21+01:00` | 0.30s | PROBE P-5 — findings=0 |
| `P-6` | `python3 knowledge/_probe_registry/probe_input_trim_enactment.py --check` | `NOT RUN AT MINT` | `2026-08-20T12:25:20+01:00` | — | NOT RUN AT MINT — environment `sandbox-render`. This probe cannot be asked in a plain shell; see `s204-D1` item 5 (the CI pixel leg). A DECLARED GAP, never an omission. |
...
<!-- GEN-BRIEF BEGIN OPEN-ITEMS · owner=machine · sha256=605a148660d92f30d6afe1f3a3f928ddbd3a71d016457d25f8d13afbccd05318 · writer=knowledge/gen_brief.py -->
## OPEN ITEMS — read from `knowledge/_state.json` at mint

Counts are GENERATED, never typed: **82 items total · 58 live · 55 open · 1 blocked · 2 parked**, measured at `2026-08-20T12:25:20+01:00`.
...
| `W-60` | dave | open | stated | Dave's eye on the 29 4px-grid snaps (#209) — the review page is the surface |
| `W-61` | dave | open | stated | Dave's eye on leading-trim applied to text-bearing inputs (#209) — keep the inert vocabulary, or revert |
...
<!-- GEN-BRIEF BEGIN DO-NOT-RULE · owner=machine · sha256=5c12d1c8e2ac544f2a4fd4e44699689437a0547f643b6adeeb42391e97ec9f47 · writer=knowledge/gen_brief.py -->
## DO-NOT-RULE — generated half

⛔ **`by: Dave` IS NOT USED AS A FILTER, and that is a correction to `s204-D1`'s own item-4 wording.** Measured at mint over `knowledge/_rulings.json`: **Dave=206** across 206 rulings. A field with one value selects everything and is therefore not a filter (#207 finding (b)).

The two generated sources are:

**1 · Store items `state=open` AND `owner=dave` — 24 item(s):**
...
| `W-60` | stated | Dave's eye on the 29 4px-grid snaps (#209) — the review page is the surface |
| `W-61` | stated | Dave's eye on leading-trim applied to text-bearing inputs (#209) — keep the inert vocabulary, or revert |

**2 · Rulings carrying a non-empty `open` field — 18 of 206:**
...
- Fintech semantic bindings (status→tint maps, delta conventions, monetary colour seats) —
  PROPOSED in $notes only; ruling is Dave's (s182-D2 precedent).
- The `intent` vocabulary — closed set, W-58 parked; do not author the field at all.
- The two delta conventions question (D-Q5, charts data/delta vs Stat-card rag arrows) is OPEN
  AND DAVE'S — the limits-meter and transaction-row must not settle it by construction; where a
  delta appears, follow the nearest gated neighbour and name the question in the meta.
- No token minting. Every value binds an existing token; "NO NEW TOKEN IS WANTED OR MINTED"
  (payment-card-visual's tokens.$note) is the standard to copy.
- No registry/MIGRATED_SNIPPETS/CATEGORIES/spine edits — conductor's serial set.
- No git operations of any kind in worker lanes. No _rulings.json. No review-signoff.
<!-- GEN-BRIEF END DO-NOT-RULE-APPEND -->

<!-- GEN-BRIEF BEGIN FENCES-ENVIRONMENT · owner=machine · sha256=a26b84a0e107118d25b8bd9805dae6417f42ffd0e570254a3b6a70662ab938c7 · writer=knowledge/gen_brief.py -->
...
|---|---|---|
| minted at | `2026-08-20T12:25:20+01:00` | the generator's clock |
| python | `3.10.12` | `sys.version.split()[0]` |
| platform | `linux` | `sys.platform` |
| repo root | `/sessions/festive-friendly-johnson/mnt/UX-design` | resolved from `knowledge/gen_brief.py` |
| HEAD | `ed20b76` | `git rev-parse --short HEAD` (rc=0) |
| dirty paths | `2` | `git status --short` (rc=0) |
...
Each worker lane owes back: (1) the file list — <Name>.reference.html + <slug>.meta.json per
member + ONE receipt at notes/_receipts/2026-08-20-209-wave3-lane<X>-<slugs>.md; (2) a claim
table with a probeable token per claim (s182-D1); (3) what was driven — at minimum the snippet
opened in a real render where the browser is reachable (env in the conductor's message), or an
honest UNPROVEN if not; (4) every fintech/design question raised as a NAMED question in the
receipt, never a settled default; (5) what stays UNPROVEN. No commits — the conductor
reconciles, runs the serial set, gates, and lands ONE commit.
