<!-- GEN-BRIEF BEGIN TITLE-GOVERNANCE · owner=machine · sha256=d7a5b43355cf960987fb1e5301388688b3224983720bca22037d0968b3f1d573 · writer=knowledge/gen_brief.py -->
# PM brief — lane `lane-2-apollo-charts` (Apollo charts)

> ⛔ **THIS FILE IS A DATED PERIOD RECORD, NOT A LIVE HOME.** It was minted at `2026-08-20T20:26:27+01:00` by `knowledge/gen_brief.py`.
...
| this artefact | `notes/_briefs/2026-08-20-210-wave6-layer2-p3-fanout-brief-v1.md` |
...
| repo HEAD at mint | `632ce97 after #210 2026-08-20 — the two REAL gating reds behind d32689b repaired at cause: 17 icon UNKNOWNs cleared (6 mirror…` |
...
WAVE 6: the seventeen Layer-2 P3 rows (derived itinerary, NO-ARTEFACT-CLASS). Row 124 (per-component variant matrices) is EXCLUDED — deferred to Dave, it is a review artefact not a component. Dave's word #210 stands: "lets just fill-er-up and I'll review later."

FOUR LANES:
- **Lane A (Opus): P3 shells** — row 100 split · 101 focused/full-page · 102 doormat (mega-footer) nav · 103 nav rail.
- **Lane B (Opus): P3 templates** — row 108 settings · 110 empty · 111 error (404/500) · 113 report · 114 confirmation.
- **Lane C (Sonnet): P3 lock-ups I** — row 116 section headings · 117 card headers · 118 hero variants · 119 stats band.
- **Lane D (Sonnet): P3 lock-ups II** — row 121 footer/doormat · 122 CTA · 123 feature grid.

Same artefact grammar as wave 5 (reference snippet + meta, `$layer` key). COMPOSE, NEVER RE-DRAW — study the wave-5 receipts first; the app shells, templates and lock-ups that already exist are your composition sources alongside the 108 components.

WAVE-5 LESSONS, BINDING:
1. The meta key is **`$layer`** — a bare `layer` fails P-1 (schema `additionalProperties:false`, `^\$` escape only). Same for any custom key: `$`-prefix, and NOTHING custom inside `relationships`.
2. **ds-005 is live**: the trim block's `:is()` weighs (0,1,2) — any trim override must be the TWO-CLASS form with the computed edge MEASURED.
3. **container-type self-query trap** (caught twice in wave 5): an element is a query container for its DESCENDANTS, never itself — a collapse rule targeting the container itself never fires and looks plausible in screenshots. Measure the collapse.
4. **Icons**: byte-match from the library OR mirror an existing declared-bespoke form (`data-bespoke="<reason>"` — see Dropdown's tick). An undeclared glyph fails the icon-source gate.
5. **Type ratchet**: composing an atom must not re-type its raw font declarations — bind composites in markup (Carousel/Page-header-lockup show the repaired form). Debt must measure exactly 1,097 after.
6. **Blast radius**: your compositions WILL extend global selectors' reach (.badge, .seg, etc.) — that gate escape is EXPECTED; declare which selectors you extended in the report; the conductor re-seeds. Do NOT run --update yourselves.
7. **The sticky/overlay lesson**: anything sticky or overlaid gets a scrolling demo frame and a measured non-overlap.
<!-- GEN-BRIEF END THE-JOB -->

<!-- GEN-BRIEF BEGIN PREMISE-TABLE · owner=machine · sha256=7663a3622639238f8c7238a3dec05c44ccc010fd342aaec7f8384b4a7ba2ec48 · writer=knowledge/gen_brief.py -->
...
|---|---|---|---|---|---|
| `P-1` | `python3 knowledge/_probe_registry/probe_meta_schema.py --check` | `0` | `2026-08-20T20:26:27+01:00` | 0.27s | PROBE P-1 — findings=0 |
| `P-2` | `python3 knowledge/_probe_registry/probe_dup_ids.py --check` | `0` | `2026-08-20T20:26:28+01:00` | 0.25s | PROBE P-2 — findings=0 |
| `P-3` | `python3 knowledge/_probe_registry/probe_dangling_var_pixel.py --check` | `NOT RUN AT MINT` | `2026-08-20T20:26:27+01:00` | — | NOT RUN AT MINT — environment `sandbox-render`. This probe cannot be asked in a plain shell; see `s204-D1` item 5 (the CI pixel leg). A DECLARED GAP, never an omission. |
| `P-4` | `python3 knowledge/_probe_registry/probe_premise_store.py --check` | `1` | `2026-08-20T20:26:28+01:00` | 0.15s | PROBE P-4 — findings=2 |
| `P-5` | `python3 knowledge/_probe_registry/probe_stale_figure.py --check` | `0` | `2026-08-20T20:26:28+01:00` | 0.46s | PROBE P-5 — findings=0 |
| `P-6` | `python3 knowledge/_probe_registry/probe_input_trim_enactment.py --check` | `NOT RUN AT MINT` | `2026-08-20T20:26:27+01:00` | — | NOT RUN AT MINT — environment `sandbox-render`. This probe cannot be asked in a plain shell; see `s204-D1` item 5 (the CI pixel leg). A DECLARED GAP, never an omission. |
...
<!-- GEN-BRIEF BEGIN OPEN-ITEMS · owner=machine · sha256=3023442700fa998082b65f23f5c38d01b1716154453b582af75abcaf7cae14d4 · writer=knowledge/gen_brief.py -->
## OPEN ITEMS — read from `knowledge/_state.json` at mint

Counts are GENERATED, never typed: **100 items total · 72 live · 67 open · 1 blocked · 4 parked**, measured at `2026-08-20T20:26:27+01:00`.
...
<!-- GEN-BRIEF BEGIN DO-NOT-RULE · owner=machine · sha256=232da6fd2aa540d1d16e99ac34e80e26aad127ff3234d9dd36a2d24d815d4628 · writer=knowledge/gen_brief.py -->
...
**1 · Store items `state=open` AND `owner=dave` — 35 item(s):**
...
- Every fintech/domain semantic PROPOSED (`s182-D2`); no colour invented; two-red law + mono ink camp untouched.
- Leading-trim block byte-identical to Command-palette line 36.
- Width is the container's (`s210-D3` discipline); demo chrome legibly commented.
- Brand mark: official HSBC masterbrand only; ⛔ crescent is the Apollo anonymity mark (#86), never product brand. Error/empty templates: no invented support copy semantics — neutral placeholder prose.
- No `intent` fields (W-58 parked) · no registration (CATEGORIES / MIGRATED_SNIPPETS / component-types / canon.css / _rulings.json / git).
- Hero variants (118): Hero component EXISTS — variants compose/extend it, relationship PROPOSED, no adjudication.
- Footer/doormat (121): Footer component EXISTS (and carries placeholder hrefs, a known open item) — compose it, do not repair its placeholders.
- Feature grid (123) and stats band (119): compose Cards/Stat-card/Kpi-tile — no new molecules.
<!-- GEN-BRIEF END DO-NOT-RULE-APPEND -->

<!-- GEN-BRIEF BEGIN FENCES-ENVIRONMENT · owner=machine · sha256=7a7be4dafc266a4d272324c58d172c8de38c551d8ecd482875111a1b9465c828 · writer=knowledge/gen_brief.py -->
...
|---|---|---|
| minted at | `2026-08-20T20:26:27+01:00` | the generator's clock |
...
| HEAD | `632ce97` | `git rev-parse --short HEAD` (rc=0) |
| dirty paths | `1` | `git status --short` (rc=0) |
...
Each lane owes back: (1) file list with bytes; (2) claim table with re-runnable probes + borrowed-region diff-proofs; (3) DRIVEN browser evidence per knowledge/_RUNBOOK-render-verify.md — goto(file://) only, light AND dark, 2-3 widths, collapse MEASURED (lesson 3), screenshots LOOKED AT; (4) gate outputs VERBATIM (grid · snippets · a11y · descender · type before/after=1,097 · P-1 · icons rc=0 for your files · binds check-D expected-fail declared); (5) receipt notes/_receipts/2026-08-20-210-wave6-lane<X>-*.md with store doc-row minted via _state.py add() — derive the next free W-id from a live re-read AT WRITE TIME and name it (four lanes run concurrently); (6) $decisionsForDave named, unanswered; (7) blast-radius selectors you extended, named (lesson 6); (8) UNPROVEN declared. NO commits, NO rulings, NO registrations. Sandbox: /tmp + root fs near-full — reuse /var/tmp/pw-browsers-s197 chromium via symlink; scratch on the outputs mount.
